...
/**
 * Populates the example database with FX option volatility surface definitions and specifications for a given list of currency pairs.
 * <p>
 * The specifications have expiry tenors as the x axis and risk reversal / butterfly / ATM quotes on the y axis. The instrument provider is
 * {@link ExampleFXOptionVolatilitySurfaceInstrumentProvider}.
 * <p>
 * The definitions have x axis tenors of (1w, 2w, 3w, 1m, 3m, 6m, 9m, 1y, 5y, 10y) and y axis tenors of (25B, 15B, 25RR, 25B, ATM).
 */
public class ExampleFXOptionVolatilitySurfaceConfigPopulator {
  ...
  private static void populateVolatilitySurfaceSpecifications(final ConfigMaster configMaster, final UnorderedCurrencyPair target, final String name) {
    final SurfaceInstrumentProvider<Tenor, Pair<Number, FXVolQuoteType>> surfaceInstrumentProvider = new ExampleFXOptionVolatilitySurfaceInstrumentProvider(
        target.toString(), "FXVOL",
        MarketDataRequirementNames.MARKET_VALUE);
    final String fullName = name + SEPARATOR + target.toString() + SEPARATOR + InstrumentTypeProperties.FOREX;
    final VolatilitySurfaceSpecification spec = new VolatilitySurfaceSpecification(fullName, target, SurfaceAndCubeQuoteType.MARKET_STRANGLE_RISK_REVERSAL,
   ...
   */
  private static ConfigItem<VolatilitySurfaceDefinition<Tenor, Pair<Number, FXVolQuoteType>>> makeConfigDocument(
      final VolatilitySurfaceDefinition<Tenor, Pair<Number, FXVolQuoteType>> definition) {
