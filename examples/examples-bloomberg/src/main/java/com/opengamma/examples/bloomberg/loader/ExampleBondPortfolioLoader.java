...
public class ExampleBondPortfolioLoader extends AbstractTool<IntegrationToolContext> {
  ...
  /** Logger. */
  private static final Logger LOGGER = LoggerFactory.getLogger(ExampleBondPortfolioLoader.class);
  ...
  protected void doRun() {
    ...
    for (final SecurityDocument shellDoc : loadAllBondSecurities()) {
  ...
  protected Iterable<SecurityDocument> loadAllBondSecurities() {
    final SecuritySearchRequest secSearch = new SecuritySearchRequest();
   ...
   */
  protected BondSecurity loadFullSecurity(final SecurityDocument shellDoc) {
    LOGGER.info("Loading security {} {}", shellDoc.getUniqueId(), shellDoc.getSecurity().getName());
    final SecurityDocument doc = getToolContext().getSecurityMaster().get(shellDoc.getUniqueId());
    final BondSecurity sec = (BondSecurity) doc.getSecurity();
    return sec;
  }
   ...
   */
  protected ManageablePortfolioNode buildPortfolioTree(final ManageablePortfolioNode rootNode, final BondSecurity security) {
   ...
   */
  protected ManageablePosition createPosition(final BondSecurity security) {
    LOGGER.info("Creating position {}", security);
    final int shares = (RandomUtils.nextInt(490) + 10) * 10;
    final ExternalId buid = security.getExternalIdBundle().getExternalId(ExternalSchemes.BLOOMBERG_BUID);
