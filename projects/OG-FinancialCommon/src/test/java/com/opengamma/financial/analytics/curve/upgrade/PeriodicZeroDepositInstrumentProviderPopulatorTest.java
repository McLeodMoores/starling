...
import static com.opengamma.financial.analytics.curve.upgrade.CurveUpgradeTestUtils.EUR_PERIODIC_ZERO_INSTRUMENTS;
...
public class PeriodicZeroDepositInstrumentProviderPopulatorTest {
  ...
  public void testHashCodeEquals() {
    ...
    assertNotEquals(RENAMING_PROVIDER, new PeriodicZeroDepositInstrumentProviderPopulator());
  }
  ...
  public void testNoPeriodicZeroDeposits() {
    final CurveSpecificationBuilderConfiguration csbc = new CurveSpecificationBuilderConfiguration(USD_DEPOSIT_INSTRUMENTS,
        ...
        USD_TENOR_SWAP_INSTRUMENTS, USD_OIS_INSTRUMENTS, USD_SIMPLE_ZERO_INSTRUMENTS, null, USD_CONTINUOUS_ZERO_INSTRUMENTS, USD_12M_SWAP_INSTRUMENTS,
   ...
   */
  @Test
  public void testPeriodicZeroDepositStrips() {
    assertEquals(USD_PERIODIC_ZERO_INSTRUMENTS, DEFAULT_PROVIDER.getInstrumentProviders(CSBC));
  }
  ...
  public void testBuilderCreation() {
    ...
    assertNull(EMPTY_MAPPER.getPeriodicallyCompoundedRateNodeIds());
    // test rename
  ...
  /**
   * Tests that an existing periodic zero rate mapper is over-written.
   */
  @Test
  public void testOverwrite() {
    ...
    final CurveNodeIdMapper mapperWithExistingNodes = CurveNodeIdMapper.builder()
        .name(NAME)
        .continuouslyCompoundedRateNodeIds(EUR_CONTINUOUS_ZERO_INSTRUMENTS) // will be copied
        .periodicallyCompoundedRateNodeIds(EUR_PERIODIC_ZERO_INSTRUMENTS) // will be over-written with USD nodes
        .build();
    final CurveNodeIdMapper expectedMapper = CurveNodeIdMapper.builder()
        .name(expectedName)
        .continuouslyCompoundedRateNodeIds(EUR_CONTINUOUS_ZERO_INSTRUMENTS)
        .periodicallyCompoundedRateNodeIds(USD_PERIODIC_ZERO_INSTRUMENTS)
        .build();
