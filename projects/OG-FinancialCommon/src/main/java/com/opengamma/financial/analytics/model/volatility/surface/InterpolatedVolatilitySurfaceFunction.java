 ...
 * (i.e. one that has time / strike axes and implied volatility values) and a 2D grid interpolator.
 */
public class InterpolatedVolatilitySurfaceFunction extends AbstractFunction.NonCompiledInvoker {
  ...
  public boolean canApplyTo(final FunctionCompilationContext context, final ComputationTarget target) {
    ...
    return scheme.equalsIgnoreCase(ExternalSchemes.BLOOMBERG_TICKER.getName())
        ...
        || scheme.equalsIgnoreCase(ExternalSchemes.OG_SYNTHETIC_TICKER.getName());
  }
  ...
  private ValueProperties getResultProperties(final String surfaceName, final String instrumentType, final String leftXExtrapolatorName,
      final String rightXExtrapolatorName, final String xInterpolatorName, final String leftYExtrapolatorName, final String rightYExtrapolatorName,
      final String yInterpolatorName) {
    final ValueProperties properties = createValueProperties()
        .with(ValuePropertyNames.SURFACE, surfaceName)
        .with(InstrumentTypeProperties.PROPERTY_SURFACE_INSTRUMENT_TYPE, instrumentType)
        .with(InterpolatedDataProperties.LEFT_X_EXTRAPOLATOR_NAME, leftXExtrapolatorName)
        .with(InterpolatedDataProperties.RIGHT_X_EXTRAPOLATOR_NAME, rightXExtrapolatorName)
        .with(InterpolatedDataProperties.X_INTERPOLATOR_NAME, xInterpolatorName)
        .with(InterpolatedDataProperties.LEFT_Y_EXTRAPOLATOR_NAME, leftYExtrapolatorName)
        .with(InterpolatedDataProperties.RIGHT_Y_EXTRAPOLATOR_NAME, rightYExtrapolatorName)
        .with(InterpolatedDataProperties.Y_INTERPOLATOR_NAME, yInterpolatorName)
        .with(ValuePropertyNames.CALCULATION_METHOD, InterpolatedDataProperties.CALCULATION_METHOD_NAME)
        .get();
    return properties;
  }
}
