...
/**
 * Shifts points on a volatility surface using the index of the expiry to specify the shift amount. All points for an expiry are shifted by the the same amount
...
public final class VolatilitySurfaceIndexShifts implements StructureManipulator<VolatilitySurface>, ImmutableBean {
  ...
  /**
   * Creates a new set of shifts The first element of the list of shifts is applied to all points for the first expiry, the second element is applied to all
   * points for the second expiry and so on. The number of shifts doesn't have to match the number of expiries in the surface. Any extra shifts are ignored. If
   * there is no shift for an expiry then no change is made to the surface at that expiry.
   *
   * @param shiftType
   *          absolute or relative
   * @param shifts
   *          the shift to apply at each expiry in the surface
   */
  ...
  /**
   * Returns a shifted volatility surface. The surface returned by {@link VolatilitySurface#getSurface()} must not be an instance of
  ...
  static {
    MetaBean.register(VolatilitySurfaceIndexShifts.Meta.INSTANCE);
  }
  ...
  public static final class Builder extends DirectFieldsBeanBuilder<VolatilitySurfaceIndexShifts> {
    ...
    private List<Double> _shifts = ImmutableList.of();
