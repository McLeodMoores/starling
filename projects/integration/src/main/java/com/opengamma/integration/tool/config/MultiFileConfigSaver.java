...
public class MultiFileConfigSaver extends DirectBean {
  ...
  public void run() throws IOException {
    ...
    for (final Class<?> clazz : result.getConfigTypes()) {
      outputFilesForConfigClass(clazz);
    }
  }

  @SuppressWarnings({"rawtypes", "unchecked" })
  protected void outputFilesForConfigClass(final Class<?> clazz) throws IOException {
    System.out.println("Outputting files for " + clazz);
    final ConfigSearchRequest searchRequest = new ConfigSearchRequest(clazz);
    final ConfigSearchResult searchResult = getConfigMaster().search(searchRequest);
    final Set<ConfigDocument> latest = new HashSet<>();
    for (final Object documentObject : searchResult.getDocuments()) {
      final ConfigDocument document = (ConfigDocument) documentObject;
    ...
    final File directory = new File(getDestinationDirectory(), clazz.getName());
    directory.mkdir();
    for (final ConfigDocument document : latest) {
      //TODO: Should properly encode file name from name
      final File documentFile = new File(directory, document.getName().trim().replace("/", "") + ".xml");
      System.out.println("-- Creating file " + documentFile.getAbsolutePath());
      final FileOutputStream fos = new FileOutputStream(documentFile);
      final FudgeXMLStreamWriter xmlStreamWriter = new FudgeXMLStreamWriter(OpenGammaFudgeContext.getInstance(), new OutputStreamWriter(fos));
      final MutableFudgeMsg msg = serializer.objectToFudgeMsg(document.getConfig().getValue());
      final FudgeMsgWriter fudgeMsgWriter = new FudgeMsgWriter(xmlStreamWriter);
  ...
  static {
    MetaBean.register(MultiFileConfigSaver.Meta.INSTANCE);
  }
  ...
  public static class Meta extends DirectMetaBean {
    ...
    public BeanBuilder<? extends MultiFileConfigSaver> builder() {
      return new DirectBeanBuilder<>(new MultiFileConfigSaver());
    }
