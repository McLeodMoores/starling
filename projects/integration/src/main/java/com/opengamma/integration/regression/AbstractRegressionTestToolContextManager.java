...
/**
 * Manages the life-cycle of components. This class is based on the original implementation in <code>RegressionTestToolContextManager</code>. However, it does
 * not require the use of a reference data provider, which is only available if Bloomberg is used as a data provider.
 */
public abstract class AbstractRegressionTestToolContextManager {
   ...
   * 
   * @param dumpFile
   *          the dump file to use: a zip file, not null
   * @param toolContextPropertiesFile
   *          a tool context, for use initializing the regression db, not null
   * @param regressionPropertiesFile
   *          a full engine context, not null
   */
   ...
   * 
   * @throws IOException
   *           if the files cannot be found or created
   */
   ...
   * @param zipFile
   *          the zip file to restore from, not null
   * @throws IOException
   *           if there is a problem reading the file
   */
  protected abstract void restoreFromZipfile(ToolContext toolContext, File zipFile) throws IOException;
