...
public class SimpleViewStatusModelTest {
  ...
  private final ViewStatusModel _viewStatusModel = new SimpleViewStatusModel(HEADERS, ROWS, RESULT);
  ...
  private static List<Object> makeRowList(final String... colums) {
    final List<Object> row = Lists.newArrayListWithCapacity(4);
    for (final Object column : colums) {
      row.add(column);
    }
    return row;
  }
  ...
  public void getKeySet() {
    final Set<ViewStatusKey> keySet = _viewStatusModel.keySet();
    assertNotNull(keySet);
    assertEquals(RESULT.size(), keySet.size());
    for (final ViewStatusKey viewStatusKey : keySet) {
