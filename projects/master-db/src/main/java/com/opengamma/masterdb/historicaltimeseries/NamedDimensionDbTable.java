 ...
 * <p>
 * This class aims to simplify working with a simple dimension table. This kind of table consists of simple deduplicated data, keyed by id. The id is used to
 * reference the data on the main "fact" table.
 * <p>
...
public class NamedDimensionDbTable {
   ...
   *
   * @param dbConnector
   *          the database connector combining all configuration, not null
   * @param variableName
   *          the variable name, used as a placeholder in SQL, not null
   * @param tableName
   *          the table name, not null
   * @param sequenceName
   *          the sequence used to generate the id, may be null
   */
   ...
   *
   * @param name
   *          the name to lookup, not null
   * @return the id, null if not stored
   */
  public Long get(final String name) {
    final String select = sqlSelectGet();
    final DbMapSqlParameterSource args = new DbMapSqlParameterSource()
        .addValue(getVariableName(), name);
    final List<Map<String, Object>> result = getDbConnector().getJdbcTemplate().queryForList(select, args);
    if (result.size() == 1) {
   ...
   * <p>
   * The SQL requires a parameter of name {@link #getVariableName()}. The statement returns a single column of the id.
   *
  ...
  public Long search(final String name) {
    final String select = sqlSelectSearch(name);
    final DbMapSqlParameterSource args = new DbMapSqlParameterSource()
   ...
   *
   * @param name
   *          the name to ensure is present, not null
   * @return the id, null if not stored
   */
  ...
  public String sqlSelectNames() {
    return "SELECT name FROM " + getTableName() + " ORDER BY name";
  }
