...
public class DefaultDbSecurityBeanMasterCallback extends DbSecurityBeanMasterCallback {
  ...
  protected Map<String, String> getIndexedProperties(final ManageableSecurity value) {
    if (value instanceof BondSecurity) {
      final BondSecurity bond = (BondSecurity) value;
      final HashMap<String, String> map = Maps.newHashMapWithExpectedSize(2);
  ...
  protected void buildIndexedPropertiesSearch(final BeanMasterSearchRequest requestToBuild, final AbstractSearchRequest requestToExtractFrom) {
    if (requestToExtractFrom instanceof BondSecuritySearchRequest) {
      final BondSecuritySearchRequest extended = (BondSecuritySearchRequest) requestToExtractFrom;
