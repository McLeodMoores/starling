...
public class DbCreateTool extends AbstractDbTool<DbToolContext> {
  ...
  protected void doRun(final boolean write, final File outputFile) throws Exception {
    ...
    if (getCommandLine().hasOption(SCHEMA_NAME_FILTER)) {
      final String[] nameArgs = getCommandLine().getOptionValues(SCHEMA_NAME_FILTER);
      final Set<String> includeNames = ImmutableSet.copyOf(nameArgs);
      final Set<String> allNames = getDbToolContext().getSchemaNames();
      final SetView<String> filteredNames = Sets.intersection(allNames, includeNames);
      if (write) {
