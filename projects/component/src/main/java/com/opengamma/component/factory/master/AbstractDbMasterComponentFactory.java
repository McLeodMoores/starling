...
public abstract class AbstractDbMasterComponentFactory<I, M extends ConfigurableDbMaster> extends AbstractComponentFactory {
  ...
  protected AbstractDbMasterComponentFactory(final String schemaName, final Class<I> masterInterface) {
    _schemaName = schemaName;
    _masterInterface = masterInterface;
  }

  protected abstract Class<? extends AbstractRemoteMaster> getRemoteInterface();

  @Override
  public void init(final ComponentRepository repo, final LinkedHashMap<String, String> configuration) throws Exception {

    final ComponentInfo info = new ComponentInfo(_masterInterface, getClassifier());

    final M dbMaster = createMaster(repo, info);

    if (getUniqueIdScheme() != null) {
      dbMaster.setUniqueIdScheme(getUniqueIdScheme());
    }

    final String resolvedScheme = dbMaster.getUniqueIdScheme();
    ...
    if (isPublishRest()) {
      info.addAttribute(ComponentInfoAttributes.REMOTE_CLIENT_JAVA, getRemoteInterface());
    }
   ...
   */
  protected I postProcess(final M master) {
  ...
  static {
    MetaBean.register(AbstractDbMasterComponentFactory.Meta.INSTANCE);
  }
