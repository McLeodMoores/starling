...
public class DataComponentServerUris {
   ...
   */
  public static URI uri(final URI baseUri) {
    final UriBuilder bld = UriBuilder.fromUri(baseUri).path("components");
    return bld.build();
  }
   ...
   * @return the URI, not null
   */
  public static URI uri(final URI baseUri, final ComponentInfo info) {
    final UriBuilder bld = UriBuilder.fromUri(baseUri).path("components/{type}/{classifier}");
