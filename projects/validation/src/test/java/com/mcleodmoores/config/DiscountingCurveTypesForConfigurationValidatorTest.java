...
/**
 * Unit tests for {@link DiscountingCurveTypesForConfigurationValidator}.
 */
public class DiscountingCurveTypesForConfigurationValidatorTest {
  ...
  /** The validator */
  private static final ConfigurationValidator<CurveConstructionConfiguration, String> VALIDATOR =
      DiscountingCurveTypesForConfigurationValidator.getInstance();
  ...
  public void testOvernightCurveType() {
    ...
    final CurveConstructionConfiguration config = new CurveConstructionConfiguration("name", Collections.singletonList(group), Collections.<String>emptyList());
    final ConfigurationValidationInfo<String> validationInfo = VALIDATOR.validate(config, VersionCorrection.LATEST, EMPTY_CONFIG_SOURCE);
  ...
  /**
   * Tests that duplicated currencies are found.
   */
  @Test
  public void testDuplicatedCurrencies() {
    final String reference1 = "ABC";
    final String reference2 = "DEF";
    final List<CurveTypeConfiguration> type1 = new ArrayList<>();
    type1.add(new DiscountingCurveTypeConfiguration(reference1));
    final List<CurveTypeConfiguration> type2 = new ArrayList<>();
    type2.add(new DiscountingCurveTypeConfiguration(reference1));
    final List<CurveTypeConfiguration> type3 = new ArrayList<>();
    type3.add(new DiscountingCurveTypeConfiguration(reference1));
    ...
    ConfigurationValidationInfo<String> validationInfo = VALIDATOR.validate(config, VersionCorrection.LATEST, EMPTY_CONFIG_SOURCE);
    assertEquals(validationInfo.getMissingConfigurations(), Collections.emptyMap());
    assertEquals(validationInfo.getValidatedConfigurations(), Collections.emptySet());
    assertEquals(validationInfo.getUnsupportedConfigurations(), Collections.emptySet());
    ...
    types.put("Curve 1", type1);
    types.put("Curve 2", type2);
    types.put("Curve 3", Collections.singletonList(new DiscountingCurveTypeConfiguration(reference2)));
    ...
    types2.put("Curve 2", type2);
    types3 = new HashMap<>();
    types3.put("Curve 3", Collections.singletonList(new DiscountingCurveTypeConfiguration(reference2)));
  ...
  /**
   * Tests that duplicated currencies in exogenous configurations are found.
   */
  @Test
  public void testDuplicatedCurrenciesExogenousConfiguration() {
    final String reference1 = "ABC";
    final String reference2 = "DEF";
    final String reference3 = "GHI";
    final List<CurveTypeConfiguration> type1 = new ArrayList<>();
    type1.add(new DiscountingCurveTypeConfiguration(reference1));
    final List<CurveTypeConfiguration> type2 = new ArrayList<>();
    type2.add(new DiscountingCurveTypeConfiguration(reference1));
    final List<CurveTypeConfiguration> type3 = new ArrayList<>();
    type3.add(new DiscountingCurveTypeConfiguration(reference2));
    final List<CurveTypeConfiguration> type4 = new ArrayList<>();
    type4.add(new DiscountingCurveTypeConfiguration(reference2));
    ...
    ConfigurationValidationInfo<String> validationInfo = VALIDATOR.validate(config, VersionCorrection.LATEST, configSource);
    assertEquals(validationInfo.getMissingConfigurations(), Collections.emptyMap());
    assertEquals(validationInfo.getValidatedConfigurations(), Collections.emptySet());
    assertEquals(validationInfo.getUnsupportedConfigurations(), Collections.emptySet());
    ...
    group3 = new CurveGroupConfiguration(1, Collections.<String, List<? extends CurveTypeConfiguration>>singletonMap(
        "Curve 5", Collections.singletonList(new DiscountingCurveTypeConfiguration(reference3))));
    configMaster = new InMemoryConfigMaster();
    configMaster.add(new ConfigDocument(ConfigItem.of(new CurveConstructionConfiguration("name2", Arrays.asList(group2, group3),
        Collections.singletonList("name3")), "name2", CurveConstructionConfiguration.class)));
    config = new CurveConstructionConfiguration("name1", Collections.singletonList(group1), Arrays.asList("name2"));
