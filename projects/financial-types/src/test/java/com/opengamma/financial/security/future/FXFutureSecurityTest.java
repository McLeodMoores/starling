...
/**
 * Tests for {@link FXFutureSecurity}.
 */
@Test(groups = TestGroup.UNIT)
public class FXFutureSecurityTest extends AbstractBeanTestCase {
  ...
  private static final Currency NUMERATOR = Currency.BRL;
  private static final Currency DENOMINATOR = Currency.AUD;
  private static final String CONTRACT_CATEGORY = "cat";
  private static final FXFutureSecurity FUTURE = new FXFutureSecurity(EXPIRY, TRADING_EXCHANGE, SETTLEMENT_EXCHANGE, CCY, UNIT_AMOUNT, NUMERATOR, DENOMINATOR,
      CONTRACT_CATEGORY);

  @Override
  public JodaBeanProperties<? extends Bean> getJodaBeanProperties() {
    return new JodaBeanProperties<>(FXFutureSecurity.class,
        Arrays.asList("expiry", "tradingExchange", "settlementExchange", "currency", "unitAmount", "numerator", "denominator", "contractCategory"),
        Arrays.asList(EXPIRY, TRADING_EXCHANGE, SETTLEMENT_EXCHANGE, CCY, UNIT_AMOUNT, NUMERATOR, DENOMINATOR, CONTRACT_CATEGORY),
        Arrays.asList(new Expiry(DateUtils.getUTCDate(2020, 6, 15)), SETTLEMENT_EXCHANGE, TRADING_EXCHANGE, Currency.BRL, UNIT_AMOUNT * 2, DENOMINATOR,
            NUMERATOR, "con"));
  }
  ...
  public void testConstructor() {
    final FXFutureSecurity future = new FXFutureSecurity(EXPIRY, TRADING_EXCHANGE, SETTLEMENT_EXCHANGE, CCY, UNIT_AMOUNT, NUMERATOR, DENOMINATOR,
        CONTRACT_CATEGORY);
    ...
    assertEquals(future.getNumerator(), NUMERATOR);
    assertEquals(future.getDenominator(), DENOMINATOR);
