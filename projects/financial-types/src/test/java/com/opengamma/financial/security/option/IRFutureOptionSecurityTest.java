...
/**
 * Tests for {@link IRFutureOptionSecurity}.
 */
@Test(groups = TestGroup.UNIT)
public class IRFutureOptionSecurityTest extends AbstractBeanTestCase {
  ...
  public JodaBeanProperties<? extends Bean> getJodaBeanProperties() {
    return new JodaBeanProperties<>(IRFutureOptionSecurity.class,
  ...
  public void testConstructor() {
    IRFutureOptionSecurity option = new IRFutureOptionSecurity();
    ...
    assertEquals(option.getSecurityType(), IRFutureOptionSecurity.SECURITY_TYPE);
    assertEquals(option.getStrike(), 0.);
    assertNull(option.getUnderlyingId());
    assertFalse(option.isMargined());
    option = new IRFutureOptionSecurity(EXCHANGE, EXPIRY, EXERCISE_TYPE, UNDERLYING, POINT_VALUE, MARGINED, CCY, STRIKE, OPTION_TYPE);
