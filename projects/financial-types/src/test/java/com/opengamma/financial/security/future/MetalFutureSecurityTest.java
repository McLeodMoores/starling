...
/**
 * Tests for {@link MetalFutureSecurity}.
 */
@Test(groups = TestGroup.UNIT)
public class MetalFutureSecurityTest extends AbstractBeanTestCase {
  ...
  public JodaBeanProperties<? extends Bean> getJodaBeanProperties() {
    return new JodaBeanProperties<>(MetalFutureSecurity.class,
        ...
        Arrays.asList(new Expiry(DateUtils.getUTCDate(2020, 6, 15)), SETTLEMENT_EXCHANGE, TRADING_EXCHANGE, Currency.BRL, UNIT_AMOUNT * 2, "con",
            UNIT_NUMBER * 2, "100oz silver", ExternalId.of("sec", "PQR")));
  }
  ...
  public void testConstructor() {
    MetalFutureSecurity future = new MetalFutureSecurity(EXPIRY, TRADING_EXCHANGE, SETTLEMENT_EXCHANGE, CCY, UNIT_AMOUNT, CONTRACT_CATEGORY);
    ...
    future = new MetalFutureSecurity(EXPIRY, TRADING_EXCHANGE, SETTLEMENT_EXCHANGE, CCY, UNIT_AMOUNT, CONTRACT_CATEGORY, UNDERLYING_ID);
    assertEquals(future.getExpiry(), EXPIRY);
    assertEquals(future.getTradingExchange(), TRADING_EXCHANGE);
    assertEquals(future.getSettlementExchange(), SETTLEMENT_EXCHANGE);
    assertEquals(future.getCurrency(), CCY);
