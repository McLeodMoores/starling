...
/**
 * Tests for {@link EquityIndexOptionSecurity}.
 */
@Test(groups = TestGroup.UNIT)
public class EquityIndexOptionSecurityTest extends AbstractBeanTestCase {
  ...
  public JodaBeanProperties<? extends Bean> getJodaBeanProperties() {
    return new JodaBeanProperties<>(EquityIndexOptionSecurity.class,
        Arrays.asList("optionType", "strike", "currency", "underlyingId", "exerciseType", "expiry", "pointValue", "exchange"),
        Arrays.asList(OPTION_TYPE, STRIKE, CCY, UNDERLYING, EXERCISE_TYPE, EXPIRY, POINT_VALUE, EXCHANGE), Arrays.asList(OptionType.PUT, STRIKE + 2,
            Currency.BRL, ExternalId.of("eid", "2"), ExerciseType.of("American"), new Expiry(DateUtils.getUTCDate(2020, 3, 21)), POINT_VALUE + 1, "DEF"));
  }
  ...
  public void testConstructor() {
    EquityIndexOptionSecurity option = new EquityIndexOptionSecurity();
    ...
    assertEquals(option.getSecurityType(), EquityIndexOptionSecurity.SECURITY_TYPE);
    assertEquals(option.getStrike(), 0.);
    assertNull(option.getUnderlyingId());
    option = new EquityIndexOptionSecurity(OPTION_TYPE, STRIKE, CCY, UNDERLYING, EXERCISE_TYPE, EXPIRY, POINT_VALUE, EXCHANGE);
  ...
  public void testAccept() {
    final EquityIndexOptionSecurity option = new EquityIndexOptionSecurity(OPTION_TYPE, STRIKE, CCY, UNDERLYING, EXERCISE_TYPE, EXPIRY, POINT_VALUE, EXCHANGE);
