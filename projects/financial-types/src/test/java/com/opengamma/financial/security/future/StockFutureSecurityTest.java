...
/**
 * Tests for {@link StockFutureSecurity}.
 */
@Test(groups = TestGroup.UNIT)
public class StockFutureSecurityTest extends AbstractBeanTestCase {
  ...
  private static final ExternalId UNDERLYING_ID = ExternalId.of("sec", "LMN");
  private static final StockFutureSecurity FUTURE = new StockFutureSecurity(EXPIRY, TRADING_EXCHANGE, SETTLEMENT_EXCHANGE, CCY, UNIT_AMOUNT, CONTRACT_CATEGORY,
      UNDERLYING_ID);

  @Override
  public JodaBeanProperties<? extends Bean> getJodaBeanProperties() {
    return new JodaBeanProperties<>(StockFutureSecurity.class,
  ...
  public void testConstructor() {
    ...
    StockFutureSecurity future = new StockFutureSecurity(EXPIRY, TRADING_EXCHANGE, SETTLEMENT_EXCHANGE, CCY, UNIT_AMOUNT, CONTRACT_CATEGORY);
    ...
    future = new StockFutureSecurity(EXPIRY, TRADING_EXCHANGE, SETTLEMENT_EXCHANGE, CCY, UNIT_AMOUNT, CONTRACT_CATEGORY, UNDERLYING_ID);
    assertEquals(future.getExpiry(), EXPIRY);
    assertEquals(future.getTradingExchange(), TRADING_EXCHANGE);
    assertEquals(future.getSettlementExchange(), SETTLEMENT_EXCHANGE);
    assertEquals(future.getCurrency(), CCY);
