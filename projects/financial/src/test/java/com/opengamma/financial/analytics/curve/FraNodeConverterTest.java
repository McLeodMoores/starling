...
public class FraNodeConverterTest {
  ...
  private static final IborIndexConvention USD_LIBOR_INDEX_CONVENTION = new IborIndexConvention("USD LIBOR Index Convention",
      USD_LIBOR_INDEX_CONVENTION_ID.toBundle(),
  ...
  static {
    CONVENTION_SOURCE.addConvention(new SwapFixedLegConvention("USD Fixed Swap Leg", ExternalIdBundle.of(SCHEME, "USD Swap Fixed Leg"), Tenor.THREE_MONTHS,
        DayCounts.ACT_360, BusinessDayConventions.FOLLOWING,
  ...
  public void testWrongConventionTypeForIborIndex() {
    ...
    final OvernightIndexConvention indexConvention = new OvernightIndexConvention("USD OVERNIGHT Index", conventionId.toBundle(), DayCounts.ACT_360, 0,
        Currency.USD, US_CALENDAR_ID);
  ...
  public void testNoFixingCalendar() {
    ...
    final CurveNodeVisitor<InstrumentDefinition<?>> converter = new FRANodeConverter(SECURITY_SOURCE, CONVENTION_SOURCE, new InMemoryHolidaySource(),
        REGION_SOURCE,
        marketValues, marketDataId, NOW);
    fraNode.accept(converter);
  }
  ...
  /**
   * Tests that the dates are calculated correctly. The valuation date is 2015-01-05, so the calculations should be: spot date = 2015-01-07, fixing start =
   * 2015-07-07, fixing end = 2015-10-07.
   */
  @Test
  public void testNoDateAdjustment() {
    ...
    final com.opengamma.analytics.financial.instrument.index.IborIndex expectedIndex = new com.opengamma.analytics.financial.instrument.index.IborIndex(
        Currency.USD, Period.ofMonths(3), 2, DayCounts.ACT_360, BusinessDayConventions.FOLLOWING, false, USD_LIBOR_INDEX_CONVENTION.getName());
    final ForwardRateAgreementDefinition expectedDefinition = ForwardRateAgreementDefinition.from(expectedFixingStart, expectedFixingEnd, 1, expectedIndex,
        rate, fixingCalendar);
    final FRANode fraNode = new FRANode(Tenor.SIX_MONTHS, Tenor.NINE_MONTHS, USD_3M_LIBOR_INDEX_ID, "Mapper");
    final CurveNodeVisitor<InstrumentDefinition<?>> converter = new FRANodeConverter(SECURITY_SOURCE, CONVENTION_SOURCE, HOLIDAY_SOURCE, REGION_SOURCE,
        marketValues, marketDataId, NOW);
  ...
  /**
   * Tests that the dates are calculated correctly. The valuation date is 2015-01-01, so the calculations should be: spot date = 2015-01-05, fixing start =
   * 2015-07-07, fixing end = 2015-10-07.
   */
  @Test
  ...
  /**
   * Tests that the tenor from the index security is used preferentially when constructing the underlying index if is available from the source. The difference
   * between the tenors in the FRA node should only be used if the convention is referenced in the node.
   */
  @Test
  public void testTenorCalculation() {
    ...
    com.opengamma.analytics.financial.instrument.index.IborIndex expectedIndex = new com.opengamma.analytics.financial.instrument.index.IborIndex(Currency.USD,
        Period.ofMonths(3), 2, DayCounts.ACT_360, BusinessDayConventions.FOLLOWING, false, USD_LIBOR_INDEX_CONVENTION.getName());
    FRANode fraNode = new FRANode(Tenor.SIX_MONTHS, Tenor.ONE_YEAR, USD_3M_LIBOR_INDEX_ID, "Mapper");
    ForwardRateAgreementDefinition expectedDefinition = ForwardRateAgreementDefinition.from(expectedFixingStart, expectedFixingEnd, 1, expectedIndex, rate,
        fixingCalendar);
    assertEquals(fraNode.accept(converter), expectedDefinition);
    // convention is used directly, so the FRA tenors are used for the underlying index tenor
    expectedIndex = new com.opengamma.analytics.financial.instrument.index.IborIndex(Currency.USD, Period.ofMonths(6), 2, DayCounts.ACT_360,
        BusinessDayConventions.FOLLOWING, false, USD_LIBOR_INDEX_CONVENTION.getName());
