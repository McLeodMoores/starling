...
public class CorrelationMatrixFunction extends AbstractFunction.NonCompiledInvoker {
  ...
  public Set<ValueRequirement> getRequirements(final FunctionCompilationContext context, final ComputationTarget target, final ValueRequirement desiredValue) {
    return Collections.singleton(new ValueRequirement(ValueRequirementNames.COVARIANCE_MATRIX, target.toSpecification(),
  ...
  public Set<ValueSpecification> getResults(final FunctionCompilationContext context, final ComputationTarget target,
    ...
    return Collections.singleton(new ValueSpecification(ValueRequirementNames.CORRELATION_MATRIX, target.toSpecification(),
        input.getProperties().copy().withoutAny(ValuePropertyNames.FUNCTION)
  ...
  public Set<ComputedValue> execute(final FunctionExecutionContext context, final FunctionInputs inputs, final ComputationTarget target,
    ...
    final DoubleLabelledMatrix2D output = new DoubleLabelledMatrix2D(input.getXKeys(), input.getXLabels(), input.getYKeys(), input.getYLabels(),
        input.getValues());
    ...
    return Collections.singleton(
        new ComputedValue(new ValueSpecification(ValueRequirementNames.CORRELATION_MATRIX, target.toSpecification(), desiredValue.getConstraints()), output));
  }
