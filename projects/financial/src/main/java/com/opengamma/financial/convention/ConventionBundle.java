...
public interface ConventionBundle extends UniqueIdentifiable {
  ...
  /**
   * The time from now to when the bond coupon is paid, in days. If the number of settlement days depends on the length of the bond, this is taken into account
   * 
   * @param bondSettlementDate
  ...
  /**
   * Gets the PublicationLag for an OvernightIndexSwap Rate. 0 if rate is published on the day corresponding to that day's interest accrual 1 if rate is
   * published on the day *following* its corresponding interest accrual period
   * 
  ...
  /**
   * Whether or not the convention for payments is end-of-month or not. In other words, if the maturity is on the last day of a month, are all other payments.
   * 
  ...
  /**
   * Field for bonds that determines whether the payment dates should be rolled to settlement. In general, this will be true for bonds but is hard-coded to
   * false for deliverables in a bond future basket
   * 
