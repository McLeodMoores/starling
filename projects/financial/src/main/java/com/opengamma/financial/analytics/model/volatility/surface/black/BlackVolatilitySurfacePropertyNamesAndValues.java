...
public class BlackVolatilitySurfacePropertyNamesAndValues {
  ...
  /**
   * Property name for {@link ValuePropertyNames#SURFACE_CALCULATION_METHOD} indicating that an interpolated Black lognormal volatility surface was used.
   */
  ...
  /**
   * Property name for the choice of whether to (a) throw an exception if the extrapolator fails. to fit a ShiftedLognormal model to the vol and gradient of the
   * last strike, (b) to reduce the slope until a fit is found or (c) to set slope to zero, such that exterior volatilities equal the last marked value
   */
