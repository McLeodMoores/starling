...
/**
 * Wrapper for querying a {@link ConfigSource} from a function. This uses a locked version/correction (not LATEST), and triggers function re-initialization when
 * the relevant configuration item(s) change.
 *
 * @param <T>
 *          the type of item returned
 */
public class ConfigSourceQuery<T> {
   ...
   *
   * @param configSource
   *          the configuration source to query, not null
   * @param clazz
   *          the object type, not null
   * @param versionCorrection
   *          the locked version/correction timestamp, not null
   */
   ...
   * @param function
   *          the function definition making the call, not null
   * @param clazz
   *          the object type, not null
   * @param <T>
   *          the object type
  ...
  public static <T> ConfigSourceQuery<T> init(final FunctionCompilationContext context, final FunctionDefinition function, final Class<T> clazz) {
    ...
    return new ConfigSourceQuery<>(OpenGammaCompilationContext.getConfigSource(context), clazz, context.getFunctionInitializationVersionCorrection());
  }
  ...
  /**
   * Fetches the "latest" config item. This is the value at the locked version/correction which is typically the time when the system was last in a consistent
   * state (eg at the start of a graph compilation/execution sequence).
   *
   * @param name
   ...
   *          the name of the item to query
   * @param versionCorrection
   *          the version/correction timestamp to query at, not null
   * @return the config item, null if not found
   */
  ...
  /**
   * Fetches the config item that was valid as of the locked version/correction (typically the time when the system was last in a consistent state).
   *
   * @param objectId
   *          the identifier if the item, not null
   * @return the config item, null if not found
   */
  public T get(final ObjectId objectId) {
    return get(objectId, getVersionCorrection());
  }
   ...
   * 
   * @param objectId
   *          the identifier of the item, not null
   * @param versionCorrection
   *          the version/correction timestamp at which to query, not null
   * @return the config item, null if not found
   */
  public T get(final ObjectId objectId, final VersionCorrection versionCorrection) {
    return (T) getConfigSource().get(objectId, versionCorrection).getValue();
  }
}
