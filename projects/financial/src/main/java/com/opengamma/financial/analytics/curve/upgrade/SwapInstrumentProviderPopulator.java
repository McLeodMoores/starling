...
/**
 * Class that populates a curve node id mapper with the curve instrument providers for swap
 * {@link com.opengamma.financial.analytics.ircurve.StripInstrumentType}s. If a map for {@link com.opengamma.financial.analytics.ircurve.strips.SwapNode} is
 * already present, this class will overwrite that entry.
 * <p>
 * The instrument provider name must be supplied, as there is a many-to-one mapping from swap strip instrument types to swap nodes. The getter method of the
...
public class SwapInstrumentProviderPopulator extends InstrumentProviderPopulator {
  ...
  /**
   * Sets the renaming function to {@link DefaultCsbcRenamingFunction}. The strip instrument type must be one of {@link StripInstrumentType#SWAP_3M},
   * {@link StripInstrumentType#SWAP_6M}, {@link StripInstrumentType#SWAP_12M} {@link StripInstrumentType#SWAP_28D} or {@link StripInstrumentType#OIS_SWAP}.
   * 
   * @param type
  ...
  /**
   * The strip instrument type must be one of {@link StripInstrumentType#SWAP_3M}, {@link StripInstrumentType#SWAP_6M}, {@link StripInstrumentType#SWAP_12M}
