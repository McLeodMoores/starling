 ...
 */
public final class MultiCurveUtils {
   ...
   * this value is null.
   * @param sensitivities The matrix of sensitivities, not null
   * @param specification The curve definition, not null
   * @return A labelled matrix
   */
  public static DoubleLabelledMatrix1D getLabelledMatrix(final DoubleMatrix1D sensitivities, final CurveSpecification specification) {
    ArgumentChecker.notNull(sensitivities, "sensitivities");
    ArgumentChecker.notNull(specification, "specification");
    final int n = sensitivities.getNumberOfElements();
    if (n != specification.getNodes().size()) {
      throw new OpenGammaRuntimeException("Did not have a sensitivity for each curve node");
    }
    final Double[] keys = new Double[n];
    final double[] values = new double[n];
    final Object[] labels = new Object[n];
    final Iterator<CurveNodeWithIdentifier> iter = specification.getNodes().iterator();
    for (int i = 0; i < n; i++) {
      keys[i] = Double.valueOf(i);
      values[i] = sensitivities.getEntry(i);
      final CurveNodeWithIdentifier node = iter.next();
      if (node instanceof PointsCurveNodeWithIdentifier) {
        ...
        labels[i] = pointsNode.getIdentifier().getValue() + " + " + pointsNode.getUnderlyingIdentifier().getValue();
      } else {
