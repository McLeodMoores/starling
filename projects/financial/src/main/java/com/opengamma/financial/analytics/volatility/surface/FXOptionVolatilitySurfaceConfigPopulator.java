...
/**
 * Populates the {@link ConfigMaster} with FX option volatility surface definitions and specifications. The surfaces are quoted in ATM / risk-reversal /
 * butterfly form and uses Bloomberg tickers.
 */
public class FXOptionVolatilitySurfaceConfigPopulator {
   ...
   *          The configuration master, not null
   * @param pairsAndNames
   *          A map of currency pairs to surface names, not null
   * @return The populated configuration master
   */
  ...
  private static void populateVolatilitySurfaceDefinitions(final ConfigMaster configMaster, final UnorderedCurrencyPair target, final String name) {
    ...
    final VolatilitySurfaceDefinition<Tenor, Pair<Number, FXVolQuoteType>> volSurfaceDefinition = new VolatilitySurfaceDefinition<>(fullName, target, EXPIRIES,
        YS);
  ...
  private static void populateVolatilitySurfaceSpecifications(final ConfigMaster configMaster, final UnorderedCurrencyPair target, final String name) {
    final SurfaceInstrumentProvider<Tenor, Pair<Number, FXVolQuoteType>> surfaceInstrumentProvider = new BloombergFXOptionVolatilitySurfaceInstrumentProvider(
        target.toString(), "Curncy",
  ...
  private static ConfigItem<VolatilitySurfaceDefinition<Tenor, Pair<Number, FXVolQuoteType>>> makeConfig(
