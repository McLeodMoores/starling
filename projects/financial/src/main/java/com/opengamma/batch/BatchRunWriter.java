...
public interface BatchRunWriter {
   ...
   * <p>
   * This creates all static data structures. This method must be called before any risk can be written for the batch in question.
   *
   * @param cycleMetadata
   *          cycle metadata of the batch, not null
   * @param batchParameters
   *          batch parameters
   * @param runCreationMode
   *          the mode of risk run cration
   * @param snapshotMode
   *          the mode defining if missing market data should be written or expected to exist upfront
   ...
   * 
   * @param riskRunId
   *          the uid of the running batch, not null
   * @param result
   *          the result of computation of the batch
   */
  ...
  /**
   * Creates a market data in the database. If the market data already exists, does nothing.
   *
   * @param marketDataUid
   *          The base unique id of market data, not null
   * @return the id of creted market data.
   */
  MarketData createMarketData(UniqueId marketDataUid);

  /**
   * Adds market data fixings to an existing market data. The market data must already exist.
   *
   * @param marketDataId
   *          Id of the market data, not null
   * @param values
   *          The fixings, not null
   */
   ...
   *
   * @param marketDataId
   *          the id of the market data to delete
