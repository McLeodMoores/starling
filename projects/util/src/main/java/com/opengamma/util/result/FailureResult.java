 ...
 *
 * @param <T>
 *          the type of the underlying result which is only required for allowing method signatures to match
 */
@BeanDefinition
public final class FailureResult<T>
   ...
   *
   * @param failures
   *          the failures, not empty, not null
   * @return the failure result, not null
   * @param <U>
   *          the type of the result
   */
  static <U> Result<U> of(final List<Failure> failures) {
    ...
    final ImmutableSet<Failure> fails = ImmutableSet.copyOf(failures);
    FailureStatus status = fails.iterator().next().getStatus();
    final StrBuilder buf = new StrBuilder();
    for (final Failure failure : fails) {
      buf.appendSeparator(", ");
    ...
    final Result<?> result = new FailureResult<>(fails, status, buf.toString());
    return Result.failure(result);
  }

  // -------------------------------------------------------------------------
  @ImmutableConstructor
  private FailureResult(final Set<Failure> failures, final FailureStatus status, final String message) {
  ...
  static {
    MetaBean.register(FailureResult.Meta.INSTANCE);
  }
  ...
  public static <T> FailureResult.Builder<T> builder() {
    return new FailureResult.Builder<>();
  }
