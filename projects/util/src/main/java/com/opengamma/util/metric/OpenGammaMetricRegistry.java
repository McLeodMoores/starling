...
public final class OpenGammaMetricRegistry {
   ...
   */
  public static synchronized void setSummaryRegistry(final MetricRegistry metricRegistry) {
   ...
   * @param metricRegistry  the registry, not null
   */
  public static synchronized void setDetailedRegistry(final MetricRegistry metricRegistry) {
    ArgumentChecker.notNull(metricRegistry, "metricRegistry");
  ...
  public static synchronized void createBasicDebuggingRegistry() {
    ...
    final Slf4jReporter logReporter = Slf4jReporter.forRegistry(s_summaryInstance)
                                          .outputTo(LoggerFactory.getLogger(OpenGammaMetricRegistry.class))
                                          .convertRatesTo(TimeUnit.SECONDS)
                                          .convertDurationsTo(TimeUnit.MILLISECONDS)
                                          .build();
    logReporter.start(1, TimeUnit.MINUTES);
    final JmxReporter jmxReporter = JmxReporter.forRegistry(s_summaryInstance).build();
    jmxReporter.start();
  }
