 ...
 * logging (MDC). MDC allows per-thread, contextual information to be output to
 * logging files. This is a map that could hold a session id for a web
 ...
 *
 * This executor ensures that MDC information is copied from the calling thread
 * onto the worker thread and removed once the worker has completed and is
 * returned to the pool.
 *
 * Because access to the MDC is via static methods, it is only possible to
 * access the context applicable to the current thread i.e. when submitting a
 * job to the executor we have access to the calling thread's MDC, but no way of
 * accessing the worker's. Similarly, when the worker executes the job, we have
 * access to the worker's MDC but not the original caller's. To handle this, the
 * class records the MDC information from a thread when it submits a job. When
 * the job is executed, the MDC information is looked up and copied to the
 * worker.
 *
 * Note that this class overrides {@link #execute(Runnable)} which means there
 * is no need to override the rest of the methods defined in
