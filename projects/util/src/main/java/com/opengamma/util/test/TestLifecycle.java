...
/**
 * Helper class for managing {@link Lifecycle} implementing objects during a test session. Objects may be created by test infrastructure
 * that must be shutdown but are embedded or wrapped in other object instances which don't expose life cycle methods so may not be
 * directly available for the caller to explicitly shut them down.
 * <p>
 ...
 * </pre>
 *
 * Any test infrastructure that creates things that must be shutdown at the end of the test can register them, after they've been started,
 * by a call to {@link #register}.
 */
public final class TestLifecycle {

  private static final ThreadLocal<TestLifecycle> INSTANCES = new ThreadLocal<>();

  private final List<Lifecycle> _toStop = new ArrayList<>();
  ...
  public static void end() {
    ...
    INSTANCES.set(null);
    for (final Lifecycle instance : toStop) {
