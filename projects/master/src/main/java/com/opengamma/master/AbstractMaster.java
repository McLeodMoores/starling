...
public interface AbstractMaster<D extends AbstractDocument> {
   ...
   * <p>
   * The identifier version string will be used to return the correct historic
   * version providing that the master supports history.
   * <p>
   * Access to a document may be controlled by permissions. If the user does not
   * have permission to view the document then an exception is thrown.
   *
   * @param uniqueId
   ...
   *           if the document requires permissions that the user does not have
   */
   ...
   * <p>
   * The version-correction will be used to return the correct historic version
   * providing that the master supports history.
   * <p>
   * Access to a document may be controlled by permissions. If the user does not
   * have permission to view the document then an exception is thrown.
   *
   ...
   * <p>
   * The specified document must contain the unique identifier. If the
   * identifier has a version it must be the latest version. The version
   * instant, correction instant and identifier will be set in the response.
   * <p>
   ...
   * @return the current state of the document, may be an update of the input
   *         document, not null
   * @throws IllegalArgumentException
   *           if the request is invalid
   * @throws com.opengamma.DataNotFoundException
   ...
   * <p>
   * A full master will store detailed historic information on documents. Thus,
   * a removal does not prevent retrieval or correction of an earlier version.
   * <p>
   ...
   * <p>
   * The versioning will be taken from the "version from" instant in each
   * specified document. The "version to" instant and "correction" instants will
   * be ignored on input. If the "version from" instant is null, version from of
   * the replaced document is used No two versioned documents may have the same
   * "version from" instant. The versions must all be within the version range
   * of the original version being replaced. The list can not be empty.
   * <p>
   * The unique identifier must specify a version of the document that is active
   * when queried with the {@link VersionCorrection latest correction instant}.
   * The unique identifier in each specified document will be ignored on input.
   *
   * @param uniqueId
   *          the unique identifier to replace, not null
   * @param replacementDocuments
   *          the replacement documents, not null
   * @return the list of versioned documents matching the input list, not null
   * @throws IllegalArgumentException
   *           if the request is invalid
   * @throws com.opengamma.DataNotFoundException
   *           if there is no document with the object identifier
   */
   ...
   * The versioning will be taken from the "version from" instant in each
   * specified document. The "version to" instant and "correction" instants will
   * be ignored on input. If the "version from" instant is null, the latest
   * instant is used. No two versioned documents may have the same "version
   * from" instant.
   * <p>
   * If the list is empty, this is equivalent to {@link #remove}. If the object
   * identifier is a {@link UniqueId}, then it must specify a version of the
   * document that is active when queried with the {@link VersionCorrection
   * latest correction instant}. The unique identifier in each specified
   * document will be ignored on input.
   *
   * @param objectId
   *          the object identifier of the document to replace, not null
   * @param replacementDocuments
   *          the replacement documents, not null
   ...
   * <p>
   * This applies a correction that replaces a single version in the data store
   * with the specified document. This is equivalent to calling
   * {@link #replaceVersion(UniqueId, List)} with a single element list of the
   * specified document.
   * <p>
   * The document must contain the unique identifier to be replaced. The
   * document "version" and "correction" instants will be ignored on input. The
   * unique identifier must specify a version of the document that is active
   * when queried with the {@link VersionCorrection latest correction instant}.
   *
   * @param replacementDocument
   *          the replacement document, not null
   ...
   * <p>
   * This applies a correction that replaces a single version in the data store
   * with the specified list. This is equivalent to calling
   * {@link #replaceVersion(UniqueId, List)} with a single element list of the
   * specified document.
   * <p>
   * The unique identifier must specify a version of the document that is active
   ...
   *
   * @param uniqueId
   *          the unique identifier to remove, not null
   * @throws IllegalArgumentException
   *           if the request is invalid
   * @throws com.opengamma.DataNotFoundException
   *           if there is no document with that unique identifier
   ...
   * <p>
   * This applies a correction that adds the specified document to the data
   * store.
   * <p>
   * The versioning will be taken from the "version from" instant in the
   * specified document. The "version to" instant and "correction" instants will
   * be ignored on input. No two active versioned documents in the data store
   * may have the same "version from" instant. If the "version from" instant is
   * null, the latest instant is used which would be equivalent to calling
   * {@link #update(AbstractDocument)}.
   * <p>
   * If the object identifier is a {@link UniqueId}, then it must specify a
   * version of the document that is active when queried with the
   * {@link VersionCorrection latest correction instant}.
   *
   * @param objectId
   *          the object identifier of the document to add a version to, not
   *          null
   * @param documentToAdd
   *          the document, not null
   * @return the current state of the document, may be an update of the input
