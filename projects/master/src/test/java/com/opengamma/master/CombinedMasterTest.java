...
public class CombinedMasterTest {
  ...
  private HolidayDocument holidayDocWithId(final String scheme, final String id) {
  ...
  @Test
  public void get() {
    final ArrayList<UniqueId> getList = Lists.newArrayList(u1, u2);
  ...
  public void pagedSearchOneElement() {
    final PagingRequest pr = PagingRequest.ALL;
    final List<HolidayDocument> singleton = Lists.newArrayList(d1);
  ...
  public void pagedSearchMultiple() {
    ...
    final List<HolidayDocument> m1Result = Lists.newArrayList(d1);
    final List<HolidayDocument> m2Result = Lists.newArrayList(d2);
    final List<HolidayDocument> expected = Lists.newArrayList(d1, d2);
  ...
  private List<HolidayDocument> runPagedSearch(final PagingRequest pr, final List<HolidayDocument> m1Result, final int m1Total, final List<HolidayDocument> m2Result, final int m2Total) {
    ...
    cMaster.pagedSearch(new SearchStrategy<HolidayDocument, HolidayMaster, HolidaySearchRequest>() {
      ...
      @Override
      public HolidaySearchResult search(final HolidayMaster master, final HolidaySearchRequest searchRequest) {
