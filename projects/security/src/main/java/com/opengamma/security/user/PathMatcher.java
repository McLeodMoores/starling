...
public class PathMatcher {
   ...
   */
  public static boolean matches(final String input, final String regex) {
    ...
    final StringBuffer quotedRegexBuffer = new StringBuffer();
    final String[] parts = regex.split("\\*", -1);
    for (int i = 0; i < parts.length; i++) {
      final String part = parts[i];
      if (part.length() > 0) {
        final String escapedPart = Pattern.quote(part);
    ...
    final String quotedRegex = quotedRegexBuffer.toString();
