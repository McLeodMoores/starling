...
/**
 * A snapshot of market data taken at a particular instant, potentially altered by hand, that should be applied to
 * computations in some scope, such as a yield curve.
 */
public interface UnstructuredMarketDataSnapshot {
  ...
  /**
   * Returns all of the values from the snapshot for a target. Only the values exactly matching the target bundle will be
   * returned (that is, it is one of the target bundles returned by {@link #getTargets}).
   *
   * @param identifiers the target identifiers, not null
