...
/**
 * Tests nodes in one portfolio for equivalence with nodes from another. All portfolio nodes and positions within the
 * structure must have valid unique identifiers.
 * <p>
 * Portfolio nodes currently contain a reference to their parent nodes, so changing one within a portfolio is not
 * possible without changing the entire portfolio node graph. This may be used to map the logically unchanged nodes
 * from one portfolio structure to nodes in another so that incremental operations may be performed.
 * <p>
 ...
 * <p>
 * Two nodes A and B are normally considered equivalent if their names match, their immediate child positions match, and
 * there is an equivalence mapping of all of the child nodes of A to and from all
 * of the child nodes of B.
 */
public class PortfolioNodeEquivalenceMapper {

  /**
   * Tests if the labels of two nodes are equal. Subclasses might relax this test to allow cheap name adjustments on a
   * portfolio if the function repository is known not to use the name of the node
  ...
  /**
   * Tests if the positions immediately under two nodes are equal. Subclasses might relax this test to allow
   * reordering of positions within a node if the function repository is known not to use the
  ...
  protected boolean getEquivalentNodes(final Collection<PortfolioNode> as, final Collection<PortfolioNode> bs, final Map<UniqueId, UniqueId> result) {
    ...
    nextA: for (final PortfolioNode a : as) { //CSIGNORE
  ...
  /**
   * Maps the node {@code a} and any child nodes to the equivalent node {@code b} and any child nodes if possible.
   * The keys of the result are from {@code a}, the values are from {@code b}. Any nodes that could not be mapped
   * are absent from the collection. If {@code a} and {@code b} are equivalent then the map will contain an entry
   * for {@code (a, b)} plus all child nodes. If {@code a} and {@code b} are not equivalent but contain child nodes
   * that are then the map will contain those child nodes.
   * <p>
  ...
  public Map<UniqueId, UniqueId> getEquivalentNodes(final PortfolioNode a, final PortfolioNode b) {
    final Map<UniqueId, UniqueId> result = new HashMap<>();
