...
public class PortfolioFudgeBuilder implements FudgeBuilder<Portfolio> {
  ...
  @Override
  public MutableFudgeMsg buildMessage(final FudgeSerializer serializer, final Portfolio portfolio) {
    ...
    if (haveAttributes(portfolio)) {
      ...
      for (final Entry<String, String> entry : portfolio.getAttributes().entrySet()) {
  ...
  @Override
  public Portfolio buildObject(final FudgeDeserializer deserializer, final FudgeMsg message) {
    ...
    if (message.hasField(ATTRIBUTES_FIELD_NAME)) {
      final FudgeMsg attributesMsg = message.getMessage(ATTRIBUTES_FIELD_NAME);
      for (final FudgeField fudgeField : attributesMsg) {
        final String key = fudgeField.getName();
        final Object value = fudgeField.getValue();
