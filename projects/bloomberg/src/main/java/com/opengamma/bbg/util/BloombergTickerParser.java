...
/**
 * <p>
 * Base class for Bloomberg ticker parsing. The rationale for having this parsing is to be able to extract instrument indicatives from a Bloomberg ticker string
 ...
 * </p>
 * <p>
 * This class is abstract, with subclasses representing different instrument types. The idiom for using the classes is to create a parser instance around the
 * Bloomberg ticker, and then call various getters on the instance to read the indicatives.
 * </p>
 *
...
public abstract class BloombergTickerParser {
  ...
  // -------- ABSTRACT METHODS --------
  /**
   * Abstract subclass method that should provide a regex for parsing the Bloomberg ticker for the given instrument type. This regex should use groups, so that
   * the created {@link java.util.regex.Matcher} can access indicatives easily.
   *
   ...
   *
   * @param matcher
   *          the matcher used for parsing, setup with the regex from {@link #getPatternString}
   */
