...
public class MongoDBPersistentSubscriptionManagerTest {
  ...
  public void persistentSubscriptionManagement() {
    final ExternalScheme idScheme = ExternalScheme.of("TestDomain");
    final MockLiveDataServer server = new MockLiveDataServer(idScheme, _cacheManager);
    server.connect();

    final MongoConnector connector = MongoTestUtils.makeTestConnector(MongoDBPersistentSubscriptionManagerTest.class.getSimpleName(), true);
    final MongoDBPersistentSubscriptionManager manager = new MongoDBPersistentSubscriptionManager(server, connector);
    manager.clean();
    ...
    final boolean removed = manager.removePersistentSubscription("testsub1");
    assertTrue(removed);
