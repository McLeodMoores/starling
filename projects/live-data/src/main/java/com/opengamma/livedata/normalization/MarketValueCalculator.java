...
public class MarketValueCalculator implements NormalizationRule {
  ...
  public MutableFudgeMsg apply(final MutableFudgeMsg msg, final String securityUniqueId, final FieldHistoryStore fieldHistory) {
    ...
    if (bid != null && ask != null) {
      ...
      if (Math.abs(bid) > TOLERANCE && getRelativeSpread(bid, ask) > MAX_ACCEPTABLE_SPREAD_TO_USE_MIDPOINT) {
        ...
        final Double last = msg.getDouble(MarketDataRequirementNames.LAST);
      ...
      final double marketValue = (bid + ask) / 2.0;
    ...
    final Double mid = msg.getDouble(MarketDataRequirementNames.MID);
    if (mid != null) {
 ...
 // Use "CLOSE" if we've been given one
    final Double close = msg.getDouble(MarketDataRequirementNames.CLOSE);
    if (close != null) {
  ...
  private MutableFudgeMsg lastKnownMarketValue(
    ...
    final Double lastKnownMarketValue = lkv.getDouble(MarketDataRequirementNames.MARKET_VALUE);
