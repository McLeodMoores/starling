...
public class RedisLastKnownValueStore implements LastKnownValueStore {
  ...
  public RedisLastKnownValueStore(final JedisPool jedisPool, final String jedisKey, final boolean writeThrough) {
  ...
  @Override
  public synchronized void updateFields(final FudgeMsg fieldValues) {
    ...
    if (isWriteThrough()) {
      ...
      try {
        for (final FudgeField field : fieldValues.getAllFields()) {
          final String redisValue = toRedisTextValue(getJedisKey(), field);
   ...
   */
  protected static String toRedisTextValue(final String jedisKey, final FudgeField field) {
   ...
   */
  public synchronized void updateFromRedis(final boolean failOnError) {
    _inMemoryStore.clear();
    final Jedis jedis = getJedisPool().getResource();
    try {
      // TODO kirk 2012-07-16 -- Give this a FudgeContext.
      final MutableFudgeMsg fudgeMsg = OpenGammaFudgeContext.getInstance().newMessage();
      final Map<String, String> allFields = jedis.hgetAll(getJedisKey());
      LOGGER.debug("Updating {} from Jedis: {}", getJedisKey(), allFields);
      for (final Map.Entry<String, String> fieldEntry : allFields.entrySet()) {
        final Object parsedRedisObject = fromRedisTextValue(fieldEntry.getValue());
   ...
   */
  protected static Object fromRedisTextValue(final String value) {
    ...
    if (value.startsWith("[") && value.endsWith("]")) {
      ...
      try {
        final String stripped = value.substring(1, value.length() - 1);
        final String[] split = stripped.split(Pattern.quote(","));
        final double[] doubleValues = new double[split.length];
