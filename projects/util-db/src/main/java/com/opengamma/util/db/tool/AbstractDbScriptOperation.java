...
public abstract class AbstractDbScriptOperation<T extends DbToolContext> extends AbstractDbOperation<T> {
  ...
  protected AbstractDbScriptOperation(final T dbToolContext, final boolean write, final File outputFile) {
  ...
  protected DbScript getCreationScript(final String groupName) {
    final DbSchemaGroupMetadata dbSchemaGroupMetadata = DbScriptUtils.getDbSchemaGroupMetadata(groupName);
  ...
  protected List<DbScript> getMigrationScripts(final String groupName) {
    final Integer currentGroupVersion = getCurrentGroupVersion(groupName);
    if (currentGroupVersion == null) {
      return null;
    }
    return DbScriptUtils.getDbSchemaGroupMetadata(groupName).getMigrateScripts(getDatabaseName(), currentGroupVersion);
  }
   ...
   */
  protected Integer getCurrentGroupVersion(final String groupName) {
