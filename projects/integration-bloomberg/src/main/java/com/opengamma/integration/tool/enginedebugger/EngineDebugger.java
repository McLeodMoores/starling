...
public class EngineDebugger extends AbstractTool<IntegrationToolContext> {
  ...
  private void initialize() {
    ...
    final JScrollPane failuresScrollPane = new JScrollPane(_failuresTreeTable);
  ...
  private TreeModel getPortfolioTreeModel(final UniqueId portfolioId, final IntegrationToolContext toolContext) {
  ...
  private ResolutionFailureTreeTableModel createResolutionFailureTreeTableModel(final Object leafNode, final String valueReq) {
    ...
    if (_marketDataSpecifications != null) {
      for (final MarketDataSpecification marketDataSpec : _marketDataSpecifications) {
    ...
    final DependencyGraphBuildTrace trace = getToolContext().getDependencyGraphTraceProvider().getTrace(properties);
    final ResolutionFailureTreeTableModel failuresTreeTableModel = new ResolutionFailureTreeTableModel(trace.getFailures());
    return failuresTreeTableModel;
  }

  private void updateTreeTableModel(final JTree portfolioTree) {
    final TreePath selectionPath = portfolioTree.getSelectionPath();
