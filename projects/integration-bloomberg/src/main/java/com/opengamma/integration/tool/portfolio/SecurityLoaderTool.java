...
public class SecurityLoaderTool extends AbstractTool<IntegrationToolContext> {
  ...
  private Set<ExternalId> filterPresentIds(final Set<ExternalId> externalIds) {
    final Set<ExternalId> filtered = new HashSet<>();
    final SecuritySource securitySource = getToolContext().getSecuritySource();
    for (final ExternalId externalId : externalIds) {
      final Collection<Security> securitySet = securitySource.get(externalId.toBundle());
  ...
  private void loadSecurityData(final boolean write, final Set<ExternalId> externalIds) {
    ...
    final Map<ExternalIdBundle, ManageableSecurity> loadedSecurities = bulkSecurityLoader.loadSecurity(externalIdBundles);
    for (final ManageableSecurity entry : loadedSecurities.values()) {
      if (write) {
        final ManageableSecurity security = SecurityMasterUtils.addOrUpdateSecurity(secMaster, entry);
        if (isVerbose()) {
