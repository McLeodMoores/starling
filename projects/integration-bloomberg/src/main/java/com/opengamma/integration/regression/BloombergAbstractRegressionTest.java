...
public abstract class BloombergAbstractRegressionTest extends AbstractRegressionTest {
  ...
  /**
   * Initializes the test using the default tool context properties file - this cut down context is used to initialize the database. A
   * "regressionPropertiesFile" is also required. This is used to execute the views and must therefore contain a full engine configuration. Typically this can
   * be created by using the fullstack.properties/ini as a starting point, removing the enterprise services such as web and amq exposure.
   *
   * @param regressionRoot
  ...
  protected AbstractRegressionTestToolContextManager createToolContextManager(final File regressionRoot, final String toolContextPropertiesFile,
    ...
    return new BloombergRegressionTestToolContextManager(new File(regressionRoot, AbstractGoldenCopyDumpCreator.DB_DUMP_ZIP), toolContextPropertiesFile,
