...
public class PortfolioLoaderResource {
  ...
  public Response uploadPortfolio(final FormDataMultiPart formData) throws IOException {
    final FormDataBodyPart fileBodyPart = getBodyPart(formData, "file");
    final FormDataBodyPart filexmlBodyPart = getBodyPart(formData, "filexml");

    if (filexmlBodyPart.getFormDataContentDisposition().getFileName().toLowerCase().endsWith("xml")) {
      // xml can contain multiple portfolios
      final Object filexmlEntity = filexmlBodyPart.getEntity();
      final InputStream filexmlStream = new WorkaroundInputStream(((BodyPartEntity) filexmlEntity).getInputStream());
      for (final PositionReader positionReader : returnPorfolioReader(filexmlStream)) {
  ...
  private Iterable<? extends PositionReader> returnPorfolioReader(final InputStream fileStream) {
    return new XmlFileReader(fileStream, new SchemaRegister());
  }

  private static FormDataBodyPart getBodyPart(final FormDataMultiPart formData, final String fieldName) {
  ...
  private static String getString(final FormDataMultiPart formData, final String fieldName) {
    final FormDataBodyPart bodyPart = getBodyPart(formData, fieldName);
  ...
  private static SheetFormat getFormatForFileName(final String fileName) {
