...
public class DepGraphBasicTest extends AbstractDependencyGraphBuilderTest {
  ...
  public void singleOutputSingleFunctionNode() {
    ...
    try {
      ...
      final MockFunction function = helper.addFunctionProducing1and2();
  ...
  public void unsatisfiableDependency() {
    ...
    try {
      ...
      final ValueRequirement anotherReq = new ValueRequirement("Req-3", helper.getTarget().toSpecification());
      final DependencyGraphBuilder builder = helper.createBuilder(null);
  ...
  public void doubleLevelNoLiveData() {
    ...
    try {
      final DepGraphTestHelper helper = helper();
      final MockFunction fn1 = helper.addFunctionRequiring2Producing1();
      final MockFunction fn2 = helper.addFunctionProducing2();
      final DependencyGraphBuilder builder = helper.createBuilder(null);
    ...
    try {
      ...
      for (final DependencyNode node : nodes) {
        if (ObjectUtils.equals(node.getFunction().getFunctionId(), fn1.getUniqueId())) {
          assertTrue(node.hasOutputValue(helper.getSpec1()));
          assertEquals(1, node.getInputCount());
          final ValueSpecification inputSpec = node.getInputValue(0);
          ...
          final ValueSpecification outputSpec = node.getOutputValue(0);
  ...
  public void incrementalBuild() {
    ...
    try {
      ...
      builder.setDependencyGraph(
          new DependencyGraphImpl("DEFAULT", Collections.<DependencyNode>emptySet(), 0, Collections.<ValueSpecification, Set<ValueRequirement>>emptyMap()));
