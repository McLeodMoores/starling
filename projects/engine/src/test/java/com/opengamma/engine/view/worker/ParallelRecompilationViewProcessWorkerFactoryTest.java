...
public class ParallelRecompilationViewProcessWorkerFactoryTest {
  ...
  public void testIgnoreValidity() {
    ...
    final ViewExecutionOptions options =
        new ExecutionOptions(new InfiniteViewCycleExecutionSequence(), ExecutionFlags.triggersEnabled().awaitMarketData().ignoreCompilationValidity().get());
  ...
  public void testPassthrough() {
    ...
    final ParallelRecompilationViewProcessWorkerFactory factory = new ParallelRecompilationViewProcessWorkerFactory(underlying);
    factory.setDelegate(factory);
    final ViewExecutionOptions options = new ExecutionOptions(new InfiniteViewCycleExecutionSequence(),
        ExecutionFlags.triggersEnabled().awaitMarketData().get());
    factory.createWorker(context, options, viewDefinition);
    Mockito.verify(underlying, Mockito.only()).createWorker(context, options, viewDefinition);
  }
  ...
  public void testParallel() {
    ...
    Mockito.when(underlying.createWorker(Matchers.<ViewProcessWorkerContext>anyObject(),
        Matchers.<ViewExecutionOptions>anyObject(), Matchers.<ViewDefinition>anyObject())).then(
  ...
  public void testDeferred() {
    ...
    final ViewDefinition viewDefinition = Mockito.mock(ViewDefinition.class);
    final ViewProcessWorker delegateWorker = Mockito.mock(ViewProcessWorker.class);
    Mockito.when(underlying.createWorker(Matchers.<ViewProcessWorkerContext>anyObject(), Matchers.<ViewExecutionOptions>anyObject(),
        Matchers.<ViewDefinition>anyObject())).then(
        new Answer<ViewProcessWorker>() {
          @Override
          public ViewProcessWorker answer(final InvocationOnMock invocation) throws Throwable {
  ...
  public void testImmediate() {
    ...
    final ViewProcessWorkerContext context = Mockito.mock(ViewProcessWorkerContext.class);
    final ViewExecutionOptions options = new ExecutionOptions(new InfiniteViewCycleExecutionSequence(),
        ExecutionFlags.none().parallelCompilation(ParallelRecompilationMode.IMMEDIATE_EXECUTION).get());
    final ViewDefinition viewDefinition = Mockito.mock(ViewDefinition.class);
    final ViewProcessWorker delegateWorker = Mockito.mock(ViewProcessWorker.class);
    Mockito.when(underlying.createWorker(Matchers.<ViewProcessWorkerContext>anyObject(), Matchers.<ViewExecutionOptions>anyObject(),
        Matchers.<ViewDefinition>anyObject())).then(
