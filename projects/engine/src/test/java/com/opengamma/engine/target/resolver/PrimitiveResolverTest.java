...
public class PrimitiveResolverTest {
  ...
  public void testUnorderedCurrencyPairResolver() {
    ...
    assertEquals(resolver.resolveObject(UnorderedCurrencyPair.of(Currency.GBP, Currency.USD).getUniqueId(), VersionCorrection.LATEST),
        UnorderedCurrencyPair.of(Currency.GBP, Currency.USD));
    assertEquals(resolver.resolveObject(UniqueId.of("Foo", "Bar"), VersionCorrection.LATEST), null);
  }
  ...
  public void testPrimitiveResolver() {
    ...
    final ExternalId[] eids = new ExternalId[] {ExternalId.of("Foo", "A"), ExternalId.of("Foo-", "A"), ExternalId.of("Foo\\", "A"),
        ExternalId.of("Foo", "A-B"), ExternalId.of("Foo-Bar", "A"),
        ExternalId.of("Foo-Bar", "A-B"), ExternalId.of("Foo\\-Bar", "A\\B"), ExternalId.of("Foo\\\\Bar", "A\\\\B") };
    for (final ExternalId eid : eids) {
