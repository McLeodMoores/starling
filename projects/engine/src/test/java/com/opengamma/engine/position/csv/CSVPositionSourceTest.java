...
public class CSVPositionSourceTest {
  ...
  public void parseLineOneIdentifier() {
    final Position position = CSVPositionSource.parseLine(new String[] {"98.4", "KIRK", "MY-ID"}, UID);
    ...
    final ExternalId id = position.getSecurityLink().getExternalId().iterator().next();
    assertNotNull(id);
  ...
  public void parseLineThreeIdentifiers() {
    final Position position = CSVPositionSource.parseLine(new String[] {"98.4", "Domain1", "Value1", "Domain2", "Value2", "Domain3", "Value3"}, UID);
    assertNotNull(position);
    ...
    for (final ExternalId id : position.getSecurityLink().getExternalId()) {
      assertNotNull(id);
      assertNotNull(id.getScheme());
      assertNotNull(id.getValue());
  ...
  private String createTempTestPortfolioDirectory() throws IOException {
    final String tempDirName = getTempPortfolioDirectory();
    ...
    final Collection<String> testPortfolio1 = new ArrayList<>();
    ...
    final Collection<String> testPortfolio2 = new ArrayList<>();
  ...
  private void cleanUpTestPortfolios(final String portfolioDirName) throws IOException {
    FileUtils.deleteDirectory(new File(portfolioDirName));
  }

  public void testLoadPortfolios() throws IOException {
    final String portfolioDirName = createTempTestPortfolioDirectory();
    final CSVPositionSource pm = new CSVPositionSource(portfolioDirName);

    assertEquals(2, pm.getPortfolioIds().size());

    // Loaded correctly
    final ObjectId[] portIds = pm.getPortfolioIds().toArray(new ObjectId[0]);
    final Portfolio port1 = pm.getPortfolio(portIds[0].atVersion("0"), VersionCorrection.LATEST);
    assertEquals(6, port1.getRootNode().getPositions().size());
    final Portfolio port2 = pm.getPortfolio(portIds[1].atVersion("0"), VersionCorrection.LATEST);
    assertEquals(4, port2.getRootNode().getPositions().size());
    final Portfolio port2b = pm.getPortfolio(portIds[1], VersionCorrection.LATEST);
    ...
    final PortfolioNode rootNode1 = pm.getPortfolioNode(port1.getRootNode().getUniqueId(), VersionCorrection.LATEST);
    assertEquals(6, rootNode1.getPositions().size());

    // Retrieval by position
    final Position port1Pos0 = rootNode1.getPositions().get(0);
