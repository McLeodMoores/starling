 ...
 * <p>
 * Capabilities are calculated at an invoker level so an invoker should only dispatch to a homogeneous set of calculation
 * nodes. The calculation nodes a given job invoker dispatches to are assumed to share the same local cache.
 */
public interface JobInvoker {
   ...
   * <p>
   * These will be used to determine which jobs will get farmed to this invoker. It will not be modified by the job dispatcher.
   * It should have a very efficient equals operation that can cheaply identify when the capabilities exported by the invoker
   * have changed.
   *
   ...
   * @param receiver the result receiver; must be signaled with either success or failure unless false is returned to indicate
   * the invoker did not accept the job, not null
   ...
   *
   * @param callback the object the invoker should register itself with when it is ready to receive {@link #invoke} calls
   * again. This must not be called inline from this method, if the invoker is ready it must return true.
   ...
   * and re-dispatched. This method must not block or take long to complete. If determining status is costly, the node
   * should return true and use {@link JobInvocationReceiver#jobFailed} asynchronously if the job will not complete.
   */
