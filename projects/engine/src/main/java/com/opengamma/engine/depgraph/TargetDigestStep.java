...
/* package */final class TargetDigestStep extends FunctionIterationStep.IterationBaseStep {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(TargetDigestStep.class);

  private static final Profiler PROFILER_SUCCESS = Profiler.create(TargetDigestStep.class, "success");
  private static final Profiler PROFILER_WASTE = Profiler.create(TargetDigestStep.class, "waste");
      ...
      try {
        ...
        do {
          ...
          if (constraints.isSatisfiedBy(properties)) {
            LOGGER.info("Trying digest resolution {} for {}", properties, requirement);
            ...
            if (!functionDef.getTargetType().isCompatible(target.getType())) {
              LOGGER.debug("Function {} type is not compatible with {}", functionDef, target);
              continue;
            }
              ...
              if (requirement.getValueName() == result.getValueName()) {
                if (composedConstraints.isSatisfiedBy(result.getProperties())) {
                  ...
                  if (resolvedOutput != matchedResult) {
                    ...
                    LOGGER.debug("Composed original output of {} to {}", matchedResult, resolvedOutput);
                  }
                  break;
                }
              }
            ...
            if (resolvedOutput == null) {
              LOGGER.debug("Requirement {} not satisfied by results {}", requirement, results);
              continue;
            }
            LOGGER.info("Inferred requirement {} for {}", composedRequirement, requirement);
  ...
  protected void reportResult() {
    LOGGER.debug("Successful digest resolution of {} via {}", getValueRequirement(), getDesiredValue());
    PROFILER_SUCCESS.tick(_timeSpent);
    _timeSpent = 0;
  }
