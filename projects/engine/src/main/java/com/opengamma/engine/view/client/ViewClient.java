...
/**
 * Represents a managed client of a specific view process in the context of a particular user. This is the unit of external interaction
 * for accessing computation results.
 * <p>
 * The client begins detached from any particular view process. In order to receive results, it must be attached to a view process.
 * This creates demand for the process to execute, ensuring that it remains executing while the client is attached.
 * <p>
 * If the application requires asynchronous updates, it must set a result listener. This receives notifications of events from the view
 * process. The result mode for this client may be controlled using {@link #setResultMode(ViewResultMode)}; this allows unwanted types of
 * result to be filtered out, potentially avoiding the overhead of unnecessary serialisation.
 * <p>
 * The application may poll for the latest full result by calling {@link #getLatestResult()}. Listeners can be set or changed even when
 * the client is detached from a view process; this allows the necessary listeners to be in place before results commence.
 * <p>
 * The per-client flow of results is controlled through {@link #pause()} and {@link #resume()}. By default, results will flow immediately,
 * but the client may be paused before attaching it to a view process.
 * <p>
 * When results are paused, they are incrementally batched to be delivered as a single, collapsed result when they are resumed. This result
 * is specific to the individual client that has been paused.
 * <p>
 * Use {@link #setUpdatePeriod(long)} to throttle the frequency of updates exposed through this client.
 * <p>
 * Always call {@link #shutdown()} from any state to allow resources associated with the managed view to be released when the client is no
 * longer required. Without this, the view process may continue executing indefinitely.
 */
@PublicAPI
public interface ViewClient extends UniqueIdentifiable {
   ...
   * <p>
   * As part of this call, the client is updated with the latest state of the view process which may result in listeners being
   * invoked, depending on the state of the client.
   *
   ...
   * <p>
   * As part of this call, the client is updated with the latest state of the view process which may result in listeners
   * being invoked, depending on the state of the client.
   *
   * @param definitionId the unique ID of the view definition, not null
   * @param executionOptions the view execution options, not null
  ...
  // each cycle.
  /**
   * Gets the live data override injector for the view process to which the client is attached. This allows arbitrary live data to
   * be overridden; the effects of doing so will be seen in future view cycles.
   *
  ...
  /**
   * Gets the latest view definition referenced by the view process to which the client is attached. This could be a different version
   * from the one used in the computation of the latest result as seen by this client; to access a specific version use the appropriate
   * {@link CompiledViewDefinition} or {@link ViewCycle}.
   *
  ...
  /**
   * Sets the minimum time between successive results delivered to the listener, thus providing the ability to throttle the rate of updates. Even when
   * this is in use, an update may still require multiple calls to the listener, for example the notification of errors and/or compilation results.
   * This is achieved by merging any updates which arrive in between the minimum period, and releasing only a single, merged update at the correct
   * time. Set this to 0 to specify an unrestricted rate of updates.
   *
  ...
  //-------------------------------------------------------------------------
  /**
   * Pauses the flow of results exposed through this client. They continue to be received internally, and these are delivered as a merged
   * result when updates are resumed.
   */
  void pause();
  ...
  /**
   * Gets whether the attached view process has completed from the perspective of the client. This is consistent with any data flow restrictions being applied
   * through this view client, so may occur after the process actually completes. This is intended for batch processing; if the view process is running with
   * an infinite number of evaluation times then this method will block forever.
   *
  ...
  /**
   * Blocks until the view process completes from the perspective of the client. This is consistent with any data flow restrictions being applied through this
   * view client, so may occur after the process actually completes. This is intended for batch processing; if the view process is running with an infinite
   * number of evaluation times then this method will block forever.
   *
   * @throws IllegalStateException if the view client is not attached to a view process
  ...
  /**
   * Indicates whether the result of a completed view cycle is available to this client. This is consistent with any data flow restrictions being applied
   * through this view client, so does not necessarily reflect the most recent state of the view process.
   *
  ...
  /**
   * Gets the latest compiled view definition. This is consistent with any data flow restrictions being applied through this view client, so does not
   * necessarily represent the most recent state of the view process.
   *
  ...
  /**
   * Gets the full result from the latest view cycle. This is consistent with any data flow restrictions being applied through this view client, so does
   * not necessarily represent the most recent state of the view process.
   * <p>
  ...
  /**
   * Sets diagnostic information that will be sent to a view process and will be available in all logging on the view process via MDC.
   * See <a href=http://logback.qos.ch/manual/mdc.html></a>. If this method is called whilst the client is already attached to a view process,
   * then an IllegalStateException will be thrown. If connecting to an existing view process, then this data is ignored (this avoids
  ...
  /**
   * Creates a reference to the latest view cycle. This is consistent with any data flow restrictions being applied through this view client,
   * so does not necessarily represent the most recent state of the view process.
   *
  ...
  //-------------------------------------------------------------------------
  /**
   * Ensures at least a minimum level of logging output is present in the results for the given value specifications. Changes will take effect
   * from the next computation cycle.
   * <p>
   * If another view client connected to the same view process has already changed the level of logging for one or more results then all view clients
   * will see the maximum level requested.
   * <p>
   ...
   * <p>
   * Results which are not terminal outputs - that is, results which were not directly requested in the view definition and are not present in the result
   * model - may be referenced, but these can only be accessed using {@link ViewCycle#queryResults(com.opengamma.engine.view.cycle.ComputationCycleQuery)}.
   *
  ...
  //-------------------------------------------------------------------------
  /**
   * Terminates this client, detaching it from any process, disconnecting it from any listener, and releasing any resources. This method <b>must</b>
