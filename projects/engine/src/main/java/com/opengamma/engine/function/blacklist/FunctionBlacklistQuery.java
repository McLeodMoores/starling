...
/**
 * Querying interface to a blacklist resource to test whether items are matched
 * by the blacklisting rules on that resource. Using a query instance is likely
 * to be faster than fetching the rules from a blacklist and testing each one as
 * it may be able to construct indexes.
 * <p>
 * Many of the functions have two forms; one taking the function identifier and
 * function parameters and one taking the composite
 * {@link com.opengamma.engine.function.ParameterizedFunction}. This is to
 * reflect different use cases (calculation nodes will use the components, graph
 * builders will use the composite). An implementation will typically write one
...
public interface FunctionBlacklistQuery {
  ...
  /**
   * Tests if a function should not be used against a target with the given
   * inputs to produce the requested outputs. The underlying blacklist may
   * relate to the individual function, target, inputs, outputs or combinations
   * of all. If used at a calculation node, the implementation may have access
   * to the value cache and base a decision on the
   * {@link com.opengamma.engine.value.ComputedValue}s corresponding to the
   * inputs.
   *
   * @param functionIdentifier
   *          the function instance to test
   * @param functionParameters
   *          the function parameters to test
   * @param target
   *          the target specification to test
   * @param inputs
   *          the input value specifications to test
   * @param outputs
   *          the output value specifications to test
   * @return true if the combination is blacklisted, false otherwise
   */
  boolean isBlacklisted(String functionIdentifier, FunctionParameters functionParameters, ComputationTargetSpecification target,
   ...
   *
   * @param function
   *          the parameterized function instance to test
   * @param target
   *          the target specification to test
   * @param inputs
   *          the input value specifications to test
  ...
  /**
   * Tests if a dependency graph node should not be used. The underlying
   * blacklist may relate to the individual function, target, inputs, outputs or
   * combinations of all. If used at a calculation node, the implementation may
   * have access to the value cache and base a decision on the
   * {@link com.opengamma.engine.value.ComputedValue}s corresponding to the
   * inputs.
   *
   * @param node
   *          the dependency graph node to test
   * @return true if the node is blacklisted, false otherwise
   */
  boolean isBlacklisted(DependencyNode node);

  /**
   * Tests if a job item should not be executed. The underlying blacklist may
   * relate to the individual function, target, inputs, outputs or combinations
   * of all. If used at a calculation node, the implementation may have access
   * to the value cache and base a decision on the
   * {@link com.opengamma.engine.value.ComputedValue}s corresponding to the
