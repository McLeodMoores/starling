...
public class ComputationCacheQueryFudgeBuilder implements FudgeBuilder<ComputationCycleQuery> {
  ...
  @Override
  public MutableFudgeMsg buildMessage(final FudgeSerializer serializer, final ComputationCycleQuery object) {
    ...
    for (final ValueSpecification valueSpecification : object.getValueSpecifications()) {
  ...
  @Override
  public ComputationCycleQuery buildObject(final FudgeDeserializer deserializer, final FudgeMsg message) {
    final ComputationCycleQuery computationCacheQuery = new ComputationCycleQuery();
    computationCacheQuery.setCalculationConfigurationName(message.getString(CALCULATION_CONFIGURATION_FIELD_NAME));

    final Collection<ValueSpecification> specs = Lists.newArrayList();

    final FudgeMsg valueSpecificationMessage = message.getMessage(VALUE_SPECIFICATIONS_FIELD_NAME);
    for (final FudgeField fudgeField : valueSpecificationMessage) {
