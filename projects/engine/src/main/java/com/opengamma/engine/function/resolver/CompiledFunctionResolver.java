...
public interface CompiledFunctionResolver {
   ...
   * <p>
   * The resolution finds functions that are capable of satisfying the requirement. If multiple functions can satisfy,
   * they should be returned from highest priority to lowest priority.
   *
   ...
   * @return the function(s) found, the specification from the output set that matches the requirement and the maximal
   * set of outputs from the function on that target
   */
  Iterator<Triple<ParameterizedFunction, ValueSpecification, Collection<ValueSpecification>>> resolveFunction(String valueName,
      ComputationTarget target, ValueProperties constraints);
