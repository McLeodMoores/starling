...
/*package*/ abstract class AbstractResultModel<T> implements Serializable {
  ...
  private final Map<T, Map<Pair<String, ValueProperties>, ComputedValueResult>> _valuesByName = new HashMap<>();
  ...
  protected Map<Pair<String, ValueProperties>, ComputedValueResult> getValuesByName(final T name) {
    final Map<Pair<String, ValueProperties>, ComputedValueResult> values = _valuesByName.get(name);
    if (values != null) {
