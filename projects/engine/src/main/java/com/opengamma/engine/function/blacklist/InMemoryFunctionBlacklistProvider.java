...
/**
 * An in-memory function blacklist implementation. Any time a named blacklist is requested an empty one will be constructed if
 * this has not already been done so.
 */
public class InMemoryFunctionBlacklistProvider implements ManageableFunctionBlacklistProvider {

  private final ConcurrentMap<String, ManageableFunctionBlacklist> _blacklists = new ConcurrentHashMap<>();
