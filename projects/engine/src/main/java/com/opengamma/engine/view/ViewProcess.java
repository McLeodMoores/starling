...
/**
 * A view process represents a computation job for a {@link ViewDefinition}; that is, a sequence of one or more computation cycles. Each cycle evaluates the
 * view definition at a particular time. A job may run forever, for example for real-time evaluation of the view definition, or it may run over a finite set of
 * evaluation times, for example in batch computations.
 * <p>
...
public interface ViewProcess extends UniqueIdentifiable {
  ...
  /**
   * A convenience method for obtaining the latest view definition. This is not necessarily the version in use by any current computation.
   *
