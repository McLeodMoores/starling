...
/**
 * Provides operations for querying a dependency graph. These are designed in particular for remote use where the full dependency graph is
 * not available locally, but complex queries may need to be made.
 */
@PublicAPI
public interface DependencyGraphExplorer {
   ...
   * <p>
   * Note that if the {@link ValueSpecification} originated from the results of executing the dependency graph then its properties will uniquely identify
   * the subgraph which produced the output; otherwise, the subgraph returned may represent one of several producing the given output.
   * <p>
   ...
   * <p>
   * Note that this is similar to {@link #getSubgraphProducing}, returning what would be the root node for that graph but without the
   * overhead of constructing the terminal output subset.
   *
   * @param output the output, not null
