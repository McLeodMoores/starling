...
public interface ViewProcessWorkerFactory {
  ...
  /**
   * Creates a worker to execute the view for the process. The nature of the worker will be chosen based on system resources/capabilities, the
   * execution options, and any attributes of the view definition that can influence how best to execute it. Possible options include a local thread,
   * a pool of local threads, and/or a proxy to remotely executing code.
   * <p>
   * The context passed to the job encapsulates all necessary details about the owning process and provides a feedback mechanism for the results of
   * execution to be posted to.
   *
