...
/**
 * Read/write interface to a blacklist resource. An update interface is typically obtained from a {@link ManageableFunctionBlacklistProvider}.
 * A blacklist resource may be shared and be used/updated by multiple agents. The order of updates is not guaranteed to other agents querying
 * the list as they may be performing best efforts caching.
 */
public interface ManageableFunctionBlacklist extends FunctionBlacklist {
  ...
  /**
   * Adds one or more rules to the blacklist with a default time to live. If there are already matching rules their time to live must be
  ...
  /**
   * Adds one or more rules to the blacklist with a specific time to live. If there are already matching rules their time to live must be
   * replaced with the new value.
   *
   * @param rules the rules to add, not null
   * @param timeToLive the time to live in seconds
