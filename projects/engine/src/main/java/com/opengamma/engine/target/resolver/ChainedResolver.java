...
public class ChainedResolver<T extends UniqueIdentifiable> implements ObjectResolver<T> {
  ...
  private static final class ChainedDeepResolver implements DeepResolver {
    ...
    ChainedDeepResolver(final DeepResolver first, final DeepResolver second) {
      _first = first;
      _second = second;
    }
  ...
  public ChainedResolver(final ObjectResolver<? extends T> first, final ObjectResolver<? extends T> second) {
    _first = first;
    _second = second;
    final DeepResolver firstDeep = first.deepResolver();
    final DeepResolver secondDeep = second.deepResolver();
    if (firstDeep != null) {
      if (secondDeep != null) {
  ...
  /**
   * Utility function for creating a chain of resolvers, for example when adding them to a {@link ComputationTargetTypeMap}. The first parameter is the second
   * resolver to use (the existing one in the map), the second parameter is the first resolver (the new value to the map). The returned value is the resolver
   * chain.
   *
   * @deprecated Use {@link #CREATE_RESOLVER}
   */
  @Deprecated
   ...
   * chain.
   */
  public static final BiFunction<ObjectResolver<?>, ObjectResolver<?>, ObjectResolver<?>> CREATE_RESOLVER = new BiFunction<ObjectResolver<?>, ObjectResolver<?>, ObjectResolver<?>>() {
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @Override
