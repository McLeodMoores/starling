...
public class ChainedResolver<T extends UniqueIdentifiable> implements ObjectResolver<T> {
  ...
  /**
   * Utility function for creating a chain of resolvers, for example when adding them to a {@link ComputationTargetTypeMap}.
   * The first parameter is the second resolver to use (the existing one in the map), the second parameter is the first
   * resolver (the new value to the map). The returned value is the resolver chain.
   */
  public static final Function2<ObjectResolver<?>, ObjectResolver<?>, ObjectResolver<?>> CREATE =
      new Function2<ObjectResolver<?>, ObjectResolver<?>, ObjectResolver<?>>() {
