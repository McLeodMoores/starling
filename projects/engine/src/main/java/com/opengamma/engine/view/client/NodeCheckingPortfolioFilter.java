...
/**
 * Portfolio filter that checks each node and its children to determine whether it should remain in the filtered set. The principles for the filtering are:
 *
 * P-root - P-child1 - P-child11 - P-child2 - P-child21 - P-child22 - P-child3 - P-child31
 *
 * If we are permissioned for the whole portfolio, do no filtering. If we are permissioned for whole branch e.g. just P-child2 in diagram then take that node as
 * the new root. If we are permissioned for discrete branches then we need to create artificial parents to fill out the graph If we are doing any filtering then
 * aggregates become meaningless (as they will include results from missing nodes) so they need to be replaced with the artificial nodes. Finally, strip off any
 * partially-applicable roots which only have a single child as they add clutter without much value
 */
public class NodeCheckingPortfolioFilter implements PortfolioFilter {

  /**
   * Generates a unique id for the portfolios. As the ids are never actually used (for database lookups etc) we can just use an int.
   */
  private static int s_portfolioId;
  /**
   * Generates a unique id for the portfolio nodes. As the ids are never actually used (for database lookups etc) we can just use an int.
   */
  private static int s_portfolioNodeId;

  /**
   * The node checker which performs a permission check on an individual node disregarding its parents and children.
   */
  ...
  /**
   * Recursively copy or remove nodes depending on whether they are accessible or not.
   *
   * @param checker
   *          the checker to use for each node
   * @param node
   *          the node tree to copy
   * @return an optional node tree, empty if there are no permissions, else populated with the copied node tree
   */
  private Optional<? extends PortfolioNode> buildRestrictedRootNode(final PortfolioPermissionChecker checker,
    ...
    switch (checker.permissionCheck(node)) {
      ...
      default:
        ...
        final SimplePortfolioNode newRoot = new SimplePortfolioNode(UniqueId.of("RESTRICTED_NODE", "PN_" + s_portfolioNodeId++),
        ...
        for (final Map.Entry<PortfolioNode, PortfolioPermission> entry : getAccessibleChildNodes(node, checker).entrySet()) {
          final PortfolioNode childNode = entry.getValue() == ALLOW ? entry.getKey() : buildRestrictedRootNode(checker, entry.getKey()).get();
