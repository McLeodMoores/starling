...
/* package */abstract class WatchedJob extends DispatchableJob {
   ...
   */
  WatchedJob(final DispatchableJob creator, final CalculationJob job) {
    super(creator, job);
  }
  ...
  private static final class HalfJobResultReceiver implements JobResultReceiver {
    ...
    HalfJobResultReceiver(final JobResultReceiver underlying, final WatchedJob creator, final CalculationJob next) {
    ...
    private void resultReceived(final CalculationJobSpecification spec, final long duration, final Set<String> nodeIds,
      ...
      if (next != null) {
        ...
        if (_underlying instanceof HalfJobResultReceiver) {
          ...
          for (final String nodeId : nodeIdSet) {
  ...
  /**
   * Split a job into two that can be executed sequentially. Any values that are produced in the first but needed in the
   * second and previously considered "private" will be "shared" in the new jobs. The job identifier of the second will
   * be the same as the original job so that any of its tails will recognize its completion as before. The first will
   * receive a new job identifier from JobIdSource.
   *
  ...
  private static DispatchableJob splitJob(final WatchedJob creator, final CalculationJob job) {
    ...
    final List<CalculationJobItem> headItems = new ArrayList<>(headItemCount);
    final Set<ValueSpecification> headShared = new HashSet<>();
    final Set<ValueSpecification> headPrivate = new HashSet<>();
    ...
    final List<CalculationJobItem> tailItems = new ArrayList<>(tailItemCount);
    final Set<ValueSpecification> tailShared = new HashSet<>();
    final Set<ValueSpecification> tailPrivate = new HashSet<>();
    ...
    final CalculationJob head = new CalculationJob(job.getSpecification().withJobId(JobIdSource.getId()),
        job.getFunctionInitializationIdentifier(), job.getResolverVersionCorrection(), null, headItems, headHint);
    ...
    final CalculationJob tail = new CalculationJob(job.getSpecification(), job.getFunctionInitializationIdentifier(),
        job.getResolverVersionCorrection(), null, tailItems, tailHint);
