 ...
 * <p>
 * A node consists of a computation target (e.g. a {@link Security}), a function to operate on that target, input values for the function, and
 * output values generated by the function. Relationships with other nodes - either input or dependent - indicate how input values are
 * produced and how output values are to be used.
 * <p>
 * A node instance only holds references to nodes that produce its input values. Any given node, and the sub-graph required to produce its
 * outputs, may then be present in multiple graphs. Node instances are immutable and thread-safe.
 */
@PublicAPI
public interface DependencyNode {
   ...
   * </ul>
   * <p>
   * Note that comparing two nodes in this way can be an expensive operation for large/deep graphs, especially if used as part of a general
   * graph comparison. This should be used for debugging/testing only.
   */
  Strategy<DependencyNode> HASHING_STRATEGY = new Strategy<DependencyNode>() {
    ...
    public boolean equals(final DependencyNode a, final DependencyNode b) {
      ...
      for (final Map.Entry<ValueSpecification, DependencyNode> aInput : aInputs.entrySet()) {
