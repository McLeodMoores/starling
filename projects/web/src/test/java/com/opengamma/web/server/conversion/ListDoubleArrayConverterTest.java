...
/**
 * Tests for {@link ListDoubleArrayConverter}.
 */
@Test(groups = TestGroup.UNIT)
public class ListDoubleArrayConverterTest {
  ...
  private static final ListDoubleArrayConverter CONVERTER = new ListDoubleArrayConverter();
  ...
  public void testText() {
    assertEquals(CONVERTER.convertToText(CONVERTERS, SPEC, VALUES), "List");
  }
  ...
  public void testFull() {
    ...
    assertArrayEquals((double[][]) map.get("matrix"), new double[][] { { 1., 4. }, { 2., 5. }, { 3., 6. } });
   ...
   * Tests creation of the summary display.
   */
  public void testSummaryObjectArray() {
    final List<Double[]> values = Arrays.asList(new Double[] { 1., 2., 3. }, new Double[] { 4., 5., 6. });
    final Object converted = CONVERTER.convertForDisplay(CONVERTERS, SPEC, values, ConversionMode.SUMMARY);
    assertTrue(converted instanceof Map);
    @SuppressWarnings("unchecked")
    final Map<String, Map<String, Object>> map = (Map<String, Map<String, Object>>) converted;
  ...
  public void testUnsupportedType() {
    final List<Float[]> values = Arrays.asList(new Float[][] { new Float[] { 1F, 2F } });
    CONVERTER.convertForDisplay(CONVERTERS, SPEC, values, ConversionMode.FULL);
  }
}
