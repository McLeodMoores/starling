...
public class WebDevBundleResource extends AbstractWebBundleResource {
  ...
  @GET
  @Produces("text/css")
  public Response get(@PathParam("bundleId") final String idStr) {
    return Response.ok(getCssImports(idStr)).build();
  }

  private String getCssImports(final String bundleId) {
    final BundleManager bundleManager = data().getDevBundleManager();
    final Bundle bundle = bundleManager.getBundle(bundleId);
