...
public class LongPollingConnectionManager {
  ...
  private final Map<String, LongPollingUpdateListener> _updateListeners = new ConcurrentHashMap<>();
   ...
   */
  /* package */ LongPollingUpdateListener handshake(final String userId, final String clientId, final ConnectionTimeoutTask timeoutTask) {
    final LongPollingUpdateListener listener = new LongPollingUpdateListener(clientId, userId, timeoutTask);
   ...
   */
  /* package */ boolean longPollHttpConnect(final String userId, final String clientId, final Continuation continuation) {
    // TODO check args
    final LongPollingUpdateListener listener = _updateListeners.get(clientId);
    if (listener != null) {
  ...
  // for testing
  /* package */ boolean isClientConnected(final String clientId) {
    final LongPollingUpdateListener listener = _updateListeners.get(clientId);
   ...
   */
  /* package */ void longPollHttpTimeout(final String clientId, final Continuation continuation) {
    final LongPollingUpdateListener listener = _updateListeners.get(clientId);
    if (listener != null) {
   ...
   */
  /* package */ void disconnect(final String clientId) {
    final LongPollingUpdateListener listener = _updateListeners.remove(clientId);
    if (listener != null) {
      listener.disconnect();
    }
  }
