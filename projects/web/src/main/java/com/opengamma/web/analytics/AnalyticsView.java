...
/**
 * This is the top level object of the back-end of the the analytics user interface. A view displays analytics data for a view definition and a set of market
 * data (e.g. live, historical etc). A view manages two grids of data, one displaying the portfolio analytics and the other displaying primitives (e.g. curves,
 * surfaces). Each of these grids can have any number of viewports which represent the portion of the grid that the user is viewing.
 * <p>
 * Each top level grid can have also have any number of dependency graph grids which show how the data in a grid cell is calculated.
 */
public interface AnalyticsView {
   ...
   *          the compiled view definition whose data will be displayed in the grids
   * @param resolvedPortfolio
   *          the view's portfolio with all securities resolved
   ...
   *
   * @param t
   *          Exception that triggered the failure, possibly null
   ...
   *
   * @param results
   *          the results of the calculation cycle
   * @param viewCycle
   *          the data associated with the calculation cycle
   ...
   *
   * @param gridType
   *          the required grid structure, not null
   * @param viewportId
  ...
  /**
   * Creates a viewport for one of the top level grids. A viewport represents the visible portion of the grid. Any grid cells that are scrolled off the screen
   * are not part of the viewport. The client only receives data for cells in the viewport and only receives notification of new data if data in the viewport
   * changes. If the only data that changes in a calculation cycle is not part of a viewport then no update needs to be sent to the client. There can be any
   * number of viewports for each grid.
   *
   * @param requestId
   *          the request id
   * @param gridType
   *          the required grid structure, not null
   * @param viewportId
   *          the unique ID for the viewport, unique for each viewport in a view
   * @param callbackId
   *          the value that is sent to the client with notification that new data is available for the viewport. The server makes no assumptions about its
   *          format other than the fact that it must be unique for each viewport in a view.
   * @param structureCallbackId
   *          the value that is sent to the client with notification that new structure is available for the viewport. The server makes no assumptions about its
   *          format other than the fact that it must be unique for each viewport in a view.
   * @param viewportDefinition
   *          defines the rows and columns in the viewport and whether the viewport's data should be expanded or a summary for data types which can't fit in a
   *          cell, e.g. vectors, matrices, curves.
   ...
   *
   * @param gridType
   *          the grid that the dependency graph grid belongs to
   * @param graphId
  ...
  /**
   * Opens a grid showing the dependency graph of calculations for a cell in one of the main grids. TODO should include the structure version otherwise there's
   * a minor race condition
   *
   * @param requestId
   *          the ID of the request
   * @param gridType
   *          Specifies which of the main grids
   * @param graphId
   *          A unique ID for the dependency graph grid
   * @param callbackId
   *          A value that is sent to the client with notification that the structure has changed. The server makes no assumptions about its format other than
   *          the fact that it must be unique for each grid in a view.
   * @param row
   *          The row of the cell whose dependency graph should be opened
   * @param col
   *          The column of the cell whose dependency graph should be opened
   */
  void openDependencyGraph(int requestId, GridType gridType, int graphId, String callbackId, int row, int col);

  /**
   * Opens a grid showing the dependency graph of calculations for a cell in one of the main grids. This is used by the client to reconnect after a server
   * restart when the server has lost all the view state.
   *
   * @param requestId
   *          the ID of the request
   * @param gridType
   ...
   *          Name of the calculation configuration containing the value we're interested in
   * @param valueRequirement
   *          Requirement which requests the value we're interested in
   */
  ...
  /**
   * Creates a viewport for a dependency graph grid. A viewport represents the visible portion of the grid. Any grid cells that are scrolled off the screen are
   * not part of the viewport. The client only receives data for cells in the viewport and only receives notification of new data if data in the viewport
   * changes. If the only data that changes in a calculation cycle is not part of a viewport then no update needs to be sent to the client. There can be any
   * number of viewports for each grid.
   *
   * @param requestId
   ...
   *          the ID of the dependency graph
   * @param viewportId
   *          the unique ID for the viewport
   * @param callbackId
   *          the value that is sent to the client with notification that new data is available for the viewport. The server makes no assumptions about its
   *          format other than the fact that it must be unique for each viewport in a view.
   * @param structureCallbackId
   ...
   *
   * @param gridType
   *          the grids that the dependency graph grid belongs to
   * @param graphId
   *          the ID of the dependency graph
   * @param viewportId
   *          the ID of the viewport
   ...
   *
   * @param gridType
   *          the grid type, not null.
   * @param format
   *          the type formatter type, not null.
   * @return the current data for the viewport
   */
