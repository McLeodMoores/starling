...
public final class DependencyGraphGrid extends AnalyticsGrid<DependencyGraphViewport> {
   ...
   *          the view definition from which the graph and calculations were
   *          derived
   ...
   * @param callbackId
   *          the ID that's passed to listeners when the row and column
   *          structure of the grid changes
   * @param targetResolver
   *          the resolver for looking up the target of the calculation given
   *          its specification
   * @param functions
   *          the function repository for any additional function metadata, not
   *          null
   ...
   */
  /* package */static DependencyGraphGrid create(final CompiledViewDefinition compiledViewDef, final ValueRequirement valueRequirement, final String calcConfigName, final ViewCycle cycle, final String callbackId,
      final ComputationTargetResolver targetResolver, final FunctionRepositoryFactory functions, final ViewportListener viewportListener, final ValueMappings valueMappings) {
    final DependencyGraphStructureBuilder builder = new DependencyGraphStructureBuilder(compiledViewDef, valueRequirement, calcConfigName, targetResolver, functions.constructRepository(Instant
   ...
   * Creates an instance.
   */
  private DependencyGraphGrid(final DependencyGraphGridStructure gridStructure, final String calcConfigName, final String callbackId, final ViewCycle cycle, final ViewportListener viewportListener,
  ...
  @Override
  protected DependencyGraphViewport createViewport(final ViewportDefinition viewportDefinition, final String callbackId, final String structureCallbackId, final ResultsCache cache) {
