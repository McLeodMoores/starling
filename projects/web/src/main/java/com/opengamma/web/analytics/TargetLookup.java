...
/* package */ class TargetLookup {
  ...
  /* package */ TargetLookup(final UnversionedValueMappings valueMappings, final List<? extends MainGridStructure.Row> rows) {
  ...
  /* package */ Pair<String, ValueRequirement> getRequirementForCell(final int rowIndex, final ColumnSpecification colSpec) {
    ...
    final ValueRequirement valueReq = new ValueRequirement(colSpec.getValueName(), row.getTarget(), colSpec.getValueProperties());
    final String calcConfigName = colSpec.getCalcConfigName();
  ...
  /* package */ Pair<String, ValueSpecification> getTargetForCell(final int rowIndex, final ColumnSpecification colSpec) {
    if (rowIndex < 0 || rowIndex >= _rows.size()) {
      throw new IllegalArgumentException("Row is outside grid bounds: row=" + rowIndex + ", rowCount=" + _rows.size());
    }
  ...
  private Pair<String, ValueSpecification> getTargetForCell(final MainGridStructure.Row row, final ColumnSpecification colSpec) {
    if (colSpec == null) {
      return null;
    }
  ...
  /* package */ Iterator<Pair<String, ValueSpecification>> getTargetsForColumn(final ColumnSpecification colSpec) {
    return Iterators.transform(_rows.iterator(), new Function<MainGridStructure.Row, Pair<String, ValueSpecification>>() {
      @Override
      public Pair<String, ValueSpecification> apply(final MainGridStructure.Row row) {
  ...
  /* package */ MainGridStructure.Row getRow(final int index) {
