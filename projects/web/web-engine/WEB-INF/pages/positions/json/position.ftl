{
    ...
    "trades": [
        <#list position.trades as trade>{
            ...
            "id": "${trade.uniqueId.objectId}", <#-- note the toString below have formatters available if 310 needs them, see history for this file -->
            "trade_date_time": "<#if trade.tradeDate?has_content>${trade.tradeDate}</#if><#if trade.tradeTime?has_content> ${trade.tradeTime.toString()} (${trade.tradeTime.toString()})</#if>",
            "premium": "<#if trade.premium?has_content>${trade.premium}</#if><#if trade.premiumCurrency?has_content> ${trade.premiumCurrency}</#if>",
            "premium_date_time": "<#if trade.premiumDate?has_content>${trade.premiumDate}</#if><#if trade.premiumTime?has_content> ${trade.premiumTime.toString()} (${trade.premiumTime.toString()})</#if>",
