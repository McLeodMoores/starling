...
public class LogLinearExtrapolator1DTest {
  ...
  public void sameIntervalsTest() {
    ...
    final double[][] yValues = new double[][] { { 1.001, 1.001, 1.001, 1.001, 1.001, 1.001, 1.001, 1.001, 1.001, 1.001 },
      { 11., 11., 8., 5., 1.001, 1.001, 5., 8., 11., 11. }, { 1.001, 1.001, 5., 8., 9., 9., 11., 12., 18., 18. } };
      ...
      final Interpolator1D interp = NamedInterpolator1dFactory.of(MonotonicLogNaturalCubicSplineInterpolator1dAdapter.NAME);
      ...
      final NamedInterpolator1d combined2 = NamedInterpolator1dFactory.of(MonotonicLogNaturalCubicSplineInterpolator1dAdapter.NAME,
          LogLinearExtrapolator1dAdapter.NAME, LogLinearExtrapolator1dAdapter.NAME);
      ...
      for (int k = 0; k < yDim; ++k) {
        ...
        for (int j = 0; j < nData; ++j) {
          ...
          final Interpolator1DDataBundle dataBund1Up = combined1.getDataBundleFromSortedArrays(xValues, yValues1Up);
          final Interpolator1DDataBundle dataBund1Dw = combined1.getDataBundleFromSortedArrays(xValues, yValues1Dw);
          for (int i = 0; i < nKeys; ++i) {
            final double res1 = 0.5 * (combined1.interpolate(dataBund1Up, xKeys[i]) - combined1.interpolate(dataBund1Dw, xKeys[i])) / EPS / yValues[k][j];
            assertEquals(res1, combined1.getNodeSensitivitiesForValue(bundle1, xKeys[i])[j], Math.max(Math.abs(yValues[k][j]) * EPS, EPS) * 1.e2);// because
            // gradient
            // is
            // NOT
            // exact
  ...
  public void differentIntervalsTest() {
    final double[] xValues = new double[] { 1.0328724558967068, 1.2692381049172323, 2.8611430465380905, 4.296118458251132, 7.011992052151352, 7.293354144919639,
        8.557971037612713, 8.77306861567384, 10.572470371584489, 12.96945799507056 };
    final double[][] yValues = new double[][] {
      { 1.1593075755231343, 2.794957672828094, 4.674733634811079, 5.517689918508841, 6.138447304104604, 6.264375977142906, 6.581666492568779,
        8.378685055774037, 10.005246918325483, 10.468304334744241 },
      { 9.95780079114617, 8.733013195721913, 8.192165283188197, 6.539369493529048, 6.3868683960757515, 4.700471352238411, 4.555354921077598,
          3.780781869340659, 2.299369456202763, 0.9182441378327986 } };
          final int nData = xValues.length;
          final int nKeys = 100 * nData;
          final double[] xKeys = new double[nKeys];
