...
import com.opengamma.analytics.financial.provider.curve.builder.DiscountingMethodBondCurveBuilder;
import com.opengamma.analytics.financial.provider.curve.builder.DiscountingMethodBondCurveSetUp;
...
/**
 * Builds and tests discounting and US government curves. The discounting curve and government curves are constructed
 * simultaneously.
 * <p>
 * The discounting curve contains the overnight deposit rate and OIS swaps. The government curve contains the overnight
 * deposit rate and bills, notes and bonds.
 */
...
public class UsdDiscountingGovernment3Test extends CurveBuildingTests {
  ...
  static {
    for (int i = 0; i < BILL_MATURITY.length; i++) {
      BILL_SECURITY[i] = new BillSecurityDefinition(Currency.USD, BILL_MATURITY[i], 1, 1, NYC, YIELD_BILL_USGOVT,
          DayCounts.ACT_360, NAME_COUNTERPART);
  ...
  private static final ZonedDateTime[] BOND_START_ACCRUAL_DATE = new ZonedDateTime[] {
      DateUtils.getUTCDate(2012, 07, 31), DateUtils.getUTCDate(2012, 8, 15), DateUtils.getUTCDate(2012, 07, 31),
    DateUtils.getUTCDate(2012, 07, 31), DateUtils.getUTCDate(2012, 8, 15), DateUtils.getUTCDate(2012, 8, 15) };
  /** Bond maturities */
  private static final ZonedDateTime[] BOND_MATURITY = new ZonedDateTime[] {
      DateUtils.getUTCDate(2014, 07, 31), DateUtils.getUTCDate(2015, 05, 15), DateUtils.getUTCDate(2017, 07, 31),
  ...
  static {
    for (int i = 0; i < BOND_MATURITY.length; i++) {
      BOND_SECURITY[i] = BondFixedSecurityDefinition.from(Currency.USD, BOND_MATURITY[i], BOND_START_ACCRUAL_DATE[i], Period.ofMonths(6), RATE_FIXED[i], 3,
          1, NYC_OLD, DayCounts.ACT_360, BusinessDayConventions.FOLLOWING, YIELD_BOND_USGOVT, false, NAME_COUNTERPART, "General collateral");
  ...
  static {
    final Period[] tenors = new Period[] {
        Period.ofDays(0), Period.ofDays(0), Period.ofDays(0), Period.ofDays(0), Period.ofDays(0), Period.ofDays(0), Period.ofDays(0),
   ...
   * discounting curve and government curve. The sensitivities to the government curve should be zero.
  ...
  private static void testDiscountingCurveSensitivities(final CurveBuildingBlockBundle fullInverseJacobian,
    ...
    assertFiniteDifferenceSensitivities(fullInverseJacobian, fixingTs, BUILDER_FOR_TEST, CURVE_NAME_DSC_USD,
        CURVE_NAME_GOVTUS_USD, NOW, GOVTUS_USD_GENERATORS, GOVTUS_USD_ATTR, GOVTUS_USD_MARKET_QUOTES, true);
  }

  /**
   * Tests the sensitivities of the government curve to changes in the market data points used in the
   * discounting and government curves.
   * @param fullInverseJacobian  analytic sensitivities
   * @param fixingTs  the fixing time series
   */
  private static void testGovernmentCurveSensitivities(final CurveBuildingBlockBundle fullInverseJacobian,
      final Map<Index, ZonedDateTimeDoubleTimeSeries> fixingTs) {
    // sensitivities to discounting
    assertFiniteDifferenceSensitivities(fullInverseJacobian, fixingTs, BUILDER_FOR_TEST, CURVE_NAME_GOVTUS_USD,
        CURVE_NAME_DSC_USD, NOW, DSC_USD_GENERATORS, DSC_USD_ATTR, DSC_USD_MARKET_QUOTES, false);
