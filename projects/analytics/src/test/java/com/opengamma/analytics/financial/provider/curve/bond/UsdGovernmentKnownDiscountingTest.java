...
public class UsdGovernmentKnownDiscountingTest extends CurveBuildingTests {
  ...
  static {
    final DiscountingMethodCurveSetUp builder = DiscountingMethodCurveBuilder.setUp().building(CURVE_NAME_DSC_USD).using(CURVE_NAME_DSC_USD)
  ...
  /** The curve builder */
  private static final DiscountingMethodBondCurveSetUp BUILDER_FOR_TEST = DiscountingMethodBondCurveBuilder.setUp().building(CURVE_NAME_GOVTUS_USD)
  ...
  static {
    BEFORE_TODAYS_FIXING = BUILDER_FOR_TEST.copy().withFixingTs(FIXING_TS_WITHOUT_TODAY).withKnownData(KNOWN_DATA_BEFORE_FIXING).getBuilder().buildCurves(NOW);
    AFTER_TODAYS_FIXING = BUILDER_FOR_TEST.copy().withFixingTs(FIXING_TS_WITH_TODAY).withKnownData(KNOWN_DATA_AFTER_FIXING).getBuilder().buildCurves(NOW);
  }
  ...
  public void testInstrumentsInCurvePriceToZero() {
    final Map<String, InstrumentDefinition<?>[]> definitions = BUILDER_FOR_TEST.copy().withFixingTs(FIXING_TS_WITH_TODAY).getBuilder()
  ...
  private static void testGovernmentCurveSensitivities(final CurveBuildingBlockBundle fullInverseJacobian,
    ...
    assertFiniteDifferenceSensitivities(fullInverseJacobian, fixingTs, BUILDER_FOR_TEST.withKnownData(knownData), CURVE_NAME_GOVTUS_USD, CURVE_NAME_GOVTUS_USD,
        NOW, GOVTUS_USD_GENERATORS, GOVTUS_USD_ATTR, GOVTUS_USD_MARKET_QUOTES, false);
  }
