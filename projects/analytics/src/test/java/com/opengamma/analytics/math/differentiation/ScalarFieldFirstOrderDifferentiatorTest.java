...
public class ScalarFieldFirstOrderDifferentiatorTest {
  ...
  public void test() {
    ...
    final DoubleMatrix1D anGrad = G.apply(x);
    final DoubleMatrix1D fdFwdGrad = FORWARD.differentiate(F).apply(x);
    final DoubleMatrix1D fdCentGrad = CENTRAL.differentiate(F).apply(x);
    final DoubleMatrix1D fdBackGrad = BACKWARD.differentiate(F).apply(x);
  ...
  public void domainTest() {
    ...
    for (int k = 0; k < 3; k++) {
      final DoubleMatrix1D fdRes = fdGradFunc.apply(x[k]);
      final DoubleMatrix1D alRes = G.apply(x[k]);
      for (int i = 0; i < 2; i++) {
