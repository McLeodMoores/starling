...
public class PCHIPInterpolator1DTest {
  ...
  private static final Interpolator1D INTERPOLATOR = new PchipInterpolator1dAdapter();
  ...
  public void dataBundleTest() {
    final Interpolator1DDataBundle db = INTERPOLATOR.getDataBundle(X_DATA, Y_DATA);
    final double[] keys = db.getKeys();
    final double[] values = db.getValues();
  ...
  public void montonicTest() {
    ...
    for (int i = 1; i < n; i++) {
      final double x = low + i * range / (n - 1);
      final double y = INTERPOLATOR.interpolate(DATA, x);
      assertTrue(y > value);
      value = y;
      if (print) {
  ...
  public void samexNodesTest() {
    ...
    final double y = INTERPOLATOR.interpolate(data, 1.0);
