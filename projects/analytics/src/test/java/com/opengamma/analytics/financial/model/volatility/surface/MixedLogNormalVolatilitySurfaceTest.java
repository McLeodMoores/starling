...
public class MixedLogNormalVolatilitySurfaceTest {
  ...
  public void printTest() {
    ...
    final BlackVolatilitySurfaceStrike ivs = MixedLogNormalVolatilitySurface.getImpliedVolatilitySurface(fc, data);
    PDEUtilityTools.printSurface("imp vol", ivs.getSurface(), 0.01, 2.0, spot * 0.1, spot * 3.0);

    final LocalVolatilitySurfaceStrike lvs = MixedLogNormalVolatilitySurface.getLocalVolatilitySurface(fc, data);
    ...
    final LocalVolatilitySurfaceStrike lv2 = cal.getLocalVolatility(ivs, fc);
    ...
    final EquityVarianceSwapStaticReplication vsPricer = new EquityVarianceSwapStaticReplication();
    ...
    final double temp = Math.sqrt(2 * (Math.log(sum1) - sum2) / t);
    System.out.println("expected: " + temp);

    final PureLocalVolatilitySurface plv = VolatilitySurfaceConverter.convertLocalVolSurface(lv2, new EquityDividendsCurvesBundle(spot, discountCurve, AffineDividends.noDividends()));
    final EquityVarianceSwapBackwardsPurePDE backCal = new EquityVarianceSwapBackwardsPurePDE();
    res = backCal.expectedVariance(spot, discountCurve, AffineDividends.noDividends(), t, plv);
    System.out.println(Math.sqrt(res[0] / t));

    final double vol = lvs.getVolatility(0.0, 25.0);
  ...
  public void flatTest() {
    ...
    final double[] w = new double[] {0.6, 0.4 };
    final double[] sigma = new double[] {vol, vol };
    final MultiHorizonMixedLogNormalModelData data = new MultiHorizonMixedLogNormalModelData(w, sigma);
    final BlackVolatilitySurfaceStrike ivs = MixedLogNormalVolatilitySurface.getImpliedVolatilitySurface(fc, data);
    final LocalVolatilitySurfaceStrike lvs = MixedLogNormalVolatilitySurface.getLocalVolatilitySurface(fc, data);
    for (int i = 0; i < 100; i++) {
      final double t = 5.0 * RANDOM.nextDouble();
      //strikes in range +- 8-sigma
      final double a = 16 * (RANDOM.nextDouble() - 0.5);
      final double k = spot * Math.exp((r - vol * vol / 2) * t + Math.sqrt(t) * vol * a);
      final double iv = ivs.getVolatility(t, k);
      final double lv = lvs.getVolatility(t, k);
  ...
  public void nonflatTest1() {
    ...
    final ForwardCurve fc = new ForwardCurve(spot, r);
    final double vol = 0.3;
    final double[] w = new double[] {0.8, 0.2 };
    final double[] sigma = new double[] {0.2, 0.7 };
    final MultiHorizonMixedLogNormalModelData data = new MultiHorizonMixedLogNormalModelData(w, sigma);
    final BlackVolatilitySurfaceStrike ivs = MixedLogNormalVolatilitySurface.getImpliedVolatilitySurface(fc, data);
    final LocalVolatilitySurfaceStrike lvs = MixedLogNormalVolatilitySurface.getLocalVolatilitySurface(fc, data);
    final LocalVolatilitySurfaceStrike lvs2 = dupire.getLocalVolatility(ivs, fc);
    ...
    for (int i = 0; i < 100; i++) {
      ...
      final double k = spot * Math.exp((r - vol * vol / 2) * t + Math.sqrt(t) * vol * a);
      final double lv = lvs.getVolatility(t, k);
      final double lv2 = lvs2.getVolatility(t, k);
  ...
  public void nonflatTest2() {
    ...
    final ForwardCurve fc = new ForwardCurve(spot, r);
    final double vol = 0.3;
    final double[] w = new double[] {0.9, 0.1 };
    final double[] sigma = new double[] {0.2, 0.7 };
    final double[] mu = new double[] {0.1, -0.1 };
  ...
  public void shortDatedOptionTest() {
    ...
    final BlackVolatilitySurfaceStrike ivs = MixedLogNormalVolatilitySurface.getImpliedVolatilitySurface(fwdCurve, mlnData);
    final LocalVolatilitySurfaceStrike lvs = MixedLogNormalVolatilitySurface.getLocalVolatilitySurface(fwdCurve, mlnData);
    final LocalVolatilitySurfaceMoneyness lvsm = LocalVolatilitySurfaceConverter.toMoneynessSurface(lvs, fwdCurve);
    ...
    final ConvectionDiffusionPDE1DStandardCoefficients pde = pdeProvider.getForwardLocalVol(lvsm);
    final Function1D<Double, Double> initialCond = initialConProvider.getForwardCallPut(true);
    final double xL = 0.8;
    final double xH = 1.2;
    final BoundaryCondition lower = new NeumannBoundaryCondition(-1.0, xL, true);
    ...
    final PDE1DDataBundle<ConvectionDiffusionPDE1DCoefficients> dbF = new PDE1DDataBundle<ConvectionDiffusionPDE1DCoefficients>(pde, initialCond, lower, upper, grid);
    final PDETerminalResults1D res = (PDETerminalResults1D) solver.solve(dbF);
    final double minK = Math.exp(-6 * rootT);
    final double maxK = Math.exp(6 * rootT);
    final Map<Double, Double> vols = PDEUtilityTools.priceToImpliedVol(fwdCurve, t, res, minK, maxK, true);
    final Interpolator1D interpolator = NamedInterpolator1dFactory.of("DoubleQuadratic");
    final Interpolator1DDataBundle idb = interpolator.getDataBundle(vols);

    //set up for solving backwards PDE
    final ConvectionDiffusionPDE1DStandardCoefficients pdeB = pdeProvider.getBackwardsLocalVol(t, lvsm);
    final double sL = xL * spot;
    final double sH = xH * spot;
    ...
    final int index = SurfaceArrayUtils.getLowerBoundIndex(gridB.getSpaceNodes(), spot);
    final double s1 = gridB.getSpaceNode(index);
    final double s2 = gridB.getSpaceNode(index + 1);
    final double w = (s2 - spot) / (s2 - s1);

    //solve a separate backwards PDE for each strike
    for (int i = 0; i < 10; i++) {
      final double z = -5 + i;
      final double k = spot * Math.exp(0.4 * rootT * z);
      final double x = k / fwd;
      final double vol = ivs.getVolatility(t, k);
      final double volFPDE = interpolator.interpolate(idb, x);

      final boolean isCall = k >= fwd;
      final BoundaryCondition lowerB = new NeumannBoundaryCondition(isCall ? 0 : -1, sL, true);
      final BoundaryCondition upperB = new NeumannBoundaryCondition(isCall ? 1 : 0, sH, false);

      final Function1D<Double, Double> bkdIC = initialConProvider.getEuropeanPayoff(k, isCall);
      final PDE1DDataBundle<ConvectionDiffusionPDE1DCoefficients> dbB = new PDE1DDataBundle<ConvectionDiffusionPDE1DCoefficients>(pdeB, bkdIC, lowerB, upperB, gridB);
      final PDEResults1D resB = solver.solve(dbB);
      final double price1 = resB.getFunctionValue(index);
      final double price2 = resB.getFunctionValue(index + 1);
      final double vol1 = BlackFormulaRepository.impliedVolatility(price1, s1, k, t, isCall);
      final double vol2 = BlackFormulaRepository.impliedVolatility(price2, s2, k, t, isCall);
      final double volBPDE = w * vol1 + (1 - w) * vol2;
