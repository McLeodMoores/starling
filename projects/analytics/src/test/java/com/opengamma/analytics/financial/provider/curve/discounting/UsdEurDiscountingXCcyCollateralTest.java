...
public class UsdEurDiscountingXCcyCollateralTest extends CurveBuildingTests {
  ...
  private static final GeneratorSwapXCcyIborIbor EURIBOR3MUSDLIBOR3M = new GeneratorSwapXCcyIborIbor("EURIBOR3MUSDLIBOR3M", EURIBOR_INDEX, LIBOR_INDEX, TARGET,
      NYC); // Spread on EUR leg
  /** Generates EUR/USD FX swaps */
  private static final GeneratorForexSwap GENERATOR_FX_EURUSD = new GeneratorForexSwap("EURUSD", Currency.EUR, Currency.USD, TARGET, EURIBOR_INDEX.getSpotLag(),
  ...
  private static final ZonedDateTimeDoubleTimeSeries TS_ON_USD_WITH_TODAY = ImmutableZonedDateTimeDoubleTimeSeries.ofUTC(new ZonedDateTime[] { PREVIOUS, NOW },
      new double[] { 0.07, 0.08 });
  /** Fed funds fixing series before today's fixing */
  private static final ZonedDateTimeDoubleTimeSeries TS_ON_USD_WITHOUT_TODAY = ImmutableZonedDateTimeDoubleTimeSeries.ofUTC(new ZonedDateTime[] { PREVIOUS },
      new double[] { 0.07 });
  /** EONIA fixing series after today's fixing */
  private static final ZonedDateTimeDoubleTimeSeries TS_ON_EUR_WITH_TODAY = ImmutableZonedDateTimeDoubleTimeSeries.ofUTC(new ZonedDateTime[] { PREVIOUS, NOW },
      new double[] { 0.06, 0.07 });
  /** EONIA fixing series before today's fixing */
  private static final ZonedDateTimeDoubleTimeSeries TS_ON_EUR_WITHOUT_TODAY = ImmutableZonedDateTimeDoubleTimeSeries.ofUTC(new ZonedDateTime[] { PREVIOUS },
      new double[] { 0.06 });
  ...
  private static final DiscountingMethodCurveSetUp BUILDER = DiscountingMethodCurveBuilder.setUp().buildingFirst(CURVE_NAME_USD_DSC_FF)
      ...
      .thenBuilding(CURVE_NAME_USD_FWD_L3).using(CURVE_NAME_USD_FWD_L3).forIborIndex(LIBOR_INDEX.toIborTypeIndex()).withInterpolator(INTERPOLATOR_LINEAR)
      .thenBuilding(CURVE_NAME_EUR_DSC_EO).using(CURVE_NAME_EUR_DSC_EO).forDiscounting(Currency.EUR).forOvernightIndex(EONIA_INDEX.toOvernightIndex())
      .withInterpolator(INTERPOLATOR_LINEAR).thenBuilding(CURVE_NAME_EUR_FWD_E3).using(CURVE_NAME_EUR_FWD_E3).forIborIndex(EURIBOR_INDEX.toIborTypeIndex())
      .withInterpolator(INTERPOLATOR_LINEAR).withKnownData(MULTICURVE_KNOWN_DATA);
  /** Market quotes for the USD discounting curve */
  private static final double[] USD_DSC_FF_MARKET_QUOTES = new double[] { 0.0015, 0.0015, 0.0010, 0.0010, 0.0010, 0.0010, 0.0010, 0.0010, 0.0015, 0.0020,
      0.0035, 0.0050, 0.0130 };
  /** Vanilla instrument generators for the USD discounting curve */
  private static final GeneratorInstrument<? extends GeneratorAttribute>[] USD_DSC_FF_GENERATORS = new GeneratorInstrument<?>[] { GENERATOR_DEPOSIT_ON_USD,
  ...
  private static final GeneratorInstrument<? extends GeneratorAttribute>[] USD_FWD_L3_GENERATORS = new GeneratorInstrument<?>[] { GENERATOR_USDLIBOR3M,
  ...
  private static final double[] EUR_DSC_EO_MARKET_QUOTES = new double[] { 0.0400, 0.0400, 0.0400, 0.0400, 0.0400, 0.0400, 0.0400, 0.0400, 0.0400, 0.0400,
      0.0400, 0.0400 };
  /** Vanilla instrument generators for the EUR discounting curve */
  private static final GeneratorInstrument<? extends GeneratorAttribute>[] EUR_DSC_EO_GENERATORS = new GeneratorInstrument<?>[] { GENERATOR_DEPOSIT_ON_EUR,
  ...
  private static final GeneratorInstrument<? extends GeneratorAttribute>[] EUR_FWD_E3_GENERATORS = new GeneratorInstrument<?>[] { GENERATOR_EURIBOR3M,
  ...
  static {
    COLLATERAL_BUILDER = DiscountingMethodCurveBuilder.setUp().building(CURVE_NAME_EUR_DSC_USDFF).using(CURVE_NAME_EUR_DSC_USDFF).forDiscounting(Currency.EUR)
        .withInterpolator(INTERPOLATOR_LINEAR);
  }
  /** Market values for the EUR discounting with USD collateral curve */
  private static final double[] EUR_DSC_USDFF_MARKET_QUOTES = new double[] { 0.0010, 0.0010, 0.00025, 0.0005, 0.00075, 0.0020, 0.0030, 0.0038, -0.0050, -0.0050,
      -0.0050, -0.0045, -0.0040 };
  /** Vanilla instrument generators for the EUR discounting with USD collateral curve */
  private static final GeneratorInstrument<? extends GeneratorAttribute>[] EUR_DSC_USDFF_GENERATORS = new GeneratorInstrument<?>[] { GENERATOR_DEPOSIT_ON_EUR,
  ...
  static {
    ...
    COLLATERAL_BEFORE_FIXING = COLLATERAL_BUILDER.copy().withKnownData(knownDataBeforeFixing)
        .withKnownBundle(new CurveBuildingBlockBundle(new LinkedHashMap<>(BEFORE_FIXING.getSecond().getData()))).withFixingTs(FIXING_TS_WITHOUT_TODAY)
        .getBuilder().buildCurves(NOW);
    COLLATERAL_AFTER_FIXING = COLLATERAL_BUILDER.copy().withKnownData(knownDataAfterFixing).withFixingTs(FIXING_TS_WITH_TODAY)
        .withKnownBundle(new CurveBuildingBlockBundle(new LinkedHashMap<>(AFTER_FIXING.getSecond().getData()))).getBuilder().buildCurves(NOW);
  }
  ...
  public void testInstrumentsInCurvePriceToZero() {
    ...
    curveConstructionTest(definitions.get(CURVE_NAME_USD_DSC_FF), BEFORE_FIXING.getFirst(), PresentValueDiscountingCalculator.getInstance(),
        FIXING_TS_WITHOUT_TODAY, FX_MATRIX, NOW, Currency.USD);
    curveConstructionTest(definitions.get(CURVE_NAME_USD_FWD_L3), BEFORE_FIXING.getFirst(), PresentValueDiscountingCalculator.getInstance(),
        FIXING_TS_WITHOUT_TODAY, FX_MATRIX, NOW, Currency.USD);
    curveConstructionTest(definitions.get(CURVE_NAME_EUR_DSC_EO), BEFORE_FIXING.getFirst(), PresentValueDiscountingCalculator.getInstance(),
        FIXING_TS_WITHOUT_TODAY, FX_MATRIX, NOW, Currency.EUR);
    curveConstructionTest(definitions.get(CURVE_NAME_EUR_FWD_E3), BEFORE_FIXING.getFirst(), PresentValueDiscountingCalculator.getInstance(),
        FIXING_TS_WITHOUT_TODAY, FX_MATRIX, NOW, Currency.EUR);
    // after fixing
    definitions = BUILDER.copy().withFixingTs(FIXING_TS_WITH_TODAY).getBuilder().getDefinitionsForCurves(NOW);
    curveConstructionTest(definitions.get(CURVE_NAME_USD_DSC_FF), AFTER_FIXING.getFirst(), PresentValueDiscountingCalculator.getInstance(),
        FIXING_TS_WITH_TODAY, FX_MATRIX, NOW, Currency.USD);
    curveConstructionTest(definitions.get(CURVE_NAME_USD_FWD_L3), AFTER_FIXING.getFirst(), PresentValueDiscountingCalculator.getInstance(),
        FIXING_TS_WITH_TODAY, FX_MATRIX, NOW, Currency.USD);
    curveConstructionTest(definitions.get(CURVE_NAME_EUR_DSC_EO), AFTER_FIXING.getFirst(), PresentValueDiscountingCalculator.getInstance(),
        FIXING_TS_WITH_TODAY, FX_MATRIX, NOW, Currency.EUR);
    curveConstructionTest(definitions.get(CURVE_NAME_EUR_FWD_E3), AFTER_FIXING.getFirst(), PresentValueDiscountingCalculator.getInstance(),
        FIXING_TS_WITH_TODAY, FX_MATRIX, NOW, Currency.EUR);
    // before fixing
    definitions = COLLATERAL_BUILDER.copy().withFixingTs(FIXING_TS_WITHOUT_TODAY).getBuilder().getDefinitionsForCurves(NOW);
    curveConstructionTest(definitions.get(CURVE_NAME_EUR_DSC_USDFF), COLLATERAL_BEFORE_FIXING.getFirst(), PresentValueDiscountingCalculator.getInstance(),
        FIXING_TS_WITHOUT_TODAY, FX_MATRIX, NOW, Currency.EUR);
    // after fixing
    definitions = COLLATERAL_BUILDER.copy().withFixingTs(FIXING_TS_WITH_TODAY).getBuilder().getDefinitionsForCurves(NOW);
    curveConstructionTest(definitions.get(CURVE_NAME_EUR_DSC_USDFF), COLLATERAL_AFTER_FIXING.getFirst(), PresentValueDiscountingCalculator.getInstance(),
        FIXING_TS_WITH_TODAY, FX_MATRIX, NOW, Currency.EUR);
  }
  ...
  public void testFiniteDifferenceSensitivities() {
    ...
    assertFiniteDifferenceSensitivities(BEFORE_FIXING.getSecond(), FIXING_TS_WITHOUT_TODAY, BUILDER, CURVE_NAME_USD_DSC_FF, CURVE_NAME_USD_DSC_FF, NOW,
        USD_DSC_FF_GENERATORS, USD_DSC_FF_ATTR, USD_DSC_FF_MARKET_QUOTES, false);
    assertFiniteDifferenceSensitivities(AFTER_FIXING.getSecond(), FIXING_TS_WITH_TODAY, BUILDER, CURVE_NAME_USD_DSC_FF, CURVE_NAME_USD_DSC_FF, NOW,
        USD_DSC_FF_GENERATORS, USD_DSC_FF_ATTR, USD_DSC_FF_MARKET_QUOTES, false);
    ...
    assertFiniteDifferenceSensitivities(BEFORE_FIXING.getSecond(), FIXING_TS_WITHOUT_TODAY, BUILDER, CURVE_NAME_USD_FWD_L3, CURVE_NAME_USD_DSC_FF, NOW,
        USD_DSC_FF_GENERATORS, USD_DSC_FF_ATTR, USD_DSC_FF_MARKET_QUOTES, false);
    assertFiniteDifferenceSensitivities(AFTER_FIXING.getSecond(), FIXING_TS_WITH_TODAY, BUILDER, CURVE_NAME_USD_FWD_L3, CURVE_NAME_USD_DSC_FF, NOW,
        USD_DSC_FF_GENERATORS, USD_DSC_FF_ATTR, USD_DSC_FF_MARKET_QUOTES, false);
    // LIBOR sensitivities to LIBOR
    assertFiniteDifferenceSensitivities(BEFORE_FIXING.getSecond(), FIXING_TS_WITHOUT_TODAY, BUILDER, CURVE_NAME_USD_FWD_L3, CURVE_NAME_USD_FWD_L3, NOW,
        USD_FWD_L3_GENERATORS, USD_FWD_L3_ATTR, USD_FWD_L3_MARKET_QUOTES, false);
    assertFiniteDifferenceSensitivities(AFTER_FIXING.getSecond(), FIXING_TS_WITH_TODAY, BUILDER, CURVE_NAME_USD_FWD_L3, CURVE_NAME_USD_FWD_L3, NOW,
        USD_FWD_L3_GENERATORS, USD_FWD_L3_ATTR, USD_FWD_L3_MARKET_QUOTES, false);
    ...
    assertFiniteDifferenceSensitivities(BEFORE_FIXING.getSecond(), FIXING_TS_WITHOUT_TODAY, BUILDER, CURVE_NAME_EUR_DSC_EO, CURVE_NAME_USD_DSC_FF, NOW,
        USD_DSC_FF_GENERATORS, USD_DSC_FF_ATTR, USD_DSC_FF_MARKET_QUOTES, true);
    assertFiniteDifferenceSensitivities(AFTER_FIXING.getSecond(), FIXING_TS_WITH_TODAY, BUILDER, CURVE_NAME_EUR_DSC_EO, CURVE_NAME_USD_DSC_FF, NOW,
        USD_DSC_FF_GENERATORS, USD_DSC_FF_ATTR, USD_DSC_FF_MARKET_QUOTES, true);
    // EONIA sensitivities to LIBOR - expect sensitivities to be zero
    assertFiniteDifferenceSensitivities(BEFORE_FIXING.getSecond(), FIXING_TS_WITHOUT_TODAY, BUILDER, CURVE_NAME_EUR_DSC_EO, CURVE_NAME_USD_FWD_L3, NOW,
        USD_FWD_L3_GENERATORS, USD_FWD_L3_ATTR, USD_FWD_L3_MARKET_QUOTES, true);
    assertFiniteDifferenceSensitivities(AFTER_FIXING.getSecond(), FIXING_TS_WITH_TODAY, BUILDER, CURVE_NAME_EUR_DSC_EO, CURVE_NAME_USD_FWD_L3, NOW,
        USD_FWD_L3_GENERATORS, USD_FWD_L3_ATTR, USD_FWD_L3_MARKET_QUOTES, true);
    // EONIA sensitivities to EONIA
    assertFiniteDifferenceSensitivities(BEFORE_FIXING.getSecond(), FIXING_TS_WITHOUT_TODAY, BUILDER, CURVE_NAME_EUR_DSC_EO, CURVE_NAME_EUR_DSC_EO, NOW,
        EUR_DSC_EO_GENERATORS, EUR_DSC_EO_ATTR, EUR_DSC_EO_MARKET_QUOTES, false);
    assertFiniteDifferenceSensitivities(AFTER_FIXING.getSecond(), FIXING_TS_WITH_TODAY, BUILDER, CURVE_NAME_EUR_DSC_EO, CURVE_NAME_EUR_DSC_EO, NOW,
        EUR_DSC_EO_GENERATORS, EUR_DSC_EO_ATTR, EUR_DSC_EO_MARKET_QUOTES, false);
    ...
    assertFiniteDifferenceSensitivities(BEFORE_FIXING.getSecond(), FIXING_TS_WITHOUT_TODAY, BUILDER, CURVE_NAME_EUR_FWD_E3, CURVE_NAME_USD_DSC_FF, NOW,
        USD_DSC_FF_GENERATORS, USD_DSC_FF_ATTR, USD_DSC_FF_MARKET_QUOTES, true);
    assertFiniteDifferenceSensitivities(AFTER_FIXING.getSecond(), FIXING_TS_WITH_TODAY, BUILDER, CURVE_NAME_EUR_FWD_E3, CURVE_NAME_USD_DSC_FF, NOW,
        USD_DSC_FF_GENERATORS, USD_DSC_FF_ATTR, USD_DSC_FF_MARKET_QUOTES, true);
    // EURIBOR sensitivities to LIBOR - expect sensitivities to be zero
    assertFiniteDifferenceSensitivities(BEFORE_FIXING.getSecond(), FIXING_TS_WITHOUT_TODAY, BUILDER, CURVE_NAME_EUR_FWD_E3, CURVE_NAME_USD_FWD_L3, NOW,
        USD_FWD_L3_GENERATORS, USD_FWD_L3_ATTR, USD_FWD_L3_MARKET_QUOTES, true);
    assertFiniteDifferenceSensitivities(AFTER_FIXING.getSecond(), FIXING_TS_WITH_TODAY, BUILDER, CURVE_NAME_EUR_FWD_E3, CURVE_NAME_USD_FWD_L3, NOW,
        USD_FWD_L3_GENERATORS, USD_FWD_L3_ATTR, USD_FWD_L3_MARKET_QUOTES, true);
    // EURIBOR sensitivities to EONIA
    assertFiniteDifferenceSensitivities(BEFORE_FIXING.getSecond(), FIXING_TS_WITHOUT_TODAY, BUILDER, CURVE_NAME_EUR_FWD_E3, CURVE_NAME_EUR_DSC_EO, NOW,
        EUR_DSC_EO_GENERATORS, EUR_DSC_EO_ATTR, EUR_DSC_EO_MARKET_QUOTES, false);
    assertFiniteDifferenceSensitivities(AFTER_FIXING.getSecond(), FIXING_TS_WITH_TODAY, BUILDER, CURVE_NAME_EUR_FWD_E3, CURVE_NAME_EUR_DSC_EO, NOW,
        EUR_DSC_EO_GENERATORS, EUR_DSC_EO_ATTR, EUR_DSC_EO_MARKET_QUOTES, false);
    // EURIBOR sensitivities to EURIBOR
    assertFiniteDifferenceSensitivities(BEFORE_FIXING.getSecond(), FIXING_TS_WITHOUT_TODAY, BUILDER, CURVE_NAME_EUR_FWD_E3, CURVE_NAME_EUR_FWD_E3, NOW,
        EUR_FWD_E3_GENERATORS, EUR_FWD_E3_ATTR, EUR_FWD_E3_MARKET_QUOTES, false);
    assertFiniteDifferenceSensitivities(AFTER_FIXING.getSecond(), FIXING_TS_WITH_TODAY, BUILDER, CURVE_NAME_EUR_FWD_E3, CURVE_NAME_EUR_FWD_E3, NOW,
        EUR_FWD_E3_GENERATORS, EUR_FWD_E3_ATTR, EUR_FWD_E3_MARKET_QUOTES, false);
