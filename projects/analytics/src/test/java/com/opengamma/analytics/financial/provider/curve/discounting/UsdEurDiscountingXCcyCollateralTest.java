...
public class UsdEurDiscountingXCcyCollateralTest {
  ...
  private static final ZonedDateTimeDoubleTimeSeries TS_ON_EUR_WITH_TODAY = ImmutableZonedDateTimeDoubleTimeSeries.ofUTC(new ZonedDateTime[] {DateUtils.getUTCDate(2011, 9, 27),
      DateUtils.getUTCDate(2011, 9, 28) }, new double[] {0.06, 0.07 });
  ...
  static {
    for (int i = 0; i < 2; i++) {
      USD_DSC_FF_ATTR[i] = new GeneratorAttributeIR(USD_DSC_FF_TENOR[i], Period.ZERO);
    }
    for (int i = 2; i < USD_DSC_FF_TENOR.length; i++) {
      USD_DSC_FF_ATTR[i] = new GeneratorAttributeIR(USD_DSC_FF_TENOR[i]);
    }
  }
  ...
  static {
    for (int i = 0; i < USD_FWD_L3_TENOR.length; i++) {
      USD_FWD_L3_ATTR[i] = new GeneratorAttributeIR(USD_FWD_L3_TENOR[i]);
    }
  }
  ...
  static {
    for (int i = 0; i < EUR_DSC_EO_TENOR.length; i++) {
      EUR_DSC_EO_ATTR[i] = new GeneratorAttributeIR(EUR_DSC_EO_TENOR[i]);
    }
  }
  ...
  static {
    for (int i = 0; i < EUR_FWD_E3_TENOR.length; i++) {
      EUR_FWD_E3_ATTR[i] = new GeneratorAttributeIR(EUR_FWD_E3_TENOR[i]);
    }
  }
  ...
  static {
    for (int i = 0; i < 2; i++) {
      EUR_DSC_USDFF_ATTR[i] = new GeneratorAttributeIR(EUR_DSC_USDFF_TENOR[i], Period.ZERO);
    }
    for (int i = 2; i < EUR_DSC_USDFF_TENOR.length; i++) {
      EUR_DSC_USDFF_ATTR[i] = new GeneratorAttributeFX(EUR_DSC_USDFF_TENOR[i], FX_MATRIX);
    }
  }
  ...
  static {
    ...
    FWD_ON_MAP.put(CURVE_NAME_USD_DSC_FF, new IndexON[] {USD_OVERNIGHT_INDEX });
    FWD_ON_MAP.put(CURVE_NAME_EUR_DSC_EO, new IndexON[] {EUR_OVERNIGHT_INDEX });
    FWD_IBOR_MAP.put(CURVE_NAME_USD_FWD_L3, new IborIndex[] {USD_3M_LIBOR_INDEX });
    FWD_IBOR_MAP.put(CURVE_NAME_EUR_FWD_E3, new IborIndex[] {EUR_3M_EURIBOR_INDEX });
