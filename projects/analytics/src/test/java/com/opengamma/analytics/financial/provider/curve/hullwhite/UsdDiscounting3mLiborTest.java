...
public class UsdDiscounting3mLiborTest extends CurveBuildingTests {
  ...
  /** EDM3 */
  private static final InterestRateFutureSecurityDefinition EDM3_DEFINITION = InterestRateFutureSecurityDefinition.fromFixingPeriodStartDate(M3_START,
      USD_3M_LIBOR_INDEX, FUTURE_NOTIONAL, 0.25, "EDM3", NYC);
  /** EDU3 */
  private static final InterestRateFutureSecurityDefinition EDU3_DEFINITION = InterestRateFutureSecurityDefinition.fromFixingPeriodStartDate(U3_START,
      USD_3M_LIBOR_INDEX, FUTURE_NOTIONAL, 0.25, "EDU3", NYC);
  /** EDZ3 */
  private static final InterestRateFutureSecurityDefinition EDZ3_DEFINITION = InterestRateFutureSecurityDefinition.fromFixingPeriodStartDate(Z3_START,
      USD_3M_LIBOR_INDEX, FUTURE_NOTIONAL, 0.25, "EDZ3", NYC);
  /** EDH4 */
  private static final InterestRateFutureSecurityDefinition EDH4_DEFINITION = InterestRateFutureSecurityDefinition.fromFixingPeriodStartDate(H4_START,
      USD_3M_LIBOR_INDEX, FUTURE_NOTIONAL, 0.25, "EDH4", NYC);
  /** CTPM3 */
  private static final SwapFuturesPriceDeliverableSecurityDefinition CTPM3_DEFINITION = SwapFuturesPriceDeliverableSecurityDefinition.from(M3_START,
      USD6MLIBOR3M, Period.ofYears(2), FUTURE_NOTIONAL, 0.0050);
  /** CFPM3 */
  private static final SwapFuturesPriceDeliverableSecurityDefinition CFPM3_DEFINITION = SwapFuturesPriceDeliverableSecurityDefinition.from(M3_START,
      USD6MLIBOR3M, Period.ofYears(5), FUTURE_NOTIONAL, 0.0100);
  /** CNPM3 */
  private static final SwapFuturesPriceDeliverableSecurityDefinition CNPM3_DEFINITION = SwapFuturesPriceDeliverableSecurityDefinition.from(M3_START,
      USD6MLIBOR3M, Period.ofYears(10), FUTURE_NOTIONAL, 0.0200);
  /** CBPM3 */
  private static final SwapFuturesPriceDeliverableSecurityDefinition CBPM3_DEFINITION = SwapFuturesPriceDeliverableSecurityDefinition.from(M3_START,
  ...
  private static final ZonedDateTimeDoubleTimeSeries TS_ON_USD_WITHOUT_TODAY = ImmutableZonedDateTimeDoubleTimeSeries.ofUTC(new ZonedDateTime[] { NOW },
      new double[] { 0.0007 });
  ...
  private static final HullWhiteOneFactorProviderDiscount HW_KNOWN_DATA = new HullWhiteOneFactorProviderDiscount(KNOWN_DATA, MODEL_PARAMETERS, Currency.USD);
  ...
  private static final HullWhiteMethodCurveSetUp CONSECUTIVE_BUILDER = HullWhiteMethodCurveBuilder.setUp().buildingFirst(CURVE_NAME_DSC_USD)
      ...
      .thenBuilding(CURVE_NAME_FWD3_USD).using(CURVE_NAME_FWD3_USD).forIborIndex(USD_3M_LIBOR_INDEX.toIborTypeIndex()).withInterpolator(INTERPOLATOR)
      .withKnownData(HW_KNOWN_DATA);
  /** Builds the curves simultaneously */
  private static final HullWhiteMethodCurveSetUp SIMULTANEOUS_BUILDER = HullWhiteMethodCurveBuilder.setUp().building(CURVE_NAME_DSC_USD, CURVE_NAME_FWD3_USD)
      .using(CURVE_NAME_DSC_USD).forDiscounting(Currency.USD).forOvernightIndex(FED_FUNDS_INDEX.toOvernightIndex()).withInterpolator(INTERPOLATOR)
      .using(CURVE_NAME_FWD3_USD).forIborIndex(USD_3M_LIBOR_INDEX.toIborTypeIndex()).withInterpolator(INTERPOLATOR).withKnownData(HW_KNOWN_DATA);
  /** Market values for the discounting curve */
  private static final double[] DSC_USD_MARKET_QUOTES = new double[] { 0.0022, 0.00127, 0.00125, 0.00126, 0.00126, 0.00125, 0.001315, 0.001615, 0.00243,
  ...
  static {
    final Period[] discountingTenors = new Period[] { Period.ofDays(0), Period.ofMonths(1), Period.ofMonths(2), Period.ofMonths(3), Period.ofMonths(6),
  ...
  public void testInstrumentsInCurvePriceToZero() {
    ...
    curveConstructionTest(definitions.get(CURVE_NAME_DSC_USD), CONSECUTIVE_BEFORE_FIXING.getFirst(), PresentValueHullWhiteCalculator.getInstance(),
        FIXING_TS_WITHOUT_TODAY, FX_MATRIX, NOW, Currency.USD, EPS);
    curveConstructionTest(definitions.get(CURVE_NAME_FWD3_USD), CONSECUTIVE_BEFORE_FIXING.getFirst(), PresentValueHullWhiteCalculator.getInstance(),
        FIXING_TS_WITHOUT_TODAY, FX_MATRIX, NOW, Currency.USD, EPS);
    // after fixing
    definitions = CONSECUTIVE_BUILDER.copy().withFixingTs(FIXING_TS_WITH_TODAY).getBuilder().getDefinitionsForCurves(NOW);
    curveConstructionTest(definitions.get(CURVE_NAME_DSC_USD), CONSECUTIVE_AFTER_FIXING.getFirst(), PresentValueHullWhiteCalculator.getInstance(),
        FIXING_TS_WITH_TODAY, FX_MATRIX, NOW, Currency.USD, EPS);
    curveConstructionTest(definitions.get(CURVE_NAME_FWD3_USD), CONSECUTIVE_AFTER_FIXING.getFirst(), PresentValueHullWhiteCalculator.getInstance(),
        FIXING_TS_WITH_TODAY, FX_MATRIX, NOW, Currency.USD, EPS);
    // discounting and LIBOR
    // before fixing
    definitions = SIMULTANEOUS_BUILDER.copy().withFixingTs(FIXING_TS_WITHOUT_TODAY).getBuilder().getDefinitionsForCurves(NOW);
    curveConstructionTest(definitions.get(CURVE_NAME_DSC_USD), SIMULTANEOUS_BEFORE_FIXING.getFirst(), PresentValueHullWhiteCalculator.getInstance(),
        FIXING_TS_WITHOUT_TODAY, FX_MATRIX, NOW, Currency.USD, EPS);
    curveConstructionTest(definitions.get(CURVE_NAME_FWD3_USD), SIMULTANEOUS_BEFORE_FIXING.getFirst(), PresentValueHullWhiteCalculator.getInstance(),
        FIXING_TS_WITHOUT_TODAY, FX_MATRIX, NOW, Currency.USD, EPS);
    // after fixing
    definitions = SIMULTANEOUS_BUILDER.copy().withFixingTs(FIXING_TS_WITH_TODAY).getBuilder().getDefinitionsForCurves(NOW);
    curveConstructionTest(definitions.get(CURVE_NAME_DSC_USD), SIMULTANEOUS_AFTER_FIXING.getFirst(), PresentValueHullWhiteCalculator.getInstance(),
        FIXING_TS_WITH_TODAY, FX_MATRIX, NOW, Currency.USD, EPS);
    curveConstructionTest(definitions.get(CURVE_NAME_FWD3_USD), SIMULTANEOUS_AFTER_FIXING.getFirst(), PresentValueHullWhiteCalculator.getInstance(),
        FIXING_TS_WITH_TODAY, FX_MATRIX, NOW, Currency.USD, EPS);
  }
  ...
  private static void testDiscountingCurveSensitivities1(final CurveBuildingBlockBundle fullInverseJacobian,
    ...
    assertFiniteDifferenceSensitivities(fullInverseJacobian, fixingTs, CONSECUTIVE_BUILDER, CURVE_NAME_DSC_USD, CURVE_NAME_DSC_USD, NOW, DSC_USD_GENERATORS,
  ...
  private static void testDiscountingCurveSensitivities2(final CurveBuildingBlockBundle fullInverseJacobian,
      final Map<Index, ZonedDateTimeDoubleTimeSeries> fixingTs) {
    // sensitivities to discounting
    assertFiniteDifferenceSensitivities(fullInverseJacobian, fixingTs, SIMULTANEOUS_BUILDER, CURVE_NAME_DSC_USD, CURVE_NAME_DSC_USD, NOW, DSC_USD_GENERATORS,
        DSC_USD_ATTR, DSC_USD_MARKET_QUOTES, false);
    // sensitivities to 3m LIBOR should be zero
    assertFiniteDifferenceSensitivities(fullInverseJacobian, fixingTs, SIMULTANEOUS_BUILDER, CURVE_NAME_DSC_USD, CURVE_NAME_FWD3_USD, NOW, FWD3_USD_GENERATORS,
   ...
   *          the curve builder
   */
  private static void testLiborCurveSensitivities(final CurveBuildingBlockBundle fullInverseJacobian, final Map<Index, ZonedDateTimeDoubleTimeSeries> fixingTs,
      final HullWhiteMethodCurveSetUp builder) {
    // sensitivities to discounting
    assertFiniteDifferenceSensitivities(fullInverseJacobian, fixingTs, builder, CURVE_NAME_FWD3_USD, CURVE_NAME_DSC_USD, NOW, DSC_USD_GENERATORS, DSC_USD_ATTR,
        DSC_USD_MARKET_QUOTES, false);
    // sensitivities to 3m LIBOR
    assertFiniteDifferenceSensitivities(fullInverseJacobian, fixingTs, builder, CURVE_NAME_FWD3_USD, CURVE_NAME_FWD3_USD, NOW, FWD3_USD_GENERATORS,
