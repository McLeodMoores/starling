    ...
    for (int i = 0; i <= nStrike; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        final EuropeanVanillaOption optionVV = new EuropeanVanillaOption(strikesVV[j], forexOption[i].getTimeToExpiry(),
            true);
        vega[j] = BLACK_FUNCTION.getVegaFunction(optionVV).apply(dataBlackATM);
      }
      final double vegaFlat = BLACK_FUNCTION.getVegaFunction(forexOption[i]).apply(dataBlackATM);
      vega[1] = vegaFlat;
