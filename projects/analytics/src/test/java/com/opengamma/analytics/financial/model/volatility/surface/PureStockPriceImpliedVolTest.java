...
public class PureStockPriceImpliedVolTest {
  ...
  static {
    ...
    D = new Function1D<Double, Double>() {
      @Override
      public Double apply(final Double t) {
        final double r_t = R.apply(t);
        double sum = 0.0;
        for (int index = 0; index < N_DIVS; index++) {
          if (TAU[index] > t) {
            sum += ALPHA[index] / R.apply(TAU[index]);
          }
        }
        return sum * r_t;
      }
    ...
    final Function<Double, Double> price = new Function<Double, Double>() {
      ...
      public Double evaluate(final Double... tk) {
        final double t = tk[0];
        final double k = tk[1];
        final double f = F.apply(t);
        final double d = D.apply(t);
  ...
  public void calandarSpreadTest() {
    ...
    //before
    final double fM = F.apply(t - dt);
    final double dM = D.apply(t - dt);
