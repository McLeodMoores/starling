...
public class FailingSABRInterpolatorTest {
  ...
  private static final double[] DATE_ARRAY =
    {9.0 / 365, 44.0 / 365, 72.0 / 365, 135.0 / 365, 226.0 / 365, 317.0 / 365, 499.0 / 365, 681.0 / 365, 863.0 / 365, 1045.0 / 365, 1227. / 365 };

  private static final double[][] STRIKE_MATRIX = {
      {2550, 2650, 2700, 2725, 2750, 2775, 2800, 2825, 2850, 2900, 3000 },
  ...
  private final static double[][] VOLS_MATRIX = { {0.54257, 0.4121, 0.1627, 0.2012, 0.1439, 0.1302, 0.1354, 0.1292, 0.1308, 0.13788, 0.17734 },
  ...
  private final static double[] NODE_POINTS_1 = {1. / 365, 1.0 / 12, 3.0 / 12, 6.0 / 12, 9.0 / 12, 1, 18.0 / 12, 2, 3, 4, 5, 6, 7, 8, 9, 10, 12, 15, 20, 25, 30 };
  private final static double[] YIELDS_1 = {0.0007, 0.002647, 0.003185, 0.00342, 0.00414, 0.00501, 0.006465, 0.005805, 0.007665, 0.00997, 0.01227, 0.014275, 0.016050, 0.01763, 0.01906, 0.02034,
  ...
  private final static DoublesCurve INTER_CURVE = InterpolatedDoublesCurve.from(NODE_POINTS_1, YIELDS_1, PCHIP);
  private final static YieldCurve TEST_CURVE_1 = new YieldCurve("testkurve", INTER_CURVE);
  private final static ConstantDoublesCurve DIV = new ConstantDoublesCurve(0.0383);
  private final static YieldCurve DIV_1 = new YieldCurve("test", DIV);
  private final static ForwardCurve FWD = new ForwardCurveYieldImplied(2791.0, TEST_CURVE_1, DIV_1);

  public static void SABRSurface() {

    final SmileSurfaceDataBundle surfdatabundle = new StandardSmileSurfaceDataBundle(FWD, DATE_ARRAY, STRIKE_MATRIX, VOLS_MATRIX);
    ...
    for (int i = 0; i < 11; i++) {
      System.out.println(FWD.getForward(DATE_ARRAY[i]));
      for (int j = 0; j < 13; j++) {
        System.out.println("Expiry = " + DATE_ARRAY[i] + ", Strike = " + STRIKE_MATRIX[2][j] + ", " + "Vol = " + surf1.getVolatility(DATE_ARRAY[i], STRIKE_MATRIX[2][j]));
      }
    }
  }
