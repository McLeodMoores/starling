...
public class LogNaturalCubicMonotonicityPreservingInterpolator1DTest {
  ...
  public void localMinimumTest() {
    ...
    final Interpolator1D interpWrap = NamedInterpolator1dFactory.of(MonotonicLogNaturalCubicSplineInterpolator1dAdapter.NAME);
    ...
    for (int j = 0; j < nData; ++j) {
      ...
      final Interpolator1DDataBundle dataBund1Up = interpWrap.getDataBundleFromSortedArrays(xValues, yValues1Up);
      final Interpolator1DDataBundle dataBund1Dw = interpWrap.getDataBundleFromSortedArrays(xValues, yValues1Dw);
      for (int i = 0; i < nKeys; ++i) {
        final double res1 = 0.5 * (interpWrap.interpolate(dataBund1Up, xKeys[i]) - interpWrap.interpolate(dataBund1Dw, xKeys[i])) / EPS / yValues[j];
        ...
        {9.0115183927982674, 8.7923765265015685, 8.3742614861844871, 8.2582572028821648, 8.0294763379975009, 6.8678544068867886, 6.53398463318387, 6.4832650514113164, 5.3777446119604493,
            ...
            for (int k = 0; k < yDim; ++k) {
              ...
              for (int j = 0; j < nData; ++j) {
                ...
                final Interpolator1DDataBundle dataBund1Dw = interpWrap.getDataBundleFromSortedArrays(xValues, yValues1Dw);
                for (int i = 0; i < nKeys; ++i) {
                  final double res1 = 0.5 * (interpWrap.interpolate(dataBund1Up, xKeys[i]) - interpWrap.interpolate(dataBund1Dw, xKeys[i])) / EPS / yValues[k][j];
