...
public class AmericanExchangeOptionFunctionProviderTest {
  ...
  private static final BjerksundStenslandModel BS = new BjerksundStenslandModel();
          ...
          for (final double rho : rhoSet) {
            for (final double dividend : DIVIDENDS) {
              for (final double quant1 : quant1Set) {
                ...
                final double appDiv = BS.price(SPOT * quant1, quant2 * spot2, interest - b2, b1 - b2, TIME, volhat, true);
                ...
                final double[] greek = MODEL.getGreeks(function, SPOT, spot2, vol, sigma2, rho, interest, dividend, div2);
                final double[] greekTri = MODEL_TRI.getGreeks(functionTri, SPOT, spot2, vol, sigma2, rho, interest, dividend, div2);
          ...
          for (final double rho : rhoSet) {
            for (final double dividend : DIVIDENDS) {
              for (final double quant1 : quant1Set) {
                ...
                final double b1 = interest - dividend;
                final double b2 = interest - div2;
                final double[] app = BS.getPriceAdjoint(SPOT * quant1, quant2 * spot2, interest - b2, b1 - b2, TIME, volhat, true);
                final double price = app[0];
                final double delta1 = quant1 * app[1];
                final double delta2 = quant2 * app[2];
                //                final double theta = app[5];
                final double[] appSpot1Up = BS.getPriceAdjoint((SPOT + eps) * quant1, quant2 * spot2, interest - b2, b1 - b2, TIME, volhat, true);
                final double[] appSpot1Down = BS.getPriceAdjoint((SPOT - eps) * quant1, quant2 * spot2, interest - b2, b1 - b2, TIME, volhat, true);
                final double[] appSpot2Up = BS.getPriceAdjoint(SPOT * quant1, quant2 * (spot2 + eps), interest - b2, b1 - b2, TIME, volhat, true);
                final double[] appSpot2Down = BS.getPriceAdjoint(SPOT * quant1, quant2 * (spot2 - eps), interest - b2, b1 - b2, TIME, volhat, true);
