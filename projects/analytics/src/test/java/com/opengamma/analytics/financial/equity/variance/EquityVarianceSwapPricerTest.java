...
public class EquityVarianceSwapPricerTest {
  ...
  private static final double[][] STRIKES = new double[][] { {50, 55, 60, 65, 70, 75, 80}, {50, 55, 60, 65, 70, 75, 80}, {50, 55, 60, 65, 70, 75, 80},
    ...
    // private static final AffineDividends ZERO_DIVIDENDS = new
    // AffineDividends(TAU, new double[5], new double[5]);
    ...
    static {
      // PureImpliedVolatilitySurface flat = new
      // PureImpliedVolatilitySurface(ConstantDoublesSurface.from(PURE_VOL));
      // find "market" prices if the pure implied volatility surface was flat
      // OTM_PRICES_FLAT = getOptionPrices(SPOT, EXPIRIES, STRIKES,
      // DISCOUNT_CURVE, DIVIDENDS, flat);
      ...
      // get the 'market' prices for the hypothetical pure implied volatility
      // surface
      // OTM_PRICES = getOptionPrices(SPOT, EXPIRIES, STRIKES, DISCOUNT_CURVE,
      // DIVIDENDS, PURE_VOL_SURFACE);
      // MARKET_VOLS = getMarketVols(SPOT, EXPIRIES, STRIKES, DISCOUNT_CURVE,
      // DIVIDENDS, PURE_VOL_SURFACE);
      // MARKET_VOLS_FLAT = getMarketVols(SPOT, EXPIRIES, STRIKES, DISCOUNT_CURVE,
      // DIVIDENDS, flat);
    ...
    // /**
    // * This is really just testing whether we can recover the flat pure implied
    // volatility surface from the option prices - the code paths after that point
    // are identical
    // */
    // @Test
    // public void flatSurfaceTest() {
    // EquityVarianceSwapPricer pricer = new EquityVarianceSwapPricer();
    // EquityVarianceSwapStaticReplication staRep = new
    // EquityVarianceSwapStaticReplication();
    // double[] rvExpected = staRep.expectedVariance(SPOT, DISCOUNT_CURVE,
    // DIVIDENDS, EVS.getTimeToObsEnd(), new
    // PureImpliedVolatilitySurface(ConstantDoublesSurface.from(PURE_VOL)));
    // @SuppressWarnings("unused")
    // double[] noDivRVExpected = staRep.expectedVariance(SPOT, DISCOUNT_CURVE,
    // NULL_DIVIDENDS, EVS.getTimeToObsEnd(), new
    // PureImpliedVolatilitySurface(ConstantDoublesSurface.from(PURE_VOL)));
    //
    // double rv1 = pricer.priceFromOTMPrices(EVS_COR_FRO_DIVS, SPOT,
    // DISCOUNT_CURVE, DIVIDENDS, EXPIRIES, STRIKES, OTM_PRICES_FLAT);
    // double rv2 = pricer.priceFromOTMPrices(EVS, SPOT, DISCOUNT_CURVE,
    // DIVIDENDS, EXPIRIES, STRIKES, OTM_PRICES_FLAT);
    // double rv1PDE = pricer.priceFromLocalVol(EVS_COR_FRO_DIVS, SPOT,
    // DISCOUNT_CURVE, DIVIDENDS, MARKET_VOLS_FLAT);
    // double rv2PDE = pricer.priceFromLocalVol(EVS, SPOT, DISCOUNT_CURVE,
    // DIVIDENDS, MARKET_VOLS_FLAT);
    // @SuppressWarnings("unused")
    // double noDivRV = pricer.priceFromOTMPrices(EVS, SPOT, DISCOUNT_CURVE,
    // NULL_DIVIDENDS, EXPIRIES, STRIKES, OTM_PRICES_FLAT);
    //
    // if (PRINT) {
    ...
    // //we loose a lot of accuracy going via the forward PDE, but it is hardly
    // terrible
    // //TODO investigate the source of this - it is most likely because of crude
    // integration that is done, rather than the PDE solver per se.
    // assertEquals("div corrected PDE", rvExpected[0], rv1PDE, 1e-5);
    // assertEquals("div uncorrected PDE", rvExpected[1], rv2PDE, 1e-5);
    //
    // //in this case the reconstructed implied volatility surface (which is
    // identical to the pure one as no dividends are assumed) is NOT flat so we
    // would not expect the items
    // //below to be equal
    ...
    // EquityVarianceSwapStaticReplication staRep = new
    // EquityVarianceSwapStaticReplication();
    // double[] rv = staRep.expectedVariance(SPOT, DISCOUNT_CURVE, DIVIDENDS,
    // EVS.getTimeToObsEnd(), PURE_VOL_SURFACE);
    //
    // EquityVarianceSwapPricer pricer = new EquityVarianceSwapPricer();
    // double rv1 = pricer.priceFromOTMPrices(EVS_COR_FRO_DIVS, SPOT,
    // DISCOUNT_CURVE, DIVIDENDS, EXPIRIES, STRIKES, OTM_PRICES);
    // double rv2 = pricer.priceFromOTMPrices(EVS, SPOT, DISCOUNT_CURVE,
    // DIVIDENDS, EXPIRIES, STRIKES, OTM_PRICES);
    // double rv3 = pricer.priceFromOTMPrices(EVS, SPOT, DISCOUNT_CURVE,
    // NULL_DIVIDENDS, EXPIRIES, STRIKES, OTM_PRICES);
    //
    // double rv1PDE = pricer.priceFromLocalVol(EVS_COR_FRO_DIVS, SPOT,
    // DISCOUNT_CURVE, DIVIDENDS, MARKET_VOLS);
    // double rv2PDE = pricer.priceFromLocalVol(EVS, SPOT, DISCOUNT_CURVE,
    // DIVIDENDS,
    //
    // if (PRINT) {
    ...
    //
    // /*
    // * Starting from a finite number of market prices (albeit synthetically
    // generated in this case), once cannot reproduce the entire (hypothetical)
    // implied volatility
    // * surface, especially at extreme strikes where there is no market
    // information. Since expected variance depends on the entire smile
    // (theoretically from zero to infinite strike)
    // * one cannot hope to make too accurate estimate from the prices of liquid
    // European options.
    // */
    // assertEquals("div corrected", rv[0], rv1, 5e-3);
    // assertEquals("div uncorrected", rv[1], rv2, 5e-3);
    //
    // // However these should agree better, as they are just two different
    // numerical schemes using the same date
    ...
    // /**
    // * Compute actual option (i.e. as would be observed in the market) from a
    // hypothetical <b>pure</b> implied volatility surface and dividend
    // assumptions
    ...
    // */
    // private static double[][] getOptionPrices(final double spot, final double[]
    // expiries, final double[][] strikes, final YieldAndDiscountCurve
    // discountCurve,
    // final AffineDividends dividends, final PureImpliedVolatilitySurface surf) {
    // final int nExp = expiries.length;
    // final double[][] prices = new double[nExp][];
    // final EquityDividendsCurvesBundle divCurves = new
    // EquityDividendsCurvesBundle(spot, discountCurve, dividends);
    ...
    // prices[i][j] = p * (f - d) * BlackFormulaRepository.price(1.0, x, t, vol,
    // isCall);
    // }
    // }
    // return prices;
    ...
    private static SmileSurfaceDataBundle getMarketVols(final double spot, final double[] expiries, final double[][] strikes,
        final YieldAndDiscountCurve discountCurve, final AffineDividends dividends, final PureImpliedVolatilitySurface surf) {
      final int nExp = expiries.length;
    ...
    return new StandardSmileSurfaceDataBundle(spot, f, expiries, strikes, vols, NamedInterpolator1dFactory.of("DoubleQuadratic"));
    }
