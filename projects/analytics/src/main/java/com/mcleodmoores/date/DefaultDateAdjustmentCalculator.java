...
/**
 * Adjusts a date by adding the number of spot days and then adding a tenor to the date. If this date is a holiday, the business day convention is used to
 * adjust to the appropriate business day (e.g. the preceding business day if the convention is
 * {@link com.opengamma.financial.convention.businessday.PrecedingBusinessDayConvention}.
 */
public final class DefaultDateAdjustmentCalculator implements TenorOffsetDateAdjustmentCalculator {
  ...
  public LocalDate getSettlementDate(final LocalDate date, final Tenor tenor, final BusinessDayConvention convention,
    ...
    final LocalDate endDate = TenorUtils.adjustDateByTenor(date, tenor, workingDayCalendar, 0);
    if (tenor.isBusinessDayTenor()) {
      // ON, T/N, S/N
      return endDate;
    }
    return convention.adjustDate(CalendarAdapter.of(workingDayCalendar), endDate);
  }
