...
/**
 * Interface specific to commodity curves. Compose the MulticurveProviderInterface.
 */
public interface CommodityProviderInterface extends ParameterCommodityProviderInterface {
  ...
  /**
   * Returns an unmodifiable sorted set of the names of all curves (discounting, forward, price index and issuers). If there are no curves, an empty set is
   * returned.
   * 
   * @return The names.
   ...
   * @param pointSensitivity
   *          The nodal point sensitivities
  ...
  /**
   * Gets the underlying name(s) (i.e. {@link YieldAndDiscountCurve#getName()} for a curve name; this can be multi-valued in the case of spread curves.
   * 
   * @param name
   *          The curve name
  ...
  /**
   * Gets the discount factor for one currency at a given time to maturity. TODO: extend it to a more general unique reference to include issuer/currency
   * curves? UniqueIdentifiable?
   * 
   * @param ccy
   *          The currency.
   * @param time
  ...
  /**
   * Gets the forward for one Ibor index between start and end times. TODO: Do we want to have a unique method for IborIndex and IndexON? UniqueIdentifiable?
   * 
   * @param index
   *          The Ibor index.
   * @param startTime
