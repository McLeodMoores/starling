...
public abstract class VolatilityFunctionProvider<T extends SmileModelData> {
  ...
  public Function1D<T, double[]> getVolatilityFunction(final double forward, final double[] strikes, final double timeToExpiry) {
    ...
    return new Function1D<T, double[]>() {
      @Override
      public double[] apply(final T data) {
   ...
   * @param timeToExpiry
   *          Time-toExpiry
   * @return The first column is is vol at each strike, the second and third are the forward and strike sensitivity at each strike, and the remaining columns
   *         are the model parameter sensitivities are each strike
   */
  ...
  protected Function1D<T, double[][]> getVolatilityAdjointFunctionByCallingSingleStrikes(final double forward, final double[] strikes,
   ...
   * @param timeToExpiry
   *          Time-toExpiry
   * @return Matrix (i.e. double[][]) of volatility sensitivities to model parameters, where the columns are the model parameter sensitivities at each strike
   */
  ...
  private double forwardBar(final EuropeanVanillaOption option, final double forward, final T data) {
    ...
    return (funcUp.apply(data) - funcDown.apply(data)) / 2 / EPS;
  }
  ...
  private double[] forwardBar(final double[] strikes, final double timeToExpiry, final double forward, final T data) {
    ...
    final double[] res = new double[n];
    final double[] up = funcUp.apply(data);
    final double[] down = funcDown.apply(data);
    for (int i = 0; i < n; i++) {
   ...
   *
   * @param func
   *          Function that gives the volatility for a set of model parameters
   * @param data
   *          Model parameters
   * @param index
  ...
  private double paramBar(final Function1D<T, Double> func, final T data, final int index) {
    ...
    if (data.isAllowed(index, down)) {
      if (data.isAllowed(index, up)) {
        ...
        return (func.apply(dUp) - func.apply(dDown)) / 2 / EPS;
      }
      final T dDown = (T) data.with(index, down);
  ...
  private double[] paramBarSet(final Function1D<T, double[]> func, final T data, final int index) {
    ...
    if (data.isAllowed(index, down)) {
      if (data.isAllowed(index, up)) {
        final T dUp = (T) data.with(index, up);
        final T dDown = (T) data.with(index, down);
        final double[] rUp = func.apply(dUp);
        final double[] rDown = func.apply(dDown);
      ...
      final double[] rMid = func.apply(data);
      final double[] rDown = func.apply((T) data.with(index, down));
    ...
    ArgumentChecker.isTrue(data.isAllowed(index, up), "No values and index {} = {} are allowed", index, mid);
    final double[] rMid = func.apply(data);
    final double[] rUp = func.apply((T) data.with(index, up));
    final int m = rMid.length;
    final double[] res = new double[m];
    for (int i = 0; i < m; i++) {
