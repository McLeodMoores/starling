...
public class BlackIndexOptionPricer {
   ...
   * @param indexCoupon
   *          The index coupon
   * @param defaultAdjustedFwdSpread
   *          The default-adjusted forward spread (if not given extraneously, use {@link CDSIndexCalculator#defaultAdjustedForwardSpread} to calculate)
   * @param pvFwdAnnuity
   *          The present value of the forward annuity of the underlying index (if not given extraneously, use {@link CDSIndexCalculator#indexAnnuity} to
   *          calculate this)
   */
  public BlackIndexOptionPricer(final CDSAnalytic fwdCDS, final double timeToExpiry, final ISDACompliantYieldCurve yieldCurve, final double indexCoupon,
      final double defaultAdjustedFwdSpread,
   ...
   *          The index coupon
   * @param intrinsicData
   *          credit curves, weights and recovery rates of the intrinsic names. Usually these would have first been adjusted to match index prices (using
   *          {@link PortfolioSwapAdjustment#adjustCurves})
   */
  public BlackIndexOptionPricer(final CDSAnalytic fwdCDS, final double timeToExpiry, final ISDACompliantYieldCurve yieldCurve, final double indexCoupon,
  ...
  private BlackIndexOptionPricer(final CDSAnalytic fwdCDS, final double timeToExpiry, final ISDACompliantYieldCurve yieldCurve, final double indexCoupon,
      final double[] fwdSpreadAndAnnity) {
    ...
    if (defaultAdjustedFwdSpread > 10) {
      LOGGER.warn("defaultAdjustedFwdSpread should be given as a fraction; a value of " + defaultAdjustedFwdSpread + " is " + defaultAdjustedFwdSpread * 1e4
          + "basis points.");
    }
    ArgumentChecker.isTrue(pvFwdAnnuity > 0, "pvFwdAnnuity must be positive");
    ArgumentChecker.isTrue(pvFwdAnnuity < fwdCDS.getProtectionEnd() * 1.1,
        "Value of annuity of {} is greater than length (in years) of forward CDS. Annuity should be given for unit notional",
        pvFwdAnnuity);
   ...
   * @param vol
   *          The volatility of the default-adjusted forward spread
   * @param isPayer
  ...
  public double getOptionPriceForPriceQuotedIndex(final double gK, final double vol, final boolean isPayer) {
    ArgumentChecker.isTrue(gK >= _minExercisePrice && gK < _maxExercisePrice, "The exercise price must be in the range {} to {} - value of {} is outside this",
        _minExercisePrice, _maxExercisePrice,
        gK);
