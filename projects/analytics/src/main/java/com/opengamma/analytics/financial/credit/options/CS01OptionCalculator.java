...
public class CS01OptionCalculator {
  ...
  public double indexCurveApprox(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic pillarCDS, final CDSQuoteConvention quote,
    ...
    return indexCurveApprox(fwdCDS, timeToExpiry, new CDSAnalytic[] { pillarCDS }, new CDSQuoteConvention[] { quote }, indexCoupon, yieldCurve, strike, vol,
        isPayer, bumpAmount);
  }

  public double indexCurveApprox(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic[] pillarCDS, final CDSQuoteConvention[] quotes,
      final double indexCoupon,
      final ISDACompliantYieldCurve yieldCurve, final IndexOptionStrike strike, final double vol, final boolean isPayer, final double bumpAmount) {
  ...
  /**
   * Calculation of the (parallel) CS01 of an index option, using the intrinsic value of the index: intrinsic credit curves are passed in (via intriniscData -
   * usually these would have been adjusted to reprice the index), and these are used to compute the PUF of the index (given by indexCDX); the (implied) quoted
   * spread of the index is then bumped up (by bumpAmount - normally 1bps, 1e-4) and the intrinsic curves (re)adjusted to match the bumped PUF. With these two
   * sets of curves, we calculate two values of ATM forward price, and these two values are use to compute two option prices (using Pedersen model). The
   * difference (divided by the bumpAmount) is returned.
   * <p>
   ...
   * @param bumpAmount
   *          The bump amount. Use 1e-4 for standard CS01
   * @return The difference in option price divided by the bumpAmount.
   */
  public double fullCal(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic indexCDX, final double indexCoupon,
  ...
  /**
   * Calculation of the (parallel) CS01 of an index option, using the intrinsic value of the index: intrinsic credit curves are passed in (via intriniscData -
   * usually these would have been adjusted to reprice the index), and these are used to compute the PUF for pillar points (given by indexCDX); the (implied)
   * quoted spreads of these pillars is then bumped up (by bumpAmount - normally 1bps, 1e-4) and the intrinsic curves (re)adjusted to match the bumped PUF. With
   * these two sets of curves, we calculate two values of ATM forward price, and these two values are use to compute two option prices (using Pedersen model).
   * The difference (divided by the bumpAmount) is returned.
   * <p>
   * For standard CS01 use a bumpAmount of 1e-4 and multiple the result by the notional times 1e-4.
   * 
   * @param fwdCDS
   ...
   *          time to expiry of the option
   * @param indexCDX
   *          The spot CDS that represent the `pillars'
   * @param indexCoupon
   *          The index coupon
   * @param yieldCurve
   *          The current yield curve
   ...
   *          The option strike. This can be either given as the exercise price directly (ExerciseAmount) or as a spread (SpreadBasedStrike)
   * @param vol
   *          The log-normal volatility of the flat () spread
   * @param isPayer
   *          true for payer and false for receiver option
   * @param bumpAmount
   *          The bump amount. Use 1e-4 for standard CS01
