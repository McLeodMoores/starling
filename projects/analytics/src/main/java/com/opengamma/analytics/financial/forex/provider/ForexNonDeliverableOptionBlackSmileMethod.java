...
public final class ForexNonDeliverableOptionBlackSmileMethod {
   ...
   */
  public MultipleCurrencyAmount currencyExposure(final ForexNonDeliverableOption option,
    ...
    currencyExposure[0] = CurrencyAmount.of(option.getCurrency1(),
        deltaSpot * Math.abs(option.getUnderlyingNDF().getNotionalCurrency1()) * sign);
   ...
   *
   * @param option
   *          the non-deliverable option, not null
   * @param curves
   *          the curve data, not null
   ...
   * @param directQuote
   *          true if the delta with respect to the quoted spot is required, false if the delta with respect to the inverse spot is required
   * @return the delta
   */
  public CurrencyAmount delta(final ForexNonDeliverableOption option, final BlackForexSmileProviderInterface marketData,
  ...
  /**
   * Gets the theoretical relative delta of a non-deliverable FX option. For an option where the strike is in the same units as the spot
   * rate (a direct quote), this is the same as the delta. For an option where the strike is the inverse of the spot rate (an indirect
   * quote), this gives the delta with respect to this reversed quote.
   *
   * @param option
   *          the option, not null
   * @param marketData
   ...
   */
  public double theoreticalRelativeDelta(final ForexNonDeliverableOption option, final BlackForexSmileProviderInterface marketData,
      final boolean directQuote) {
    ArgumentChecker.notNull(option, "option");
    ArgumentChecker.notNull(marketData, "marketData");
    final MulticurveProviderInterface curves = marketData.getMulticurveProvider();
   ...
   * @return the relative delta
   */
  public double relativeDelta(final ForexNonDeliverableOption option, final BlackForexSmileProviderInterface marketData,
      final boolean directQuote) {
    ArgumentChecker.notNull(option, "option");
    ArgumentChecker.notNull(marketData, "marketData");
    final MulticurveProviderInterface curves = marketData.getMulticurveProvider();
  ...
  /**
   * Computes the present value volatility sensitivity (sensitivity to one volatility point) of Forex non-deliverable option with the Black
   * function and a volatility surface.
   *
   * @param option
   *          the Forex option, not null
   * @param marketData
  ...
  public PresentValueForexBlackVolatilitySensitivity presentValueBlackVolatilitySensitivity(final ForexNonDeliverableOption option,
    ...
    final double[] priceAdjoint = BLACK_FUNCTION.getPriceAdjoint(vanillaOption, dataBlack);
    // Backward sweep
    final double volatilitySensitivityValue = priceAdjoint[2] * Math.abs(option.getUnderlyingNDF().getNotionalCurrency1())
        * (option.isLong() ? 1.0 : -1.0);
    ...
    final PresentValueForexBlackVolatilitySensitivity sensi = new PresentValueForexBlackVolatilitySensitivity(option.getCurrency1(),
        option.getCurrency2(), result);
    return sensi;
  }

  /**
   * Computes the present value volatility sensitivity (sensitivity to each point of the volatility input grid) of Forex non-deliverable
   ...
   */
  public PresentValueForexBlackVolatilityNodeSensitivityDataBundle presentValueVolatilityNodeSensitivity(
      final ForexNonDeliverableOption option,
      final BlackForexSmileProviderInterface marketData) {
    ArgumentChecker.notNull(option, "option");
    ArgumentChecker.notNull(marketData, "marketData");
    final MulticurveProviderInterface curves = marketData.getMulticurveProvider();
    ...
    final VolatilityAndBucketedSensitivities volAndSensitivities = marketData.getVolatilityAndSensitivities(option.getCurrency1(),
        option.getCurrency2(), expiryTime, strike, forward);
    final double[][] nodeWeight = volAndSensitivities.getBucketedSensitivities();
    final DoublesPair point = DoublesPair.of(expiryTime,
        option.getCurrency1().equals(marketData.getCurrencyPair().getFirst()) ? strike : 1.0 / strike);
