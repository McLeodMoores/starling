...
/**
 * Returns the change in PV01 of an instrument due to a parallel 1bp move of <b>all</b> the curves to which the bond total return swap is
 * sensitive. The value is returned in the currency of the asset.
 */
public final class BondTrsGammaPV01Calculator extends InstrumentDerivativeVisitorAdapter<ParameterIssuerProviderInterface, Double> {
  ...
  private static final InstrumentDerivativeVisitor<ParameterIssuerProviderInterface, Double> INSTANCE = new BondTrsGammaPV01Calculator();
  /**
  ...
  /**
   * Calculates the change in PV01 of an instrument due to a parallel move of each yield curve the instrument is sensitive to, scaled so
   * that the move is 1bp.
   * 
   * @param bondTrs
   *          The instrument, not null
   * @param data
   *          The curve data provider, not null
  ...
  public Double visitBondTotalReturnSwap(final BondTotalReturnSwap bondTrs, final ParameterIssuerProviderInterface data) {
    ...
    for (final Map.Entry<Pair<String, Currency>, Double> entry : pv01.getMap().entrySet()) {
      final Pair<String, Currency> bumpedNameCurrency = Pairs.of(entry.getKey().getFirst() + YieldCurveUtils.PARALLEL_SHIFT_NAME,
  ...
  private static ParameterIssuerProviderInterface getBumpedProvider(final ParameterIssuerProviderInterface data) {
    if (data instanceof IssuerProviderDiscount) {
      ...
      for (final Map.Entry<Currency, YieldAndDiscountCurve> entry : multicurveProvider.getDiscountingCurves().entrySet()) {
        ...
        bumped.getMulticurveProvider().setCurve(entry.getKey(),
            YieldCurveUtils.withParallelShift((YieldCurve) entry.getValue(), BP1, ShiftType.ABSOLUTE));
      }
      ...
      for (final Map.Entry<Pair<Object, LegalEntityFilter<LegalEntity>>, YieldAndDiscountCurve> entry : discount.getIssuerCurves()
          .entrySet()) {
        if (!(entry.getValue() instanceof YieldCurve)) {
          throw new IllegalArgumentException("Can only bump YieldCurves");
        }
