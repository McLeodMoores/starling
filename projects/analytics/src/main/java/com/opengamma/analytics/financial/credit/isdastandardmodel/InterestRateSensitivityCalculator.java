...
public class InterestRateSensitivityCalculator {
   ...
   * @param quote
   *          This can be a QuotedSpread or PointsUpFront. For quoted spread this is taken as invariant; for PUF this is converted to a quoted spread (which is
   *          then invariant). ParSpread is not supported
   * @param yieldCurveBuilder
   *          yield curve builder
   * @param marketRates
   *          the money-market and swap rates (in the correct order for the yield curve builder, i.e. in ascending order of maturity)
   * @return the parallel IR01
   */
  public double parallelIR01(final CDSAnalytic cds, final CDSQuoteConvention quote, final ISDACompliantYieldCurveBuild yieldCurveBuilder,
   ...
   *          analytic description of a CDS traded at a certain time
   * @param coupon
   *          The cds's coupon (as a fraction)
   * @param creditCurve
   ...
   * @return the parallel IR01
   */
  public double parallelIR01(final CDSAnalytic cds, final double coupon, final ISDACompliantCreditCurve creditCurve,
      final ISDACompliantYieldCurveBuild yieldCurveBuilder, final double[] marketRates) {
    ArgumentChecker.notNull(cds, "cds");
   ...
   *
   * Note, this bumps the yield curve not the underlying instrument market data. See methods which take a {@code ISDACompliantYieldCurveBuild} for bumping of
   ...
   */
  public double[] bucketedIR01(final CDSAnalytic cds, final double coupon, final ISDACompliantCreditCurve creditCurve,
      final ISDACompliantYieldCurveBuild yieldCurveBuilder, final double[] marketRates) {
    ArgumentChecker.notNull(cds, "cds");
    ArgumentChecker.notNull(creditCurve, "creditCurve");
    ArgumentChecker.notNull(yieldCurveBuilder, "yieldCurveBuilder");
  ...
  /**
   * The bucketed IR01 (Interest-Rate 01) is by definition the vector of changes in the price of a CDS when the points on the yield curve are bumped.
   *
   * Note, this bumps the yield curve not the underlying instrument market data. See methods which take a {@code ISDACompliantYieldCurveBuild} for bumping of
   * the underlying instruments.
   *
  ...
  private double priceDiff(final CDSAnalytic cds, final ISDACompliantCreditCurve creditCurve, final double coupon, final ISDACompliantYieldCurve yc1,
  ...
  private double[] priceDiff(final CDSAnalytic cds, final ISDACompliantCreditCurve creditCurve, final double coupon, final ISDACompliantYieldCurve[] bumpedYC,
