...
public class HedgeRatioCalculator {
  ...
  public DoubleMatrix1D getCurveSensitivities(final CDSAnalytic cds, final double coupon, final ISDACompliantCreditCurve creditCurve,
  ...
  /**
   * The sensitivity of a set of CDSs to the zero hazard rates at the knots of the credit curve. The element (i,j) is the sensitivity of the PV of the jth CDS
   * to the ith knot.
   *
   * @param cds
   *          Set of CDSs
   * @param coupons
   *          The coupons of the CDSs
   * @param creditCurve
   *          The credit Curve
   * @param yieldCurve
   *          the yield curve
   ...
   */
  public DoubleMatrix2D getCurveSensitivities(final CDSAnalytic[] cds, final double[] coupons, final ISDACompliantCreditCurve creditCurve,
      final ISDACompliantYieldCurve yieldCurve) {
  ...
  /**
   * Hedge a CDS with other CDSs on the same underlying (single-name or index) at different maturities. The hedge is such that the total portfolio (the CDS
   * <b>minus</b> the hedging CDSs, with notionals of the CDS notional times the computed hedge ratios) is insensitive to infinitesimal changes to the the
   * credit curve. <br>
   * Here the credit curve is built using the hedging CDSs as pillars.
   *
   * @param cds
   *          The CDS to be hedged
   * @param coupon
   *          The coupon of the CDS to be hedged
   * @param hedgeCDSs
   *          The CDSs to hedge with - these are also used to build the credit curve
   * @param hedgeCDSCoupons
   *          The coupons of the CDSs to hedge with/build credit curve
   * @param hegdeCDSPUF
   *          The PUF of the CDSs to build credit curve
   * @param yieldCurve
   *          he yield curve
   ...
   */
  public DoubleMatrix1D getHedgeRatios(final CDSAnalytic cds, final double coupon, final CDSAnalytic[] hedgeCDSs, final double[] hedgeCDSCoupons,
      final double[] hegdeCDSPUF, final ISDACompliantYieldCurve yieldCurve) {
   ...
   * Hedge a CDS with other CDSs on the same underlying (single-name or index) at different maturities. The hedge is such that the total portfolio (the CDS
   * <b>minus</b> the hedging CDSs, with notionals of the CDS notional times the computed hedge ratios) is insensitive to infinitesimal changes to the the
   * credit curve. If the number of hedge-CDSs equals the number of credit-curve knots, the system is square and is solved exactly (see below).<br>
   ...
   *          The coupon of the CDS to be hedged
   * @param hedgeCDSs
   *          The CDSs to hedge with
   * @param hedgeCDSCoupons
   *          The coupons of the CDSs to hedge with
   * @param creditCurve
   *          The credit curve
   * @param yieldCurve
   *          the yield curve
   ...
   */
  public DoubleMatrix1D getHedgeRatios(final CDSAnalytic cds, final double coupon, final CDSAnalytic[] hedgeCDSs, final double[] hedgeCDSCoupons,
      final ISDACompliantCreditCurve creditCurve, final ISDACompliantYieldCurve yieldCurve) {
   ...
   * The system may not solve if the maturities if the hedging CDSs and very different from the knot times (i.e. the sensitivity matrix is singular).
   *
   * @param cdsSensitivities
   *          vector of sensitivities of the CDS to the zero hazard rates at the credit curve knots.
   * @param hedgeCDSSensitivities
   *          matrix of sensitivities of the hedging-CDSs to the zero hazard rates at the credit curve knots. The (i,j) element is the sensitivity of the jth
   *          CDS to the ith knot.
   * @return The hedge ratios. Since we use a unit notional, there ratios should be multiplied by -notional to give the hedge notional amounts.
   */
  public DoubleMatrix1D getHedgeRatios(final DoubleMatrix1D cdsSensitivities, final DoubleMatrix2D hedgeCDSSensitivities) {
    ...
    if (nCols == nRows) {
      ...
      final LUDecompositionResult luRes = decomp.apply(hedgeCDSSensitivities);
    ...
    if (nRows < nCols) {
      ...
      throw new IllegalArgumentException(
          "Under-specified. No unique solution exists. There are " + nRows + " curve knots but " + nCols + " hedging instruments.");
    }
    ...
    final LUDecompositionCommons decomp = new LUDecompositionCommons();
    final LUDecompositionResult luRes = decomp.apply(a);
