...
public class CapletStrippingSingleStrikeDirect extends CapletStrippingAbsoluteStrike {
  ...
  private static final Interpolator1D DEFAULT_INTERPOLATOR = NamedInterpolator1dFactory.of(DoubleQuadraticInterpolator1dAdapter.NAME,
  ...
  private LeastSquareResults solveForPrice(final DoubleMatrix1D capPrices, final DoubleMatrix1D sigma, final DoubleMatrix1D start) {
    ...
    final Function<DoubleMatrix1D, DoubleMatrix2D> modelPriceJac = x -> getPricer().vegaFromCapletVols(x.getData());

    return NLLSWP.solve(capPrices, sigma, Function1dAdapter.of(modelPriceFunc), Function1dAdapter.of(modelPriceJac), start, _penalty,
        Function1dAdapter.of(allowed));
  }
  ...
  private LeastSquareResults solveForVol(final DoubleMatrix1D capVols, final DoubleMatrix1D sigma, final DoubleMatrix1D start) {
    ...
    final Function<DoubleMatrix1D, DoubleMatrix2D> modelVolJac = x -> {
      final double[] modelPrices = pricer.priceFromCapletVols(x.getData());
      final double[] modelVols = pricer.impliedVols(modelPrices);
    ...
    return NLLSWP.solve(capVols, sigma, Function1dAdapter.of(modelVolFunc), Function1dAdapter.of(modelVolJac), start, _penalty);
  }
