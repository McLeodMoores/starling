 ...
 * <p>
 * The reference volatility used for Black computation is the second volatility (usually corresponding to the ATM strike).
 */
public final class ForexOptionVanillaVannaVolgaMethod {
   ...
   */
  public MultipleCurrencyAmount presentValue(final ForexOptionVanilla optionForex,
    ...
    final SmileDeltaParameters smileAtTime = smileMulticurves.getSmile(optionForex.getCurrency1(), optionForex.getCurrency2(),
   ...
   */
  public MultipleCurrencyAmount currencyExposure(final ForexOptionVanilla optionForex,
      final BlackForexVannaVolgaProviderInterface smileMulticurves) {
    ArgumentChecker.notNull(optionForex, "Forex option");
    ArgumentChecker.notNull(smileMulticurves, "Smile");
    ArgumentChecker.isTrue(smileMulticurves.checkCurrencies(optionForex.getCurrency1(), optionForex.getCurrency2()),
  ...
  public PresentValueForexBlackVolatilitySensitivity presentValueBlackVolatilitySensitivity(final ForexOptionVanilla optionForex,
    ...
    for (int loopvv = 0; loopvv < 3; loopvv++) {
      ...
      result.add(point,
          vegaReference[loopvv] * Math.abs(optionForex.getUnderlyingForex().getPaymentCurrency1().getAmount())
  ...
  /**
   * Computes the curve sensitivity of the option present value. The sensitivity of the volatility and the weights on the forward (and on
   * the curves) is not taken into account.
   * 
   * @param optionForex
   *          The Forex option.
   * @param smileMulticurves
  ...
  /**
   * Computes the weights used for adjustment in the vanna-volga method. The weight for the second adjustment (corresponding to ATM strike)
   * is not computed as the adjustment itself is 0 for that strike in our implementation.
   * 
   * @param optionForex
   *          The option.
   * @param forward
   *          The forward FX rate.
   * @param dfDomestic
   *          The discounting factor to the payment date in the domestic currency.
   * @param strikesReference
   *          The reference strikes used for the vanna-volga method.
   * @param volatilitiesReference
   *          The volatilities at the reference strikes.
   * @return The weights.
   */
   ...
   *          The volatilities at the reference strikes.
   * @param vega
   *          The vega using the base volatility at the reference points (index 0 and 2) and at the strike (index 1). The array is changed
