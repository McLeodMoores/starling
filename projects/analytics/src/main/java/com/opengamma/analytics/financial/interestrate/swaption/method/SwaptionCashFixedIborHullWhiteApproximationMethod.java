...
/**
 * Method to compute the present value of cash-settled European swaptions with the Hull-White one factor model by a third order approximation. Reference:
 * Henrard, M., Cash-Settled Swaptions: How Wrong are We? (November 2010). Available at SSRN: http://ssrn.com/abstract=1703846
 * 
...
public class SwaptionCashFixedIborHullWhiteApproximationMethod implements PricingMethod {
  ...
  public CurrencyAmount presentValue(final SwaptionCashFixedIbor swaption, final HullWhiteOneFactorPiecewiseConstantDataBundle hwData) {
    ...
    u[2] = (swaption.getStrike() - rate) * (derivativesAnnuity[0] * derivativesRate[1] + derivativesAnnuity[1] * derivativesRate[0] * derivativesRate[0])
        - 2 * derivativesAnnuity[0]
            * derivativesRate[0] * derivativesRate[0]
        - annuity * derivativesRate[1];
    u[3] = -3
        * derivativesRate[0]
        * (derivativesAnnuity[0] * derivativesRate[1] + derivativesAnnuity[1] * derivativesRate[0] * derivativesRate[0])
        ...
        + (swaption.getStrike() - rate)
            * (derivativesAnnuity[0] * derivativesRate[2] + 3 * derivativesAnnuity[1] * derivativesRate[0] * derivativesRate[1]
                + derivativesAnnuity[2] * derivativesRate[0] * derivativesRate[0]
                    * derivativesRate[0])
    ...
    if (!swaption.getUnderlyingSwap().getFixedLeg().isPayer()) {
      pv = (u[0] - u[1] * alpha0tilde + u[2] * (1 + alpha[0] * alpha[0]) / 2.0 - u[3] * (alpha0tilde * alpha0tilde * alpha0tilde + 3.0 * alpha0tilde) / 6.0)
          * NORMAL.getCDF(kappatilde)
          + (-u[1] - u[2] * (-2.0 * alpha0tilde + kappatilde) / 2.0
              + u[3] * (-3 * alpha0tilde * alpha0tilde + 3.0 * kappatilde * alpha0tilde - kappatilde * kappatilde - 2.0) / 6.0)
  ...
  public double[] presentValueHullWhiteSensitivity(final SwaptionCashFixedIbor swaption, final HullWhiteOneFactorPiecewiseConstantDataBundle hwData) {
    ...
    u[3] = -3 * derivativesRate[0] * (derivativesAnnuity[0] * derivativesRate[1] + derivativesAnnuity[1] * derivativesRate[0] * derivativesRate[0])
        ...
        + (swaption.getStrike() - rate)
            * (derivativesAnnuity[0] * derivativesRate[2] + 3 * derivativesAnnuity[1] * derivativesRate[0] * derivativesRate[1] + derivativesAnnuity[2]
                * derivativesRate[0] * derivativesRate[0] * derivativesRate[0])
        - rate * derivativesRate[2];
    final double kappatilde = kappa + alphaIbor[0];
    final double alpha0tilde = alphaIbor[0] + x0;
    double ncdf;
    ...
    if (!swaption.getUnderlyingSwap().getFixedLeg().isPayer()) {
      alpha0tildeBar = ((-u[1] - u[3] * (3 * alpha0tilde * alpha0tilde + 3.0) / 6.0) * ncdf
          + (u[2] + u[3] * (-6.0 * alpha0tilde + 3.0 * kappatilde) / 6.0) * npdf) * pvBar;
      kappatildeBar = ((u[0] - u[1] * alpha0tilde + u[2] * (1 + alpha[0] * alpha[0]) / 2.0
          - u[3] * (alpha0tilde * alpha0tilde * alpha0tilde + 3.0 * alpha0tilde) / 6.0) * npdf
      ...
      uBar[3] = (-(alpha0tilde * alpha0tilde * alpha0tilde + 3.0 * alpha0tilde) / 6.0 * ncdf
          + (-3 * alpha0tilde * alpha0tilde + 3.0 * kappatilde * alpha0tilde - kappatilde * kappatilde - 2.0) / 6.0
              * npdf)
          * pvBar;
    } else {
    ...
    derivativesAnnuityBar[0] = (swaption.getStrike() - rate) * derivativesRate[0] * uBar[1]
        + ((swaption.getStrike() - rate) * derivativesRate[1] - 2.0 * derivativesRate[0] * derivativesRate[0])
            * uBar[2]
        + (-3 * derivativesRate[0] * derivativesRate[1] - 2 * derivativesRate[0] * derivativesRate[1] + (swaption.getStrike() - rate) * derivativesRate[2])
            * uBar[3];
    derivativesAnnuityBar[1] = (swaption.getStrike() - rate) * derivativesRate[0] * derivativesRate[0] * uBar[2]
        + (-3 * derivativesRate[0] * derivativesRate[0] * derivativesRate[0] + (swaption.getStrike() - rate) * 3 * derivativesRate[0] * derivativesRate[1])
            * uBar[3];
    ...
    derivativesRateBar[0] = ((swaption.getStrike() - rate) * derivativesAnnuity[0] - annuity)
        * uBar[1]
        + ((swaption.getStrike() - rate) * (2.0 * derivativesAnnuity[1] * derivativesRate[0]) - 4 * derivativesAnnuity[0] * derivativesRate[0])
            * uBar[2]
        + (-3 * (derivativesAnnuity[0] * derivativesRate[1] + 3.0 * derivativesAnnuity[1] * derivativesRate[0] * derivativesRate[0])
            - 2 * derivativesAnnuity[0] * derivativesRate[1] + (swaption
                .getStrike() - rate) * (3 * derivativesAnnuity[1] * derivativesRate[1] + derivativesAnnuity[2] * 3.0 * derivativesRate[0] * derivativesRate[0]))
            * uBar[3];
    derivativesRateBar[1] = ((swaption.getStrike() - rate) * derivativesAnnuity[0] - annuity) * uBar[2]
        + (-3 * derivativesRate[0] * derivativesAnnuity[0] - 2 * derivativesAnnuity[0] * derivativesRate[0]
            + (swaption.getStrike() - rate) * (3 * derivativesAnnuity[1] * derivativesRate[0]))
            * uBar[3];
    ...
    swapRateAdjointAlpha(x0, discountedCashFlowFixed, alphaFixed, discountedCashFlowIbor, alphaIbor, rateBar, derivativesRateBar, derivativesRate,
        alphaFixedBar, alphaIborBar);
    ...
    for (int loopcf = 0; loopcf < nbFixed; loopcf++) {
      MODEL.alpha(hwData.getHullWhiteParameter(), 0.0, expiryTime, expiryTime,
          swaption.getUnderlyingSwap().getFixedLeg().getNthPayment(loopcf).getPaymentTime(), partialDerivatives);
   ...
   */
  public InterestRateCurveSensitivity presentValueCurveSensitivity(final SwaptionCashFixedIbor swaption,
    ...
    if (!swaption.getUnderlyingSwap().getFixedLeg().isPayer()) {
      ...
      pv = (u[0] - u[1] * alpha0tilde + u[2] * (1 + alpha[0] * alpha[0]) / 2.0 - u[3] * (alpha0tilde * alpha0tilde * alpha0tilde + 3.0 * alpha0tilde) / 6.0)
          * ncdf
          + (-u[1] - u[2] * (-2.0 * alpha0tilde + kappatilde) / 2.0
              + u[3] * (-3 * alpha0tilde * alpha0tilde + 3.0 * kappatilde * alpha0tilde - kappatilde * kappatilde - 2.0) / 6.0) * npdf;
    } else {
      ncdf = NORMAL.getCDF(-kappatilde);
      pv = -(u[0] - u[1] * alpha0tilde + u[2] * (1 + alpha[0] * alpha[0]) / 2.0 - u[3] * (alpha0tilde * alpha0tilde * alpha0tilde + 3.0 * alpha0tilde) / 6.0)
          * ncdf
    ...
    swapRateAdjointDiscountedCF(x0, discountedCashFlowFixed, alphaFixed, discountedCashFlowIbor, alphaIbor, rateBar, derivativesRateBar, derivativesRate,
        discountedCashFlowFixedBar,
        discountedCashFlowIborBar);
    final double[] dfFixedBar = new double[nbFixed];
    final List<DoublesPair> listDf = new ArrayList<>();
    for (int loopcf = 0; loopcf < nbFixed; loopcf++) {
      dfFixedBar[loopcf] = swaption.getUnderlyingSwap().getFixedLeg().getNthPayment(loopcf).getPaymentYearFraction()
          * swaption.getUnderlyingSwap().getFixedLeg().getNthPayment(loopcf).getNotional()
          * discountedCashFlowFixedBar[loopcf];
      final DoublesPair dfSensi = DoublesPair.of(swaption.getUnderlyingSwap().getFixedLeg().getNthPayment(loopcf).getPaymentTime(),
          -swaption.getUnderlyingSwap().getFixedLeg().getNthPayment(loopcf)
              .getPaymentTime()
    ...
    for (int loopcf = 0; loopcf < cfe.getNumberOfPayments(); loopcf++) {
      ...
      final DoublesPair dfSensi = DoublesPair.of(cfeIbor.getNthPayment(loopcf).getPaymentTime(),
          -cfeIbor.getNthPayment(loopcf).getPaymentTime() * dfIbor[loopcf] * dfIborBar[loopcf]);
      listDf.add(dfSensi);
   ...
   *          The bond volatilities.
   * @param derivatives
   *          Array used to return the derivatives of the swap rate with respect to the random variable. The array is changed by the method. The values are [0]
   *          the first order derivative and [1] the second order derivative.
   ...
   *          The bond volatilities.
   * @param swapRateBar
   *          The sensitivity to the swap rate in the rest of the computation.
   * @param derivativesBar
   *          The sensitivity to the swap rate derivatives in the rest of the computation.
   * @param derivatives
   *          Array used to return the derivatives of the swap rate with respect to the random variable. The array is changed by the method. The values are [0]
   *          the first order derivative and [1] the second order derivative.
   * @param alphaFixedBar
   *          Array used to return the derivatives of the result with respect to the alphaFixed variables.
   * @param alphaIborBar
   *          Array used to return the derivatives of the result with respect to the alphaIbor variables.
   * @return The swap rate.
   */
  private double swapRateAdjointAlpha(final double x, final double[] discountedCashFlowFixed, final double[] alphaFixed, final double[] discountedCashFlowIbor,
    ...
    gBar[0] = -f[0] / (g[0] * g[0]) * swapRateBar + (-f[1] / (g[0] * g[0]) + f[0] * g[1] / (g[0] * g[0] * g[0])) * derivativesBar[0]
        + (-f[2] / (g[0] * g[0]) + 2.0 * (f[0] * g[2] + 2 * g[1] * f[1]) / (g[0] * g[0] * g[0]) - 6.0 * f[0] * g[1] * g[1] / (g[0] * g[0] * g[0] * g[0]))
            * derivativesBar[1];
    ...
    for (int loopcf = 0; loopcf < discountedCashFlowFixed.length; loopcf++) {
      ...
      alphaFixedBar[loopcf] = y2p * gBar[0] + (-y2[loopcf] - alphaFixed[loopcf] * y2p) * gBar[1]
          + (2.0 * alphaFixed[loopcf] * y2[loopcf] + alphaFixed[loopcf] * alphaFixed[loopcf] * y2p) * gBar[2];
    }
    final double[] fBar = new double[3];
    fBar[0] = 1.0 / g[0] * swapRateBar - g[1] / (g[0] * g[0]) * derivativesBar[0]
        + (-g[2] / (g[0] * g[0]) + 2 * g[1] * g[1] / (g[0] * g[0] * g[0])) * derivativesBar[1];
    ...
    for (int loopcf = 0; loopcf < discountedCashFlowIbor.length; loopcf++) {
      ...
      alphaIborBar[loopcf] = y1p * fBar[0] + (-y1[loopcf] - alphaIbor[loopcf] * y1p) * fBar[1]
          + (2 * alphaIbor[loopcf] * y1[loopcf] + alphaIbor[loopcf] * alphaIbor[loopcf] * y1p) * fBar[2];
    }
    return swapRate;
  }

  private double swapRateAdjointDiscountedCF(final double x, final double[] discountedCashFlowFixed, final double[] alphaFixed,
      final double[] discountedCashFlowIbor, final double[] alphaIbor,
      final double swapRateBar, final double[] derivativesBar, final double[] derivatives, final double[] discountedCashFlowFixedBar,
      final double[] discountedCashFlowIborBar) {
    ...
    gBar[2] = -f[0] / (g[0] * g[0]) * derivativesBar[1];
    for (int loopcf = 0; loopcf < discountedCashFlowFixed.length; loopcf++) {
      discountedCashFlowFixedBar[loopcf] = y2[loopcf] * gBar[0] + -alphaFixed[loopcf] * y2[loopcf] * gBar[1]
          + alphaFixed[loopcf] * alphaFixed[loopcf] * y2[loopcf] * gBar[2];
    }
    final double[] fBar = new double[3];
    fBar[0] = 1.0 / g[0] * swapRateBar - g[1] / (g[0] * g[0]) * derivativesBar[0]
        + (-g[2] / (g[0] * g[0]) + 2 * g[1] * g[1] / (g[0] * g[0] * g[0])) * derivativesBar[1];
    ...
    fBar[2] = 1.0 / g[0] * derivativesBar[1];
    for (int loopcf = 0; loopcf < discountedCashFlowIbor.length; loopcf++) {
      discountedCashFlowIborBar[loopcf] = y1[loopcf] * fBar[0] + -alphaIbor[loopcf] * y1[loopcf] * fBar[1]
          + alphaIbor[loopcf] * alphaIbor[loopcf] * y1[loopcf] * fBar[2];
    }
    return swapRate;
  }
   ...
   *          The number of fixed payment per year.
   * @param nbFixedPeriod
   *          The total number of payments.
   * @param derivatives
   *          Array used to return the derivatives of the annuity with respect to the swap rate. The array is changed by the method. The values are [0] the
   *          first order derivative, [1] the second order derivative and [2] the third order derivative.
