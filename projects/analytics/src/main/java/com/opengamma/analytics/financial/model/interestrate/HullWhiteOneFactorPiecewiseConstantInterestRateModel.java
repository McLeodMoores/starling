...
public class HullWhiteOneFactorPiecewiseConstantInterestRateModel {
  ...
  public double futuresConvexityFactor(final HullWhiteOneFactorPiecewiseConstantParameters data, final double t0, final double t1, final double t2) {
    ...
    for (int loopperiod = 0; loopperiod < indexT0; loopperiod++) {
      factor2 += data.getVolatility()[loopperiod] * data.getVolatility()[loopperiod]
   ...
   *          The end expiry time.
   * @param u
   *          The fixing period start time.
   * @param v
   *          The fixing period end time.
   * @param tp
   *          The payment time.
   * @return The factor.
   */
  public double paymentDelayConvexityFactor(final HullWhiteOneFactorPiecewiseConstantParameters parameters, final double startExpiry, final double endExpiry,
    ...
    for (int loopperiod = 0; loopperiod < sLen; loopperiod++) {
      factor2 += parameters.getVolatility()[loopperiod + indexStart - 1] * parameters.getVolatility()[loopperiod + indexStart - 1]
   ...
   * @param startExpiry
   *          Start time of the expiry period.
   * @param endExpiry
   *          End time of the expiry period.
   * @param numeraireTime
   *          Time to maturity for the bond numeraire.
   * @param bondMaturity
   ...
   * @param alpha
   *          The zero-coupon bond volatilities.
  ...
  public double[] swapRateDai1(final double x, final double[] discountedCashFlowFixed, final double[] alphaFixed, final double[] discountedCashFlowIbor,
    ...
    for (int loopcf = 0; loopcf < nbDcfi; loopcf++) {
      swapRateDai1[loopcf] = discountedCashFlowIbor[loopcf] * Math.exp(-alphaIbor[loopcf] * x - 0.5 * alphaIbor[loopcf] * alphaIbor[loopcf])
          * (x + alphaIbor[loopcf]) / denominator;
    }
    return swapRateDai1;
  }
  ...
  public Pair<double[], double[]> swapRateDx2Da1(final double x, final double[] discountedCashFlowFixed, final double[] alphaFixed,
    ...
    for (int loopcf = 0; loopcf < nbDcff; loopcf++) {
      termFixedBar[loopcf] = gBar - alphaFixed[loopcf] * dgBar + alphaFixed[loopcf] * alphaFixed[loopcf] * dg2Bar;
      alphaFixedBar[loopcf] = termFixed[loopcf] * (-x - alphaFixed[loopcf]) * termFixedBar[loopcf] - termFixed[loopcf] * dgBar
          + 2 * alphaFixed[loopcf] * termFixed[loopcf] * dg2Bar;
    }
    final double[] alphaIborBar = new double[nbDcfi];
    final double[] termIborBar = new double[nbDcfi];
    for (int loopcf = 0; loopcf < nbDcfi; loopcf++) {
      termIborBar[loopcf] = fBar - alphaIbor[loopcf] * dfBar + alphaIbor[loopcf] * alphaIbor[loopcf] * df2Bar;
      alphaIborBar[loopcf] = termIbor[loopcf] * (-x - alphaIbor[loopcf]) * termIborBar[loopcf] - termIbor[loopcf] * dfBar
          + 2 * alphaIbor[loopcf] * termIbor[loopcf] * df2Bar;
    }
