 ...
 * <p>
 * Computed as the average daily variance of log returns, scaled by an annualization factor, an estimate of the number of business days per
 * year
 * <p>
 ...
 * observations may be less than number expected, due to unforeseen market disruptions. To account for this, the sum is normalized by
 * nObsExpected (&ge; nObsActual) The realized variance calculated in this class do not perform this normalization. See
 * {@link VarianceSwapStaticReplication#presentValue} for an example of this normalization.
 */
public class RealizedVariance implements Function<VarianceSwap, Double> {
  ...
  public Double apply(final VarianceSwap swap) {
    ...
    for (int i = 1; i < nObs; i++) {
      ArgumentChecker.isTrue(Double.compare(obs[i], 0.0) != 0,
          "Encountered an invalid observation of zero in VarianceSwap at {}'th observation. "
