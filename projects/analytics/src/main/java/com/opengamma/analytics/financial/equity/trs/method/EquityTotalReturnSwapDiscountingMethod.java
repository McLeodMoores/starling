...
public final class EquityTotalReturnSwapDiscountingMethod {
  ...
  public MultipleCurrencyAmount presentValue(final EquityTotalReturnSwap trs, final EquityTrsDataBundle equityMulticurves) {
    ...
    if (trs.getFundingLeg().getNumberOfPayments() == 1
        || trs.getFundingLeg().getNthPayment(0) instanceof CouponIbor
        || trs.getFundingLeg().getNthPayment(0) instanceof CouponIborSpread
        || trs.getFundingLeg().getNthPayment(0) instanceof CouponON
        || trs.getFundingLeg().getNthPayment(0) instanceof CouponONSpread) {
      final MultipleCurrencyAmount previousFixingPv = MultipleCurrencyAmount.of(trs.getNotionalCurrency(),
          -trs.getNotionalAmount()
          * equityMulticurves.getCurves().getDiscountFactor(trs.getNotionalCurrency(), trs.getFundingLeg().getNthPayment(0).getPaymentTime()));
    ...
    if (trs.getFundingLeg().getNthPayment(1) instanceof CouponFixed
      ...
      final MultipleCurrencyAmount previousFixingPv = MultipleCurrencyAmount.of(trs.getNotionalCurrency(),
          -trs.getNotionalAmount()
          * equityMulticurves.getCurves().getDiscountFactor(trs.getNotionalCurrency(), trs.getFundingLeg().getNthPayment(1).getPaymentTime()));
      ...
      return previousFixingPv.plus(
          equityMulticurves.getCurves().getFxRates().convert(equityPv, trs.getNotionalCurrency())).plus(fundingLegPv0).plus(fundingLegPv1);
    }
  ...
  public MultipleCurrencyMulticurveSensitivity presentValueCurveSensitivity(final EquityTotalReturnSwap trs, final EquityTrsDataBundle equityMulticurves) {
    ArgumentChecker.notNull(trs, "equity TRS");
    ArgumentChecker.notNull(equityMulticurves, "multi-curve provider with equity price");
    final MulticurveProviderInterface multicurve = equityMulticurves.getCurves();
    final Currency notionalCurrency = trs.getNotionalCurrency();
    // First coupon ON or Ibor: Payment up to the fist payment taken into account OR Only one payment left
    if (trs.getFundingLeg().getNumberOfPayments() == 1
      ...
      final DoublesPair s = DoublesPair.of(time,
          time * trs.getNotionalAmount() * equityMulticurves.getCurves().getDiscountFactor(trs.getNotionalCurrency(), time));
      ...
      final MultipleCurrencyMulticurveSensitivity fundingLegPvcs = trs.getFundingLeg().getNthPayment(0).accept(PVCSDC, equityMulticurves.getCurves());
    ...
    if (trs.getFundingLeg().getNthPayment(1) instanceof CouponFixed
      ...
      MultipleCurrencyMulticurveSensitivity equityLegPvcs = new MultipleCurrencyMulticurveSensitivity();
      equityLegPvcs = equityLegPvcs.plus(notionalCurrency, MulticurveSensitivity.ofYieldDiscounting(mapDsc));
      final MultipleCurrencyMulticurveSensitivity fundingLegPvcs0 = trs.getFundingLeg().getNthPayment(0).accept(PVCSDC, equityMulticurves.getCurves());
      final MultipleCurrencyMulticurveSensitivity fundingLegPvcs1 = trs.getFundingLeg().getNthPayment(1).accept(PVCSDC, equityMulticurves.getCurves());
