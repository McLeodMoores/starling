...
/**
 *
 * The formula is valid for odd number of steps. The even case is NOT implemented (Since formula for even number of steps has the convergence of order -1, there
 * is no advantage to use Leisen-Reimer.)
 */
public class LeisenReimerLatticeSpecification extends LatticeSpecification {
  ...
  public double[] getParameters(final double spot, final double strike, final double timeToExpiry, final double volatility, final double interestRate,
    ...
    Validate.isTrue(nSteps % 2 != 0, "The number of steps should be odd");
