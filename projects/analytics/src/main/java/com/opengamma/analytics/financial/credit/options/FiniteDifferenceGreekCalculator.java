...
public class FiniteDifferenceGreekCalculator {
  ...
  /**
   * The spot Delta of an index option. This defined as the sensitivity of the option price to the price of underlying index (any defaults from the index reduce
   ...
   * @param indexCDX
   *          The spot CDS that represent the index
   * @param indexCoupon
   *          The index coupon
   * @param yieldCurve
   *          The current yield curve
   * @param intrinsicData
   *          credit curves, weights and recovery rates of the intrinsic names
   * @param strike
   ...
   * @param vol
   *          The log-normal volatility of the flat (pseudo) spread
   * @param isPayer
   *          true for payer and false for receiver option
   * @param bumpAmount
   *          The bump amount
   * @param type
   ...
   */
  public double delta(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic indexCDX, final double indexCoupon,
      ...
      final FiniteDifferenceType type) {
    ...
    ArgumentChecker.isTrue(Math.abs(bumpAmount) < BUMP_MAX, "Bump given as {}, but must have abs value less than {}. Bump must be given as fraction",
        bumpAmount, BUMP_MAX);
  ...
  private double optPrice(final double timeToExpiry, final CDSAnalytic indexCDX, final double indexCoupon, final ISDACompliantYieldCurve yieldCurve,
      ...
      final IndexOptionStrike strike, final double vol, final boolean isPayer, final double puf, final CDSAnalytic fwdStartingCDS,
      final IndexOptionPricer pricer) {
    final IntrinsicIndexDataBundle adjCurves = PSA.adjustCurves(puf, indexCDX, indexCoupon, yieldCurve, intrinsicData);
    final double atmFwd = INDEX_CAL.defaultAdjustedForwardIndexValue(fwdStartingCDS, timeToExpiry, yieldCurve, indexCoupon, adjCurves);
    return pricer.getOptionPremium(atmFwd, vol, strike, isPayer);
  }
   ...
   */
  public double deltaByCS01(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic indexCDX, final CDSQuoteConvention quote,
   ...
   *          true for payer and false for receiver option
   * @param bumpAmount
   *          The bump amount (normally 1bp, 1e-4)
   * @return the (CS01 ratio) delta
   */
  public double deltaByCS01(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic indexCDX, final CDSQuoteConvention quote,
      final double indexCoupon,
  ...
  /**
   * The spot Gamma of an index option. This defined as the sensitivity of the option delta to the price of underlying index, which is the second-order
   ...
   */
  public double gamma(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic indexCDX, final double indexCoupon,
      final ISDACompliantYieldCurve yieldCurve,
      final IntrinsicIndexDataBundle intrinsicData, final IndexOptionStrike strike, final double vol, final boolean isPayer, final double bumpAmount) {
   ...
   */
  public double gammaByCS01(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic indexCDX, final CDSQuoteConvention quote,
      final double indexCoupon,
      final ISDACompliantYieldCurve yieldCurve, final IndexOptionStrike strike, final double vol, final boolean isPayer) {
   ...
   *          true for payer and false for receiver option
   * @param bumpAmount
   *          The bump amount The bump amount to use in the Delta (CS01 ratio) calculation (normally 1bp, 1e-4)
   * @param largeBumpAmount
   *          The bump in quoted spread to calculate the new delta at (normally 10bp, 1e-3)
   * @return the Gamma as a Delta difference
   */
  public double gammaByCS01(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic indexCDX, final CDSQuoteConvention quote,
      final double indexCoupon,
      final ISDACompliantYieldCurve yieldCurve, final IndexOptionStrike strike, final double vol, final boolean isPayer, final double bumpAmount,
      final double largeBumpAmount) {
   ...
   */
  public double vega(final double atmFwdPrice, final CDSAnalytic fwdCDS, final double timeToExpiry, final double indexCoupon,
      final ISDACompliantYieldCurve yieldCurve, final IndexOptionStrike strike,
  ...
  /**
   * Theta is the sensitivity of the option price to calendar time; defined this way the Theta of a payer option is always negative. It is calculated by
   * computing an expected option price some time step ahead (normally one day), assuming the ATM forward price remains the same (i.e. just using its Martingale
   * property) and the yield curve takes its expected value.
   * <p>
   ...
   *          true for payer and false for receiver option
   * @param timeStep
   *          The time step (1./365 is normally used)
   * @return The option theta
   */
  public double thetaWithDefault(final double atmFwdPrice, final CDSAnalytic fwdCDS, final double timeToExpiry, final double indexCoupon,
      final ISDACompliantYieldCurve yieldCurve,
   ...
   * @return The option theta
   */
  public double thetaWithDefault(final CDSAnalytic fwdCDS, final double timeToExpiry, final double indexCoupon, final ISDACompliantYieldCurve yieldCurve,
      final IntrinsicIndexDataBundle intrinsicData,
      final IndexOptionStrike strike, final double vol, final boolean isPayer, final double timeStep) {
    final CDSAnalytic fwdStartingCDS = fwdCDS.withOffset(timeToExpiry);
  ...
  /**
   * Theta is the sensitivity of the option price to calendar time; defined this way the Theta of a payer option is always negative. It is calculated by
   * computing an expected option price some time step ahead (normally one day), assuming <b>no defaults occur</b>, and the credit &amp; yield curves take their
   * expected values.
   * <p>
  ...
  /**
   * The irDV01 (interest rate DV01) is the change in the price of an option when the market rates of the instruments used to build the yield curve are
   ...
   * @param indexCoupon
   *          the index coupon
   * @param yieldCurveBuilder
   *          A yield curve builder - takes rates (of market instruments) and produces a yield curve
   * @param irRates
   *          The market rates of the instruments used to build the yield curve
   * @param intrinsicData
   *          credit curves, weights and recovery rates of the intrinsic names
   * @param strike
   *          The option strike. This can be either given as the exercise price directly (ExerciseAmount) or as a spread (SpreadBasedStrike)
   ...
   *          true for payer and false for receiver option
   * @param bumpAmount
   *          The bump amount (normally 1bp)
   * @return The irDV01
   */
  public double irDV01(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic indexCDX, final double indexCoupon,
      final ISDACompliantYieldCurveBuild yieldCurveBuilder,
      final double[] irRates, final IntrinsicIndexDataBundle intrinsicData, final IndexOptionStrike strike, final double vol, final boolean isPayer,
  ...
  public double irDV01(final CDSAnalytic fwdCDS, final double timeToExpiry, final CDSAnalytic indexCDX, final double indexCoupon, final double indexPUF,
      final ISDACompliantYieldCurveBuild yieldCurveBuilder, final double[] irRates, final IndexOptionStrike strike, final double vol, final boolean isPayer,
      final double bumpAmount) {
  ...
  private double atmFwdBump(final double timeToExpiry, final double indexCoupon, final ISDACompliantYieldCurve yieldCurve,
      final IntrinsicIndexDataBundle intrinicData, final CDSAnalytic fwdCDS,
    ...
    return INDEX_CAL.defaultAdjustedForwardIndexValue(fwdStartingCDS, timeToExpiry - timeStep, fwdYieldCurve, indexCoupon,
        intrinicData.withCreditCurves(fwdCC));
  }
