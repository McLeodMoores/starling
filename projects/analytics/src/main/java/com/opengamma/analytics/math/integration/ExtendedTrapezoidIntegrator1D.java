...
/**
 * The trapezoid integration rule is a two-point Newton-Cotes formula that approximates the area under the curve as a trapezoid. For a function $f(x)$, $$
 * \begin{align*} \int^{x_2} _{x_1} f(x)dx \approx \frac{1}{2}(x_2 - x_1)(f(x_1) + f(x_2)) \end{align*} $$
 * <p>
 * This class is a wrapper for the
 * <a href="http://commons.apache.org/math/api-2.1/org/apache/commons/math/analysis/integration/TrapezoidIntegrator.html">Commons Math library
 * implementation</a> of trapezoidal integration.
 */
public class ExtendedTrapezoidIntegrator1D extends Integrator1D<Double, Double> {
  ...
  /**
   * Trapezoid integration method. Note that the Commons implementation fails if the lower bound is larger than the upper - in this case, the bounds are
   * reversed and the result negated. {@inheritDoc}
   */
  @Override
