...
public class CDSIndexCalculator {
   ...
   *          The yield curve
   * @param intrinsicData
   *          credit curves, weights and recover
   * @return PUF of an index
   */
  public double indexPUF(final CDSAnalytic indexCDS, final double indexCoupon, final ISDACompliantYieldCurve yieldCurve,
   ...
   */
  public double indexPV(final CDSAnalytic indexCDS, final double indexCoupon, final ISDACompliantYieldCurve yieldCurve,
      final IntrinsicIndexDataBundle intrinsicData) {
   ...
   */
  public double indexPV(final CDSAnalytic indexCDS, final double indexCoupon, final ISDACompliantYieldCurve yieldCurve,
      final IntrinsicIndexDataBundle intrinsicData, final PriceType priceType) {
    final double prot = indexProtLeg(indexCDS, yieldCurve, intrinsicData);
   ...
   *          Clean or dirty price
   * @param valuationTime
   *          The leg value is calculated for today (t=0), then rolled forward (using the risk free yield curve) to the valuation time. This is because cash
   *          payments occur on the cash-settlement-date, which is usually three working days after the trade date (today)
   * @return The index value for a unit notional.
   */
  public double indexPV(final CDSAnalytic indexCDS, final double indexCoupon, final ISDACompliantYieldCurve yieldCurve,
      final IntrinsicIndexDataBundle intrinsicData, final PriceType priceType,
   ...
   *          credit curves, weights and recovery rates of the intrinsic names
   * @param valuationTime
   *          Valuation time. The leg value is calculated for today (t=0), then rolled forward (using the risk free yield curve) to the valuation time. This is
   *          because cash payments occur on the cash-settlement-date, which is usually three working days after the trade date (today)
   * @return The normalised intrinsic value of the protection leg.
   */
  public double indexProtLeg(final CDSAnalytic indexCDS, final ISDACompliantYieldCurve yieldCurve, final IntrinsicIndexDataBundle intrinsicData,
      final double valuationTime) {
   ...
   */
  public ISDACompliantCreditCurve impliedIndexCurve(final CDSAnalytic[] pillarCDS, final double indexCoupon, final ISDACompliantYieldCurve yieldCurve,
      final IntrinsicIndexDataBundle intrinsicData) {
   ...
   *          The yield curve
   * @param indexCoupon
   *          The coupon of the index
   * @param intrinsicData
   *          credit curves, weights and recovery rates of the intrinsic names initially 100 entries, and the realised recovery rates are 0.2 and 0.35, the this
   *          value is (0.8 + 0.65)/100 )
   * @return the default adjusted forward index value
   */
  public double defaultAdjustedForwardIndexValue(final CDSAnalytic fwdStartingCDS, final double timeToExpiry, final ISDACompliantYieldCurve yieldCurve,
    ...
    ArgumentChecker.isTrue(fwdStartingCDS.getEffectiveProtectionStart() >= timeToExpiry,
   ...
   *          The coupon of the index
   * @param indexCurve
   *          Pseudo credit curve for the index.
   * @return the default adjusted forward index value
   */
  public double defaultAdjustedForwardIndexValue(final CDSAnalytic fwdStartingCDS, final double timeToExpiry, final ISDACompliantYieldCurve yieldCurve,
      final double indexCoupon,
   ...
   *          The initial number of names in the index
   * @param yieldCurve
   *          The yield curve
   * @param indexCoupon
   *          The coupon of the index
   * @param indexCurve
   *          Pseudo credit curve for the index.
   * @param initialDefaultSettlement
   *          The (normalised) value of any defaults that have already occurred (e.g. if two defaults have occurred from an index with initially 100 entries,
   *          and the realised recovery rates are 0.2 and 0.35, the this value is (0.8 + 0.65)/100 )
   * @param numDefaults
   *          The number of defaults that have already occurred
   * @return the default adjusted forward index value
   */
  public double defaultAdjustedForwardIndexValue(final CDSAnalytic fwdStartingCDS, final double timeToExpiry, final int initialIndexSize,
    ...
    final double defSet = expectedDefaultSettlementValue(initialIndexSize, timeToExpiry, indexCurve, fwdStartingCDS.getLGD(), initialDefaultSettlement,
        numDefaults);
   ...
   */
  public double defaultAdjustedForwardSpread(final CDSAnalytic fwdStartingCDS, final double timeToExpiry, final ISDACompliantYieldCurve yieldCurve,
      final IntrinsicIndexDataBundle intrinsicData) {
   ...
   * @return The normalised expected default settlement value
   */
  public double defaultAdjustedForwardSpread(final CDSAnalytic fwdStartingCDS, final double timeToExpiry, final int initialIndexSize,
      final ISDACompliantYieldCurve yieldCurve,
    ...
    ArgumentChecker.isTrue(fwdStartingCDS.getEffectiveProtectionStart() >= timeToExpiry,
        ...
        fwdStartingCDS.getEffectiveProtectionStart(), timeToExpiry);
    final double f = (initialIndexSize - numDefaults) / (double) initialIndexSize;
    final double defSettle = expectedDefaultSettlementValue(initialIndexSize, timeToExpiry, indexCurve, fwdStartingCDS.getLGD(), initialDefaultSettlement,
        numDefaults);
   ...
   *          Pseudo credit curve for the index.
   * @param lgd
   *          The index Loss Given Default (LGD)
   * @return The normalised expected default settlement value
   */
   ...
   * @return The normalised expected default settlement value
   */
  public double expectedDefaultSettlementValue(final int initialIndexSize, final double timeToExpiry, final ISDACompliantCreditCurve indexCurve,
      final double lgd,
