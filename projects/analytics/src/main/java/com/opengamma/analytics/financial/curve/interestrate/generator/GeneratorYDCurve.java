...
public abstract class GeneratorYDCurve extends GeneratorCurve {
  ...
  /**
   * Generate a curve using the parameters of a vector and an existing bundle. The existing bundle will be required if the generated curve
   * depends on previous curves.
   *
   * @param name
   *          The curve name.
   * @param multicurves
   ...
   *          The curve name.
   * @param hwMulticurves
   *          The Hull-White one-factor with multi-curves provider.
   * @param parameters
   *          The parameters.
   * @return The curve.
   */
  public YieldAndDiscountCurve generateCurve(final String name, final HullWhiteOneFactorProviderDiscount hwMulticurves,
      final double[] parameters) {
    return generateCurve(name, hwMulticurves.getMulticurveProvider(), parameters);
  }

  /**
   * Some generators require a two stage process. The generator with the general description (like interpolated) and a specific one with all
   * the details (like the node times for the interpolated). The method create the specific generator from the generic one.
   *
   * @param data
   *          The additional data.
