...
/**
 * A shifted log normal model is one where the forward and volatility in the Black model are adjusted to produce plausible prices for far OTM options. Since
 * there are two free parameters, two prices, or a price and gradient, can be matched - the prices (as viewed by implied volatility) then have sensible
 * extrapolated values (i.e. stays positive and finite).
 * <p>
 * <b>Note</b> this is not a full smile model and should never be used for values crossing ATM (there is a step in implied volatility ATM)
 * <p>
...
public class ShiftedLogNormalTailExtrapolation {
   ...
   * @param mu
   *          The shift in the distribution such that the effective forward = f*exp(mu)
   * @param theta
   *          The volatility of the distribution
   * @return The price
   */
  ...
  public static double impliedVolatility(final double forward, final double strike, final double timeToExpiry, final double mu, final double theta) {
    ...
    final double p = price(forward, strike, timeToExpiry, isCall, mu, theta);
    if (p <= 1e-100) { // TODO this is an arbitrary choice to switch to the approximation here
      final double c = Math.log(strike / forward);
      final double a = timeToExpiry / 2;
      final double b = (-c + mu - theta * theta * timeToExpiry / 2) / theta;
      final double arg = b * b - 4 * a * c;
      if (arg < 0) {
        throw new MathException("cannot solve for sigma");
      }
      final double root = Math.sqrt(arg);
      final double volGuess = isCall ? (-b - root) / 2 / a : (-b + root) / 2 / a;
      return volGuess;
    }
    if (!isCall && p >= strike) {
      final double c = Math.log(strike / forward);
   ...
   */
  public static double dualDelta(final double forward, final double strike, final double timeToExpiry, final boolean isCall, final double mu,
      final double theta) {
  ...
  public static double dVdK(final double forward, final double strike, final double timeToExpiry, final double mu, final double theta) {
    ...
    final boolean isCall = strike >= forward;
    final double vol = impliedVolatility(forward, strike, timeToExpiry, mu, theta);
    final double dd = dualDelta(forward, strike, timeToExpiry, isCall, mu, theta);
