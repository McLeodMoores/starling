...
public class CapFloorIborInArrearsSABRCapGenericReplicationMethod {
  ...
  public MultipleCurrencyAmount presentValue(final CapFloorIbor cap, final SABRCapProviderInterface sabr) {
    ...
    final CapFloorIbor capStandard = new CapFloorIbor(cap.getCurrency(), cap.getFixingPeriodEndTime(), cap.getPaymentYearFraction(),
        cap.getNotional(),
        cap.getFixingTime(),
        cap.getIndex(), cap.getFixingPeriodStartTime(), cap.getFixingPeriodEndTime(), cap.getFixingAccrualFactor(), cap.getStrike(),
        cap.isCap());
    final double forward = multicurves.getSimplyCompoundForwardRate(cap.getIndex(), cap.getFixingPeriodStartTime(),
    ...
    final double strikePart = (1.0 + cap.getFixingAccrualFactor() * cap.getStrike())
        * _baseMethod.presentValue(capStandard, sabr).getAmount(ccy);
    ...
    try {
      if (cap.isCap()) {
        integralPart = integrator.integrate(Function1dAdapter.of(integrant), cap.getStrike(), cap.getStrike() + _integrationInterval);
      } else {
        integralPart = integrator.integrate(Function1dAdapter.of(integrant), 0.0, cap.getStrike());
      }
   ...
   */
  private static final class InArrearsIntegrant implements Function<Double, Double> {
     ...
     *          The SABR data bundle used in the standard cap/floor pricing.
     */
    InArrearsIntegrant(final CapFloorIborSABRCapMethodInterface baseMethod, final CapFloorIbor capStandard,
