 ...
 * use.
 */
public final class CurveProviderConstantSpreadRolldown implements RolldownFunction<ParameterProviderInterface> {
  /**
   * A static instance.
   */
  public static final CurveProviderConstantSpreadRolldown INSTANCE = new CurveProviderConstantSpreadRolldown();

  /**
   * Private constructor.
   */
  private CurveProviderConstantSpreadRolldown() {
  }
  ...
  private MulticurveProviderDiscount rollDown(final MulticurveProviderDiscount data, final double time) {
    ...
    for (final Map.Entry<Currency, YieldAndDiscountCurve> entry : data.getDiscountingCurves().entrySet()) {
      discountingCurves.put(entry.getKey(), YieldCurveConstantSpreadRolldown.INSTANCE.rollDown(entry.getValue(), time));
    }
    ...
    for (final Map.Entry<IborIndex, YieldAndDiscountCurve> entry : data.getForwardIborCurves().entrySet()) {
      forwardIborCurves.put(entry.getKey(), YieldCurveConstantSpreadRolldown.INSTANCE.rollDown(entry.getValue(), time));
    }
    ...
    for (final Map.Entry<IndexON, YieldAndDiscountCurve> entry : data.getForwardONCurves().entrySet()) {
      forwardONCurves.put(entry.getKey(), YieldCurveConstantSpreadRolldown.INSTANCE.rollDown(entry.getValue(), time));
    }
  ...
  private IssuerProviderDiscount rollDown(final IssuerProviderDiscount data, final double time) {
    ...
    for (final Map.Entry<Pair<Object, LegalEntityFilter<LegalEntity>>, YieldAndDiscountCurve> entry : data.getIssuerCurves().entrySet()) {
      issuerCurves.put(entry.getKey(), YieldCurveConstantSpreadRolldown.INSTANCE.rollDown(entry.getValue(), time));
    }
