...
/**
 * The total risk alpha measures the performance of an asset by comparing its returns with those of a benchmark portfolio. The benchmark portfolio represents
 * the market risk matched to the total risk of the fund.
 * <p>
 * The total risk alpha is given by: $$ \begin{eqnarray*} TRA_i = R_i - \left(R_f + \frac{\mu_M - R_f}{\sigma_M}\sigma_i\right) \end{eqnarray*} $$ where $R_i$
 * is the asset return, $R_f$ is the risk-free return, $\mu_M$ is the market return, $\sigma_M$ is the standard deviation of market returns and $\sigma_i$ is
...
public class TotalRiskAlphaCalculator {
  ...
  public TotalRiskAlphaCalculator(final DoubleTimeSeriesStatisticsCalculator expectedAssetReturnCalculator,
      ...
      final DoubleTimeSeriesStatisticsCalculator assetStandardDeviationCalculator,
      final DoubleTimeSeriesStatisticsCalculator marketStandardDeviationCalculator) {
   ...
   *          The return series of the market
