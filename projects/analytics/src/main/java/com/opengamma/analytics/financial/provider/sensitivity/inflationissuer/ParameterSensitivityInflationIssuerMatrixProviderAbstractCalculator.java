 ...
 * of the way the curve is stored (interpolated yield, function parameters, etc.) and also on the way the parameters sensitivities are aggregated (the same
 * parameter can be used in several curves). The output is a DoubleMatrix1D.
 */
public abstract class ParameterSensitivityInflationIssuerMatrixProviderAbstractCalculator {

  /**
   * The sensitivity calculator to compute the sensitivity of the value with respect to the zero-coupon continuously compounded rates at different times for the
   * discounting curve and with respect to forward rates for the forward curves.
   */
  ...
  public ParameterSensitivityInflationIssuerMatrixProviderAbstractCalculator(
      final InstrumentDerivativeVisitor<InflationIssuerProviderInterface, InflationSensitivity> curveSensitivityCalculator) {
  ...
  /**
   * Computes the sensitivity with respect to the parameters. The sensitivities are stored in a DoubleMatrix1D in the order of the currencies and indexes.
   * 
   * @param instrument
   *          The instrument. Not null.
   * @param inflationCurves
   *          The inflation provider. Not null. The sensitivity with respect to the curves in the fixedCurves list will not be part of the output total
   ...
   */
  public DoubleMatrix1D calculateSensitivity(final InstrumentDerivative instrument, final InflationIssuerProviderInterface inflationCurves,
   ...
   * @return The sensitivity (as a DoubleMatrix1D).
   */
  public abstract DoubleMatrix1D pointToParameterSensitivity(InflationSensitivity sensitivity, InflationIssuerProviderInterface inflation,
