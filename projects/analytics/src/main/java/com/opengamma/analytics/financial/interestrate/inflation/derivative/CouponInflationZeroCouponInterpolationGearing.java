...
/**
 * Class describing an zero-coupon inflation coupon where the inflation increment is multiplied by a gearing factor. The start index value is known when the
 * coupon is traded/issued. The index for a given month is given in the yield curve and in the time series on the first of the month. The pay-off is
 * factor*(Index_End / Index_Start - X) with X=0 for notional payment and X=1 for no notional payment.
 */
public class CouponInflationZeroCouponInterpolationGearing extends CouponInflation implements CouponInflationGearing {
   ...
   */
  public CouponInflationZeroCouponInterpolationGearing(final Currency currency, final double paymentTime, final double paymentYearFraction,
      final double notional, final IndexPrice priceIndex,
  ...
  public CouponInflationZeroCouponInterpolationGearing withNotional(final double notional) {
    return new CouponInflationZeroCouponInterpolationGearing(getCurrency(), getPaymentTime(), getPaymentYearFraction(), notional, getPriceIndex(),
        _indexStartValue, _referenceEndTime,
