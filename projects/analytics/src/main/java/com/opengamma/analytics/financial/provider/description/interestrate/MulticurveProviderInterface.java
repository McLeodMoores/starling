...
/**
 * Interface of a multi-curves framework providing discounting factors, forward rate (linked to Ibor index), issuer/currency specific curves and currency
 * exchange rates.
 */
public interface MulticurveProviderInterface extends ParameterProviderInterface {
  ...
  /**
   * Gets the investment factor for one Ibor index between start and end times. This quantity correspond to growth between the start and the end time for an
   * investment of 1 unit, assuming the investment growth according to the underlying curve
   * 
   * @param index
   *          The Ibor index.
   * @param startTime
  ...
  /**
   * Gets the annual compound forward ( it corresponds to $\frac{DiscountFactor(t_1)}{DiscountFactor(t_1)}^(1/accrualFactor)-1$)for one Ibor index between start
   * and end times.
   * 
   * @param index
   *          The Ibor index.
  ...
  /**
   * Gets the annual compound forward for one Ibor index between start and end times. The accrual factor is computed with the start and the end time (end time
   * -start time).
   * 
   * @param index
   *          The Ibor index.
  ...
  /**
   * Gets the underlying name(s) (i.e. {@link com.opengamma.analytics.financial.model.interestrate.curve.YieldAndDiscountCurve#getName()} for a curve name; this
   * can be multi-valued in the case of spread curves.
   * 
   * @param name
   *          The curve name
  ...
  // TODO: Some method could be available from curve ID and not only from financial description (like parameterSensitivity(CurveID id, List<DoublesPair>
  // pointSensitivity))
