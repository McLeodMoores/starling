...
public class LocalVolatilitySurfaceMoneyness extends LocalVolatilitySurface<Moneyness> {
  ...
  /**
   * Depending on the application the same local volatility surface can be seem either as either a function of calendar time and value moneyness, or as a
   * function of expiry and moneyness. Value moneyness is the current value of the forward f(t,T) divided by the initial value of the forward f(0,T) for same
   * expiry T. (strike) moneyness is the strike divided by the forward f(0,T) at the required expity T. Return a volatility for the time - moneyness pair
   * provided. Interpolation/extrapolation behaviour depends on underlying surface
   * 
   * @param t
   *          time
   * @param m
   *          the moneyness. value moneyness mv = F(t,T)/F(0,T), and strike moneyness mk = k/F(0,T) where k is the strike and F(t, T) is the value of the
   *          forward for expiry at time T at time t.
