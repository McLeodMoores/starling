...
/**
 * Caplet stripping for a set of caps with the <b>same</b> (absolute) strike. The model assumes that all the forwards (that underly the caplets) 'see' a
 * volatility that is a function of their initial time-to-expiry only (i.e. this model is not time homogeneous - the 2y forward in one years time, with have a
 * different volatility that the current 1y forward.). This volatility curve is modelled as an interpolated curve with suitably chosen knots (the number of
 * knots equals the number of caps). Provided the market cap prices are arbitrage free (i.e. the prices of two caps do not imply the price of a forward starting
...
public class CapletStrippingAbsoluteStrikeInterpolation extends CapletStrippingAbsoluteStrike {
  ...
  public CapletStrippingAbsoluteStrikeInterpolation(final List<CapFloor> caps, final YieldCurveBundle yieldCurves) {
    ...
    final CombinedInterpolatorExtrapolator baseInterpolator = CombinedInterpolatorExtrapolatorFactory.getInterpolator(DEFAULT_INTERPOLATOR,
   ...
   *          List of caps with identical strikes
   * @param interpolator
   *          the combined interpolator/extrapolator used to define the vol curve. <b>It is recommended</b> that a strictly positive interpolator is used
   * @param knots
   *          knot positions (must equal the number of caps)
   */
  public CapletStrippingAbsoluteStrikeInterpolation(final List<CapFloor> caps, final YieldCurveBundle yieldCurves,
      final CombinedInterpolatorExtrapolator interpolator, final double[] knots) {
    super(caps, yieldCurves);
