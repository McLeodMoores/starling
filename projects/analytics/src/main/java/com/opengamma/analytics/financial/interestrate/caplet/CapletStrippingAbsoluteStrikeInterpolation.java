...
/**
 * Caplet stripping for a set of caps with the <b>same</b> (absolute) strike. The model assumes that all the forwards (that underly the
 * caplets) 'see' a volatility that is a function of their initial time-to-expiry only (i.e. this model is not time homogeneous - the 2y
 * forward in one years time, with have a different volatility that the current 1y forward.). This volatility curve is modelled as an
 * interpolated curve with suitably chosen knots (the number of knots equals the number of caps). Provided the market cap prices are
 * arbitrage free (i.e. the prices of two caps do not imply the price of a forward starting cap that is below its intrinsic value) it is
 * always possible to root find for the ordinates of the knots such that every cap is exactly repriced by the model. In this way the
 * volatility (and hence the price) of every caplet (of the fixed strike) can be inferred from the curve - this result will be highly
...
public class CapletStrippingAbsoluteStrikeInterpolation extends CapletStrippingAbsoluteStrike {
  ...
  public CapletStrippingAbsoluteStrikeInterpolation(final List<CapFloor> caps, final MulticurveProviderDiscount yieldCurves) {
    ...
    final Interpolator1D baseInterpolator = NamedInterpolator1dFactory.of(DEFAULT_INTERPOLATOR,
        DEFAULT_EXTRAPOLATOR);
  ...
  /**
   * Caplet stripping for a set of caps with the <b>same</b> (absolute) strike. The interpolator is double-quadratic with a linear
   * extrapolator and a transformation so it remains everywhere positive.
   *
   * @param caps
   *          List of caps with identical strikes
   * @param yieldCurves
   *          The yield curves (should include the discount and relevant Ibor projection curve)
   * @param knots
   *          knot positions (must equal the number of caps)
   */
  public CapletStrippingAbsoluteStrikeInterpolation(final List<CapFloor> caps, final MulticurveProviderDiscount yieldCurves,
      final double[] knots) {
    this(caps, yieldCurves, NamedInterpolator1dFactory.of(DEFAULT_INTERPOLATOR, DEFAULT_EXTRAPOLATOR), knots);
  }
   ...
   *          List of caps with identical strikes
   * @param interpolator
   *          the combined interpolator/extrapolator used to define the vol curve. <b>It is recommended</b> that a strictly positive
   *          interpolator is used
   * @param knots
   *          knot positions (must equal the number of caps)
   */
  public CapletStrippingAbsoluteStrikeInterpolation(final List<CapFloor> caps, final MulticurveProviderDiscount yieldCurves,
      final Interpolator1D interpolator, final double[] knots) {
    super(caps, yieldCurves);
  ...
  protected DoubleMatrix1D solveForVolDirect(final double[] capVols) {
    // TODO keep a version of this for testing, but the main solveForVol should get prices then call solveForPrice (since this roots finds,
    ...
    final Function<DoubleMatrix1D, DoubleMatrix1D> volDiffFunc = x -> {
    ...
    final DoubleMatrix1D root = ROOTFINDER.getRoot(Function1dAdapter.of(volDiffFunc), new DoubleMatrix1D(capVols));
    return root;
  }
  ...
  private DoubleMatrix1D solveForPrice(final double[] capPrices, final DoubleMatrix1D start) {
    ...
    final Function<DoubleMatrix1D, DoubleMatrix1D> weightedPriceFunc = x -> {
    ...
    final Function<DoubleMatrix1D, DoubleMatrix2D> priceJac = x -> {
    ...
    return ROOTFINDER.getRoot(Function1dAdapter.of(weightedPriceFunc), Function1dAdapter.of(priceJac), start);
  }
  ...
  private double[] getKnots() {
    ...
    // there will be at least one more time than there are caps. For co-starting caps, our experience is that it is better to use this start
    // time, but
