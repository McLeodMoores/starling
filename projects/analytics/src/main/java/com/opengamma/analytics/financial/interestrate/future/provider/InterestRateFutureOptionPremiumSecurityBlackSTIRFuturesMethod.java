...
/**
 * Method for the pricing of STIR future options with up-front premium. The pricing is done with a Black approach on the future rate
 * (1-price). The Black parameters are obtain from implied volatility providers at (expiration-delay-strike-rate) points. The delay is the
 * time difference between the underlying futures last trading date and the option expiration. The price of the underlying STIR futures is
 * computed by "discounting" (no convexity adjustment).
 */
public final class InterestRateFutureOptionPremiumSecurityBlackSTIRFuturesMethod extends FuturesSecurityBlackSTIRFuturesMethod {

  /**
   * Creates the method unique instance.
   */
  private static final InterestRateFutureOptionPremiumSecurityBlackSTIRFuturesMethod INSTANCE = new InterestRateFutureOptionPremiumSecurityBlackSTIRFuturesMethod();

  /**
   * Constructor.
   */
  private InterestRateFutureOptionPremiumSecurityBlackSTIRFuturesMethod() {
  }
   ...
   * @return The instance.
   */
  public static InterestRateFutureOptionPremiumSecurityBlackSTIRFuturesMethod getInstance() {
    return INSTANCE;
  }
   ...
   * @return The security price.
   */
  public double underlyingFuturesPrice(final InterestRateFutureOptionPremiumSecurity security, final ParameterProviderInterface blackData) {
    return METHOD_FUTURE.price(security.getUnderlyingFuture(), blackData.getMulticurveProvider());
  }

  /**
   * Computes the option security theoretical delta wrt the underlying futures price. The futures price is computed without convexity
   * adjustment. It is supposed that for a given strike the volatility does not change with the curves.
   *
   * @param security
   *          The future option security.
   * @param blackData
   ...
   * @return The delta.
   */
  public double deltaUnderlyingPrice(final InterestRateFutureOptionPremiumSecurity security,
      final BlackSTIRFuturesProviderInterface blackData) {
    ArgumentChecker.notNull(security, "Option security");
    ArgumentChecker.notNull(blackData, "Black data");
    // Forward sweep
   ...
   * @return The security price.
   */
  public double gammaUnderlyingPrice(final InterestRateFutureOptionPremiumSecurity security,
      final BlackSTIRFuturesProviderInterface blackData) {
    ArgumentChecker.notNull(security, "Option security");
    ArgumentChecker.notNull(blackData, "Black data");
    // Forward sweep
   ...
   */
  public double vegaUnderlyingPrice(final InterestRateFutureOptionPremiumSecurity security,
      final BlackSTIRFuturesProviderInterface blackData) {
    // Forward sweep
   ...
   * @return the theta.
   */
  public double thetaUnderlyingPrice(final InterestRateFutureOptionPremiumSecurity security,
