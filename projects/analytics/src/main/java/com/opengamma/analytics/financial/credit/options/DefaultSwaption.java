...
public class DefaultSwaption {
   ...
   *          The credit curve
   * @param strike
   *          The fractional strike
   * @param optionExpiry
   *          The option expiry
   * @param vol
   *          The spread volatility
   * @param isPayer
   *          True if payer swaption
   * @param hasFrontEndProt
   *          True if no-knockout swaption
   * @return The option price
   */
  public double price(final CDSAnalytic forwardStartingCDS, final ISDACompliantYieldCurve yieldCurve, final ISDACompliantCreditCurve creditCurve,
      final double strike, final double optionExpiry,
    ...
    ArgumentChecker.isTrue(forwardStartingCDS.getEffectiveProtectionStart() >= optionExpiry,
        "Have not provided a forward CDS. The option expiry is {}, but the CDS(effective) protection start time is {}", optionExpiry,
        forwardStartingCDS.getEffectiveProtectionStart());

    // front end protection is worth zero for an option to be the seller of protection
    final double fep = isPayer && hasFrontEndProt
        ? forwardStartingCDS.getLGD() * yieldCurve.getDiscountFactor(optionExpiry) * (1 - creditCurve.getSurvivalProbability(optionExpiry))
        : 0.0;
  ...
  public double priceFlat(final CDSAnalytic cds, final ISDACompliantYieldCurve yieldCurve, final ISDACompliantCreditCurve creditCurve, final double strike,
      final double optionExpiry,
    ...
    ArgumentChecker.isTrue(cds.getCashSettleTime() >= optionExpiry,
        "Have provided a forward CDS. The option expiry is {}, but the CDS cash-settlement time is {}", optionExpiry,
  ...
  public double priceMod(final CDSAnalytic cds, final ISDACompliantYieldCurve yieldCurve, final ISDACompliantCreditCurve creditCurve, final double strike,
      final double optionExpiry, final double vol,
    ...
    // front end protection is worth zero for an option to be the seller of protection
    final double fep = isPayer && hasFrontEndProt
        ? cds.getLGD() * yieldCurve.getDiscountFactor(optionExpiry) * (1 - creditCurve.getSurvivalProbability(optionExpiry))
        : 0.0;
   ...
   *          The option price
   * @param isPayer
   *          True of payer swaption
   * @param hasFrontEndProt
   *          True if no-knockout swaption
   * @return The implied volatility
   */
  public double impliedVol(final CDSAnalytic forwardStartingCDS, final ISDACompliantYieldCurve yieldCurve, final ISDACompliantCreditCurve creditCurve,
      final double strike, final double optionExpiry,
