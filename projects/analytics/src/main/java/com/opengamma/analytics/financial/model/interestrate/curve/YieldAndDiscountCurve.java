 ...
 * <p>
 * The relation between the rate <i>r(t)</i> at the maturity <i>t</i> and the discount factor <i>df(t)</i> is <i>df(t)=e<sup>-r(t)t</sup></i>.
 */
public abstract class YieldAndDiscountCurve implements InterestRateModel<Double> {
   ...
   */
  public abstract double getForwardRate(double t);
   ...
   *          The number of composition per year.
   ...
   */
  public abstract double[] getInterestRateParameterSensitivity(double time);
  ...
  /**
   * Create another {@link YieldAndDiscountCurve} with the zero-coupon rates shifted by a given amount at a given time. The shift is done around the given time
   * within the default range 1.0E-3.
   * 
   * @param t
   *          The time.
   * @param shift
