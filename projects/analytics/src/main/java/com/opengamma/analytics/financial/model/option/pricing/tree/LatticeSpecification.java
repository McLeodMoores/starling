...
public abstract class LatticeSpecification {
   ...
   *          Number of Steps
   * @param dt
   *          Time step, that is, dt * N = tiemToExpiry
   * @return {up factor, down factor, up probability, down probability}
   */
  public abstract double[] getParameters(double spot, double strike, double timeToExpiry, double volatility, double interestRate, int nSteps, double dt);
   ...
   *          Time step
   * @param greeksTmp
   *          {price_{0,0}, delta_{0,0}, gamma_{0,0}, price_{2,1}}
   * @return Theta
   */
  public double getTheta(final double spot, final double volatility, final double interestRate, final double dividend, final double dt,
      final double[] greeksTmp) {
   ...
   */
  public double[] getParametersTrinomial(final double spot, final double strike, final double timeToExpiry, final double volatility, final double interestRate,
