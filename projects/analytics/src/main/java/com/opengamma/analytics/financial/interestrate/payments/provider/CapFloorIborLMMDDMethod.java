...
public final class CapFloorIborLMMDDMethod {
   ...
   * @param lmmData
   *          The LMM and multi-curves provider.
  ...
  public MultipleCurrencyAmount presentValue(final CapFloorIbor cap, final LiborMarketModelDisplacedDiffusionProviderInterface lmmData) {
    ...
    final int index = lmmData.getLMMParameters().getTimeIndex(cap.getFixingPeriodStartTime());
    double volatility = 0;
    ...
    final double timeDependentFactor = Math
        .sqrt((Math.exp(2 * parameters.getMeanReversion() * cap.getFixingTime()) - 1.0) / (2.0 * parameters.getMeanReversion()));
    volatility *= timeDependentFactor;
    final double displacement = parameters.getDisplacement()[index];
    ...
    final double forwardDsc = (multicurves.getDiscountFactor(ccy, cap.getFixingPeriodStartTime())
        / multicurves.getDiscountFactor(ccy, cap.getFixingPeriodEndTime())
