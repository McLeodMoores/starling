...
public class ParameterSensitivityLMMDDDiscountInterpolatedFDCalculator {
   ...
   *          The shift used for finite difference.
   */
  public ParameterSensitivityLMMDDDiscountInterpolatedFDCalculator(
      final InstrumentDerivativeVisitor<LiborMarketModelDisplacedDiffusionProviderInterface, MultipleCurrencyAmount> valueCalculator,
      final double shift) {
  ...
  public MultipleCurrencyParameterSensitivity calculateSensitivity(final InstrumentDerivative instrument,
      final LiborMarketModelDisplacedDiffusionProviderDiscount lmm) {
    ...
    for (final Currency ccy : ccyDiscounting) {
      ...
      for (int loopnode = 0; loopnode < nbNodePoint; loopnode++) {
        ...
        final LiborMarketModelDisplacedDiffusionProviderDiscount marketDscBumpedPlus = new LiborMarketModelDisplacedDiffusionProviderDiscount(
            lmm.getMulticurveProvider().withDiscountFactor(ccy,
                dscBumpedPlus),
            lmm.getLMMParameters(), lmm.getLMMCurrency());
        ...
        final LiborMarketModelDisplacedDiffusionProviderDiscount marketDscBumpedMinus = new LiborMarketModelDisplacedDiffusionProviderDiscount(
            lmm.getMulticurveProvider().withDiscountFactor(ccy,
                dscBumpedMinus),
            lmm.getLMMParameters(), lmm.getLMMCurrency());
    ...
    for (final IndexON index : indexON) {
      ...
      for (int loopnode = 0; loopnode < nbNodePoint; loopnode++) {
        ...
        final YieldAndDiscountCurve fwdBumpedPlus = new YieldCurve(curveInt.getName(),
            new InterpolatedDoublesCurve(curveInt.getXDataAsPrimitive(), yieldBumpedPlus, curveInt.getInterpolator(), true));
        final LiborMarketModelDisplacedDiffusionProviderDiscount marketFwdBumpedPlus = new LiborMarketModelDisplacedDiffusionProviderDiscount(
            lmm.getMulticurveProvider().withForward(index,
                fwdBumpedPlus),
            lmm.getLMMParameters(), lmm.getLMMCurrency());
        ...
        final YieldAndDiscountCurve fwdBumpedMinus = new YieldCurve(curveInt.getName(),
            new InterpolatedDoublesCurve(curveInt.getXDataAsPrimitive(), yieldBumpedMinus, curveInt.getInterpolator(), true));
        final LiborMarketModelDisplacedDiffusionProviderDiscount marketFwdBumpedMinus = new LiborMarketModelDisplacedDiffusionProviderDiscount(
            lmm.getMulticurveProvider().withForward(index,
                fwdBumpedMinus),
            lmm.getLMMParameters(), lmm.getLMMCurrency());
