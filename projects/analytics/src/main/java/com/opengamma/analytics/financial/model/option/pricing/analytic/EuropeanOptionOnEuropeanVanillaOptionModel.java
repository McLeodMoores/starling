...
public class EuropeanOptionOnEuropeanVanillaOptionModel
  ...
  public Function1D<StandardOptionDataBundle, Double> getPricingFunction(final EuropeanOptionOnEuropeanVanillaOptionDefinition definition) {
    ...
    return new Function1D<StandardOptionDataBundle, Double>() {
      ...
      public Double evaluate(final StandardOptionDataBundle data) {
        ...
        final double sigma = data.getVolatility(t1, k1); // REVIEW emcleod 20-7-10 This will work with a flat volatility surface but
                                                         // otherwise will give odd
        // results
        final double r = data.getInterestRate(t1);
        final double b = data.getCostOfCarry();
        final double criticalValue = getCriticalValue(
            new EuropeanVanillaOptionDefinition(k2, new Expiry(DateUtils.getDateOffsetWithYearFraction(date, deltaT)), underlying.isCall()),
            data, k1);
  ...
  private double getCriticalValue(final OptionDefinition definition, final StandardOptionDataBundle data, final double k) {
    ...
    return ROOT_FINDER.getRoot(Function1dAdapter.of(f), 0., 10000.);
  }
}
