...
/**
 * Pricing method for vanilla Forex option transactions with Black function and a volatility provider. OG-Implementation: Vanilla Forex
 * options: Garman-Kohlhagen and risk reversal/strangle, version 1.5, May 2012.
 */
public final class ForexOptionVanillaBlackSmileMethod {
   ...
   * @param marketData
   *          the curve and smile data, not null
  ...
  public MultipleCurrencyAmount presentValue(final ForexOptionVanilla option, final BlackForexSmileProviderInterface marketData) {
    ...
    ArgumentChecker.isTrue(marketData.checkCurrencies(option.getCurrency1(), option.getCurrency2()),
    ...
    final double volatility = marketData.getVolatility(option.getCurrency1(), option.getCurrency2(), option.getTimeToExpiry(),
        option.getStrike(), forward);
    final BlackFunctionData dataBlack = new BlackFunctionData(forward, dfDomestic, volatility);
    final Function1D<BlackFunctionData, Double> func = BLACK_FUNCTION.getPriceFunction(option);
    final double price = func.evaluate(dataBlack) * Math.abs(option.getUnderlyingForex().getPaymentCurrency1().getAmount())
  ...
  public MultipleCurrencyAmount currencyExposure(final ForexOptionVanilla option, final BlackForexSmileProviderInterface marketData) {
    ...
    currencyExposure = currencyExposure.plus(option.getUnderlyingForex().getCurrency2(),
        -deltaSpot * Math.abs(option.getUnderlyingForex().getPaymentCurrency1().getAmount()) * spot * sign
            + price);
   ...
   *          the curve and smile data, not null
   * @param directQuote
   *          true if the gamma should be computed with respect to the direct quote (1 foreign = x domestic) or the reverse quote (1
   *          domestic = x foreign)
   * @return the relative delta
   */
  public double deltaRelative(final ForexOptionVanilla option, final BlackForexSmileProviderInterface marketData,
    ...
    final double volatility = marketData.getVolatility(option.getCurrency1(), option.getCurrency2(), option.getTimeToExpiry(),
        option.getStrike(), forward);
    final double sign = option.isLong() ? 1.0 : -1.0;
    final double deltaDirect = BlackFormulaRepository.delta(forward, option.getStrike(), option.getTimeToExpiry(), volatility,
        option.isCall()) * dfForeign * sign;
    if (directQuote) {
      return deltaDirect;
    }
  ...
  /**
   * Computes the relative delta of the Forex option multiplied by the spot rate. The relative delta is the amount in the foreign currency
   * equivalent to the option up to the first order divided by the option notional. The reason to multiply by the spot rate is to be able to
   * compute the change of value for a relative increase of e of the spot rate (from X to X(1+e)).
   * 
   * @param option
   *          the Forex option, not null
   * @param marketData
   ...
   */
  public double deltaRelativeSpot(final ForexOptionVanilla option, final BlackForexSmileProviderInterface marketData,
      final boolean directQuote) {
    ArgumentChecker.notNull(option, "option");
    ArgumentChecker.notNull(marketData, "marketData");
    ArgumentChecker.isTrue(marketData.checkCurrencies(option.getCurrency1(), option.getCurrency2()),
   ...
   * @return The delta.
   */
  public CurrencyAmount delta(final ForexOptionVanilla option, final BlackForexSmileProviderInterface marketData,
      final boolean directQuote) {
    ArgumentChecker.notNull(option, "FX option");
    ArgumentChecker.notNull(marketData, "marketData");
    final double deltaRelative = deltaRelative(option, marketData, directQuote);
    return CurrencyAmount.of(option.getUnderlyingForex().getCurrency2(),
        deltaRelative * Math.abs(option.getUnderlyingForex().getPaymentCurrency1().getAmount()));
  }
   ...
   */
  public double gammaRelative(final ForexOptionVanilla option, final BlackForexSmileProviderInterface marketData,
      final boolean directQuote) {
    ArgumentChecker.notNull(option, "option");
    ArgumentChecker.notNull(marketData, "marketData");
    ArgumentChecker.isTrue(marketData.checkCurrencies(option.getCurrency1(), option.getCurrency2()),
    ...
    final double volatility = marketData.getVolatility(option.getCurrency1(), option.getCurrency2(), option.getTimeToExpiry(),
        option.getStrike(), forward);
    final double sign = option.isLong() ? 1.0 : -1.0;
    final double gammaDirect = BlackFormulaRepository.gamma(forward, option.getStrike(), option.getTimeToExpiry(), volatility)
  ...
  /**
   * Computes the relative gamma of the Forex option multiplied by the spot rate. The relative gamma is the second order derivative of the
   * pv relative to the option notional. The reason to multiply by the spot rate is to be able to compute the change of delta for a relative
   ...
   */
  public double gammaRelativeSpot(final ForexOptionVanilla option, final BlackForexSmileProviderInterface marketData,
      final boolean directQuote) {
    ArgumentChecker.notNull(option, "option");
    ArgumentChecker.notNull(marketData, "marketData");
    ArgumentChecker.isTrue(marketData.checkCurrencies(option.getCurrency1(), option.getCurrency2()),
   ...
   * @return the gamma
   */
  public CurrencyAmount gamma(final ForexOptionVanilla option, final BlackForexSmileProviderInterface marketData,
      final boolean directQuote) {
    ArgumentChecker.notNull(option, "option");
    ArgumentChecker.notNull(marketData, "marketData");
    final double gammaRelative = gammaRelative(option, marketData, directQuote);
    return CurrencyAmount.of(option.getUnderlyingForex().getCurrency2(),
        gammaRelative * Math.abs(option.getUnderlyingForex().getPaymentCurrency1().getAmount()));
  }
   ...
   */
  public CurrencyAmount gammaSpot(final ForexOptionVanilla option, final BlackForexSmileProviderInterface marketData,
      final boolean directQuote) {
    ArgumentChecker.notNull(option, "option");
    ArgumentChecker.notNull(marketData, "marketData");
    ArgumentChecker.isTrue(marketData.checkCurrencies(option.getCurrency1(), option.getCurrency2()),
    ...
    return CurrencyAmount.of(option.getUnderlyingForex().getCurrency2(),
        gammaRelativeSpot * Math.abs(option.getUnderlyingForex().getPaymentCurrency1().getAmount()));
  }

  /**
   * Computes the Theta (derivative with respect to the time) using the forward driftless theta in the Black formula. The theta is not
   * scaled. Reference on driftless theta: The complete guide to Option Pricing Formula (2007), E. G. Haug, Mc Graw Hill, p. 67, equation
   * (2.43)
   * 
   * @param option
   *          the Forex option, not null
   ...
   */
  public MultipleCurrencyMulticurveSensitivity presentValueCurveSensitivity(final ForexOptionVanilla option,
  ...
  /**
   * Computes the volatility sensitivity of the vanilla option with the Black function and a volatility from a volatility surface. The
   ...
   */
  public PresentValueForexBlackVolatilitySensitivity presentValueBlackVolatilitySensitivity(final ForexOptionVanilla option,
      final BlackForexSmileProviderInterface marketData) {
    ArgumentChecker.notNull(option, "option");
    ArgumentChecker.notNull(marketData, "marketData");
    ArgumentChecker.isTrue(marketData.checkCurrencies(option.getCurrency1(), option.getCurrency2()),
    ...
    final BlackFunctionData dataBlack = new BlackFunctionData(forward, dfDomestic, volatility);
    final double[] priceAdjoint = BLACK_FUNCTION.getPriceAdjoint(option, dataBlack);
    final double volatilitySensitivityValue = priceAdjoint[2] * Math.abs(option.getUnderlyingForex().getPaymentCurrency1().getAmount())
        * (option.isLong() ? 1.0 : -1.0);
    ...
    final PresentValueForexBlackVolatilitySensitivity sensi = new PresentValueForexBlackVolatilitySensitivity(
        option.getUnderlyingForex().getCurrency1(), option.getUnderlyingForex()
            .getCurrency2(),
        result);
    return sensi;
  }
  ...
  public PresentValueForexBlackVolatilityNodeSensitivityDataBundle presentValueBlackVolatilityNodeSensitivity(
      final ForexOptionVanilla option,
      final BlackForexSmileProviderInterface marketData) {
    ArgumentChecker.notNull(option, "option");
    ArgumentChecker.notNull(marketData, "marketData");
    ArgumentChecker.isTrue(marketData.checkCurrencies(option.getCurrency1(), option.getCurrency2()),
        "Option currencies not compatible with smile data");
    final MulticurveProviderInterface multicurves = marketData.getMulticurveProvider();
    final PresentValueForexBlackVolatilitySensitivity pointSensitivity = presentValueBlackVolatilitySensitivity(option, marketData); // In
                                                                                                                                     // ccy2
    ...
    final VolatilityAndBucketedSensitivities volAndSensitivities = marketData.getVolatilityAndSensitivities(option.getCurrency1(),
        option.getCurrency2(),
    ...
    return new PresentValueForexBlackVolatilityNodeSensitivityDataBundle(option.getUnderlyingForex().getCurrency1(),
        option.getUnderlyingForex().getCurrency2(), new DoubleMatrix1D(volatilityModel.getTimeToExpiration()),
