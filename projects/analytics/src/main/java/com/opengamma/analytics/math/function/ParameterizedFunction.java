...
/**
 * This class defines a 1-D function that takes both its argument and parameters inputs into the {@link #evaluate} method. The function can also be converted
 * into a 1-D function of the arguments or a 1-D function of the parameters.
 *
 * For example, assume that there is a function $f(x, \overline{a})$ defined as: $$ \begin{align*} f(x, \overline{a}) = a_0 + a_1 x + a_2 x^2 + a_3 x^6
 * \end{align*} $$ The {@link #evaluate} method takes the value $x$ and the parameters $\overline{a}$ and returns the result. If the function is converted into
 * a function of the arguments, the resulting function $g(x)$ is: $$ \begin{align*} g(x) = a_0 + a_1 x + a_2 x^2 + a_3 x^6 \end{align*} $$ with $\overline{a}$
 * constant. If the function is converted into a function of the parameters, the resulting function $h(\overline{a})$ is: $$ \begin{align*} h(\overline{a}) =
 * a_0 + a_1 x + a_2 x^2 + a_3 x^6 \end{align*} $$ with $x$ constant.
 *
...
public abstract class ParameterizedFunction<S, T, U> {
  ...
  /**
   * @param x
   *          The value at which the function is to be evaluated, not null
  ...
  /**
   * @param params
   *          The parameters for which the function is to be evaluated, not null
