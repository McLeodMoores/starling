...
/**
 * Class that generates weights and abscissas for Gauss-Jacobi quadrature. The weights $w_i$ are given by: $$ \begin{align*} w_i = \frac{2^{\alpha + \beta}(2n +
 * \alpha + \beta)\Gamma(\alpha + n)\Gamma(\beta + n)}{n!\Gamma(n + \alpha + \beta + 1)J_i'(x_i) J_{i - 1}} \end{align*} $$ where $x_i$ is the $i^{th}$ root of
 * the orthogonal polynomial, $J_i$ is the $i^{th}$ polynomial and $J_i'$ is the first derivative of the $i^{th}$ polynomial. The orthogonal polynomial is
 * generated by {@link com.opengamma.analytics.math.function.special.JacobiPolynomialFunction}.
 */
public class GaussJacobiWeightAndAbscissaFunction implements QuadratureWeightAndAbscissaFunction {
   ...
   * @param beta
   *          The value of $\beta$ to use when generating the polynomials
   */
  ...
  public GaussianQuadratureData generate(final int n) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      x[i] = root;
      w[i] = GAMMA_FUNCTION.evaluate(_alpha + n) * GAMMA_FUNCTION.evaluate(_beta + n) / MathUtils.factorialDouble(n) / GAMMA_FUNCTION.evaluate(n + _c + 1) * d
          * Math.pow(2, _c)
