...
public class CubeShiftFunctionFactory {
   ...
   * @throws IllegalArgumentException
   *           If the cube type is not constant, functional, interpolated, nodal or spread
   */
  public static Cube<Double, Double, Double, Double> getShiftedCube(final Cube<Double, Double, Double, Double> cube, final double shift,
      final boolean useAdditive) {
    if (cube instanceof ConstantDoublesCube) {
      return useAdditive ? CONSTANT_ADDITIVE.evaluate((ConstantDoublesCube) cube, shift) : CONSTANT_MULTIPLICATIVE.evaluate((ConstantDoublesCube) cube, shift);
    }
    if (cube instanceof FunctionalDoublesCube) {
      return useAdditive ? FUNCTIONAL_ADDITIVE.evaluate((FunctionalDoublesCube) cube, shift)
          : FUNCTIONAL_MULTIPLICATIVE.evaluate((FunctionalDoublesCube) cube, shift);
    }
    if (cube instanceof InterpolatedDoublesCube) {
      return useAdditive ? INTERPOLATED_ADDITIVE.evaluate((InterpolatedDoublesCube) cube, shift)
          : INTERPOLATED_MULTIPLICATIVE.evaluate((InterpolatedDoublesCube) cube, shift);
    }
   ...
   *           If the cube type is not constant, functional, interpolated, nodal or spread
   */
  public static Cube<Double, Double, Double, Double> getShiftedCube(final Cube<Double, Double, Double, Double> cube, final double x, final double y,
      final double z, final double shift, final boolean useAdditive) {
    if (cube instanceof ConstantDoublesCube) {
    ...
    if (cube instanceof InterpolatedDoublesCube) {
      return useAdditive ? INTERPOLATED_ADDITIVE.evaluate((InterpolatedDoublesCube) cube, x, y, z, shift)
          : INTERPOLATED_MULTIPLICATIVE.evaluate((InterpolatedDoublesCube) cube, x, y, z, shift);
    }
    if (cube instanceof InterpolatedFromSurfacesDoublesCube && useAdditive) {
      return INTERPOLATED_FROM_CURVES_ADDITIVE.evaluate((InterpolatedFromSurfacesDoublesCube) cube, x, y, z, shift);
    }
    if (cube instanceof NodalDoublesCube) {
      return useAdditive ? NODAL_ADDITIVE.evaluate((NodalDoublesCube) cube, x, y, z, shift)
          : NODAL_MULTIPLICATIVE.evaluate((NodalDoublesCube) cube, x, y, z, shift);
    }
    throw new IllegalArgumentException("Do not have a cube shift function for cube " + cube.getClass());
  }
   ...
   * @param z
   *          An array of <i>z</i> values to shift
   * @param shift
   *          The shifts
   * @param useAdditive
   *          true if the shift is additive, false if the shift is multiplicative (i.e. a percentage shift)
  ...
  public static Cube<Double, Double, Double, Double> getShiftedCube(final Cube<Double, Double, Double, Double> cube, final double shift, final String newName,
      final boolean useAdditive) {
    if (cube instanceof ConstantDoublesCube) {
      return useAdditive ? CONSTANT_ADDITIVE.evaluate((ConstantDoublesCube) cube, shift, newName)
          : CONSTANT_MULTIPLICATIVE.evaluate((ConstantDoublesCube) cube, shift, newName);
    }
    if (cube instanceof FunctionalDoublesCube) {
      return useAdditive ? FUNCTIONAL_ADDITIVE.evaluate((FunctionalDoublesCube) cube, shift, newName)
          : FUNCTIONAL_MULTIPLICATIVE.evaluate((FunctionalDoublesCube) cube, shift, newName);
    }
    if (cube instanceof InterpolatedDoublesCube) {
      return useAdditive ? INTERPOLATED_ADDITIVE.evaluate((InterpolatedDoublesCube) cube, shift, newName)
          : INTERPOLATED_MULTIPLICATIVE.evaluate((InterpolatedDoublesCube) cube, shift, newName);
    }
    if (cube instanceof InterpolatedFromSurfacesDoublesCube && useAdditive) {
      return INTERPOLATED_FROM_CURVES_ADDITIVE.evaluate((InterpolatedFromSurfacesDoublesCube) cube, shift, newName);
    }
    if (cube instanceof NodalDoublesCube) {
      return useAdditive ? NODAL_ADDITIVE.evaluate((NodalDoublesCube) cube, shift, newName)
          : NODAL_MULTIPLICATIVE.evaluate((NodalDoublesCube) cube, shift, newName);
    }
    throw new IllegalArgumentException("Do not have a cube shift function for cube " + cube.getClass());
  }
   ...
   */
  public static Cube<Double, Double, Double, Double> getShiftedCube(final Cube<Double, Double, Double, Double> cube, final double x, final double y,
      final double z, final double shift, final String newName, final boolean useAdditive) {
    if (cube instanceof ConstantDoublesCube) {
      throw new UnsupportedOperationException("Cannot shift a single point on a constant curve");
    }
    if (cube instanceof FunctionalDoublesCube) {
    ...
    if (cube instanceof InterpolatedDoublesCube) {
      return useAdditive ? INTERPOLATED_ADDITIVE.evaluate((InterpolatedDoublesCube) cube, x, y, z, shift, newName)
          : INTERPOLATED_MULTIPLICATIVE.evaluate((InterpolatedDoublesCube) cube, x, y, z, shift, newName);
    }
    if (cube instanceof InterpolatedFromSurfacesDoublesCube && useAdditive) {
      return INTERPOLATED_FROM_CURVES_ADDITIVE.evaluate((InterpolatedFromSurfacesDoublesCube) cube, x, y, z, shift, newName);
    }
    if (cube instanceof NodalDoublesCube) {
      return useAdditive ? NODAL_ADDITIVE.evaluate((NodalDoublesCube) cube, x, y, z, shift, newName)
          : NODAL_MULTIPLICATIVE.evaluate((NodalDoublesCube) cube, x, y, z, shift, newName);
    }
    throw new IllegalArgumentException("Do not have a cube shift function for cube " + cube.getClass());
  }
