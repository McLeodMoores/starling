...
extends CalibrationEngineWithPrices<DATA_TYPE> {
  ...
  public void addInstrument(final InstrumentDerivative instrument, final double calibrationPrice) {
    ArgumentChecker.isTrue(
        instrument instanceof CapFloorInflationYearOnYearInterpolation || instrument instanceof CapFloorInflationYearOnYearMonthly
        || instrument instanceof Annuity,
    ...
    if (instrument instanceof CapFloorInflationYearOnYearInterpolation) {
      ...
      _instrumentExpiryIndex.add(Arrays
          .binarySearch(((SuccessiveRootFinderInflationYearOnYearCapFloorCalibrationObjective) _calibrationObjective).getInflationCapYearOnYearParameters()
    ...
    if (instrument instanceof Annuity) {
      ...
      if (annuity.getNthPayment(annuity.getNumberOfPayments() - 1) instanceof CapFloorInflationYearOnYearInterpolation) {
        final CapFloorInflationYearOnYearInterpolation cap = (CapFloorInflationYearOnYearInterpolation) annuity
            .getNthPayment(annuity.getNumberOfPayments() - 1);
        _calibrationTimes.add(cap.getPaymentTime());
  ...
  public void calibrate(final DATA_TYPE data) {
    ...
    final SuccessiveRootFinderInflationYearOnYearCapFloorCalibrationObjective objective =
        (SuccessiveRootFinderInflationYearOnYearCapFloorCalibrationObjective) _calibrationObjective;
