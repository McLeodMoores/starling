...
public class CapletStrippingAbsoluteStrikePSpline extends CapletStrippingAbsoluteStrike {
  ...
  /**
   * <<<<<<<
   * HEAD:projects/analytics/src/main/java/com/opengamma/analytics/financial/interestrate/capletstripping/CapletStrippingAbsoluteStrikePSpline.java
   * caplet stripping for a set of caps with the <b>same</b> (absolute) strike. The interpolator is double-quadratic with a linear
   * extrapolator and a transformation so it remains everywhere positive. For co-starting caps the knots are the first caplet expiry, then
   * the end time of all the caps expect the final one (i.e. all but the first unique caplets in the longest cap see volatilities from the
   * extrapolated part of the curve). If the caps are not co-starting it is not possible to auto-generate the knots and these should be
   * supplied. ======= caplet stripping for a set of caps with the <b>same</b> (absolute) strike. The interpolator is double-quadratic with
   * a linear extrapolator and a transformation so it remains everywhere positive. For co-starting caps the knots are the first caplet
   * expiry, then the end time of all the caps expect the final one (i.e. all but the first unique caplets in the longest cap see
   * volatilities from the extrapolated part of the curve). If the caps are not co-starting it is not possible to auto-generate the knots
   * and these should be supplied. >>>>>>>
   * develop:projects/analytics/src/main/java/com/opengamma/analytics/financial/interestrate/caplet/CapletStrippingAbsoluteStrikePSpline.java
   *
   * @param caps
  ...
  public CapletStrippingAbsoluteStrikePSpline(final List<CapFloor> caps, final MulticurveProviderDiscount yieldCurves) {
    ...
    // This assigns a unique volatility to each underlying caplet. The resultant volatility curve will not be smooth (it is piecewise
    // linear),
    // however any (higher order) function that hits all the caplet volatilities, will (by construction) also reprice exactly the market
    // caps. The
    // secondary fitting of this smooth curve (for interpolation/extrapolation to seasoned/non-standard caplets) is linear, and thus will be
    // very quick next
  ...
  // public CapletStrippingAbsoluteStrikePSpline(List<CapFloor> caps, YieldCurveBundle yieldCurves, final int differenceOrder, final double
  // lambda) {
  // super(caps, yieldCurves);
  //
  // }
  ...
  public CapletStrippingSingleStrikeResult solveForVol(final double[] capVols, final double[] errors, final boolean solveViaPrice) {
    ...
    if (solveViaPrice) {
      ...
      final Function<DoubleMatrix1D, DoubleMatrix1D> modelVolFunc = x -> {
        final double[] modelVols = pricer.impliedVols(_volModel.evaluate(x));
        return new DoubleMatrix1D(modelVols);
      };
      lsRes = NLLSWP.solve(new DoubleMatrix1D(capVols), new DoubleMatrix1D(errors), Function1dAdapter.of(modelVolFunc),
          new DoubleMatrix1D(_nWeights, capVols[capVols.length - 1]),
          _penalty);
    }
    ...
    // least-squares gives chi2 including the penalty, and is calculated via the price difference and vega w we just want the fit error
    // TODO maybe the solver should provide this?
  ...
  private LeastSquareResults solveForPrice(final double[] capPrices, final double[] capVols, final double[] errors,
      final boolean scaleByVega) {

    final int n = capPrices.length;
    final MultiCapFloorPricer pricer = getPricer();

    final Function<DoubleMatrix1D, Boolean> allowed = x -> {
    ...
    final Function<DoubleMatrix1D, DoubleMatrix1D> modelPriceFunc = x -> {
    ...
    return NLLSWP.solve(new DoubleMatrix1D(capPrices), sigma, Function1dAdapter.of(modelPriceFunc),
        new DoubleMatrix1D(_nWeights, capVols[n - 1]), _penalty, Function1dAdapter.of(allowed));
  }
