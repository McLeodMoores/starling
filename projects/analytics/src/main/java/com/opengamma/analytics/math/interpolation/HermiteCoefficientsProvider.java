...
public class HermiteCoefficientsProvider {
   ...
   *          (yValues_i)
   * @param intervals
   *          (xValues_{i+1} - xValues_{i})
   * @param slopes
   *          (yValues_{i+1} - yValues_{i})/(xValues_{i+1} - xValues_{i})
   * @param first
   *          First derivatives at xValues_i
   * @return Coefficient matrix whose i-th row vector is { a_n, a_{n-1}, ...} for the i-th interval, where a_n, a_{n-1},... are coefficients of f(x) = a_n
   *         (x-x_i)^n + a_{n-1} (x-x_i)^{n-1} + .... with n=3
   */
   ...
   * @param slopes
   *          (yValues_{i+1} - yValues_{i})/(xValues_{i+1} - xValues_{i})
   * @param slopeSensitivity
   *          Derivative values of slope with respect to yValues
   * @param firstWithSensitivity
   *          First derivative values at xValues_i and their yValues dependencies
   * @return Coefficient matrix and its node dependencies
   */
  public DoubleMatrix2D[] solveWithSensitivity(final double[] values, final double[] intervals, final double[] slopes, final double[][] slopeSensitivity,
      final DoubleMatrix1D[] firstWithSensitivity) {
    ...
    for (int i = 0; i < nData - 1; ++i) {
      ...
      for (int k = 0; k < nData; ++k) {
        coefSense[0][k] = -(2. * slopeSensitivity[i][k] - firstWithSensitivity[i + 2].getData()[k] - firstWithSensitivity[i + 1].getData()[k]) / intervals[i]
            / intervals[i];
        coefSense[1][k] = (3. * slopeSensitivity[i][k] - firstWithSensitivity[i + 2].getData()[k] - 2. * firstWithSensitivity[i + 1].getData()[k])
            / intervals[i];
   ...
   *          First derivatives at xValues_i
   * @param second
   *          Second derivatives at xValues_i
   * @return Coefficient matrix whose i-th row vector is { a_n, a_{n-1}, ...} for the i-th interval, where a_n, a_{n-1},... are coefficients of f(x) = a_n
   *         (x-x_i)^n + a_{n-1} (x-x_i)^{n-1} + .... with n=5
   */
  public double[][] solve(final double[] values, final double[] intervals, final double[] slopes, final double[] first, final double[] second) {
    ...
    for (int i = 0; i < nInt; ++i) {
      ...
      res[i][2] = 0.5 * (second[i + 1] - 3. * second[i]) / intervals[i]
          + 2. * (5. * slopes[i] - 3. * first[i] - 2. * first[i + 1]) / intervals[i] / intervals[i];
      res[i][1] = 0.5 * (3. * second[i] - 2. * second[i + 1]) / intervals[i] / intervals[i]
          + (8. * first[i] + 7. * first[i + 1] - 15. * slopes[i]) / intervals[i] / intervals[i] / intervals[i];
      res[i][0] = 0.5 * (second[i + 1] - second[i]) / intervals[i] / intervals[i] / intervals[i]
          + 3. * (2. * slopes[i] - first[i + 1] - first[i]) / intervals[i] / intervals[i] / intervals[i] / intervals[i];
    }
   ...
   * @param firstWithSensitivity
   *          First derivative values at xValues_i and their yValues dependencies
   * @param secondWithSensitivity
   *          Second derivative values at xValues_i and their yValues dependencies
   * @return Coefficient matrix and its node dependencies
   */
  public DoubleMatrix2D[] solveWithSensitivity(final double[] values, final double[] intervals, final double[] slopes, final double[][] slopeSensitivity,
      final DoubleMatrix1D[] firstWithSensitivity,
    ...
    for (int i = 0; i < nData - 1; ++i) {
      ...
      coefSense[5][i] = 1.;
      for (int k = 0; k < nData; ++k) {
        coefSense[0][k] = 0.5 * (secondWithSensitivity[i + 2].getData()[k] - secondWithSensitivity[i + 1].getData()[k]) / intervals[i] / intervals[i]
            / intervals[i]
            + 3. * (2. * slopeSensitivity[i][k] - firstWithSensitivity[i + 2].getData()[k] - firstWithSensitivity[i + 1].getData()[k]) / intervals[i]
                / intervals[i] / intervals[i] / intervals[i];
        coefSense[1][k] = 0.5 * (3. * secondWithSensitivity[i + 1].getData()[k] - 2. * secondWithSensitivity[i + 2].getData()[k]) / intervals[i] / intervals[i]
            +
            (8. * firstWithSensitivity[i + 1].getData()[k] + 7. * firstWithSensitivity[i + 2].getData()[k] - 15. * slopeSensitivity[i][k]) / intervals[i]
        ...
        coefSense[2][k] = 0.5 * (secondWithSensitivity[i + 2].getData()[k] - 3. * secondWithSensitivity[i + 1].getData()[k]) / intervals[i] + 2.
            * (5. * slopeSensitivity[i][k] - 3. * firstWithSensitivity[i + 1].getData()[k] - 2. * firstWithSensitivity[i + 2].getData()[k]) / intervals[i]
            / intervals[i];
