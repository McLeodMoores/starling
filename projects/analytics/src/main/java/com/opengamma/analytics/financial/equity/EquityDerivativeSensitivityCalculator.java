...
public class EquityDerivativeSensitivityCalculator {
  ...
  /**
   * This calculates the sensitivity of the present value (PV) to a unit move in the forward. The volatility surface remains unchanged.
   *
   * @param derivative
  ...
  public Double calcForwardSensitivity(final InstrumentDerivative derivative, final StaticReplicationDataBundle market,
    ...
    // Shift UP
    StaticReplicationDataBundle bumpedMarket = new StaticReplicationDataBundle(market.getVolatilitySurface(), market.getDiscountCurve(),
        market.getForwardCurve()
            .withFractionalShift(relShift));
    final double pvUp = derivative.accept(_pricer, bumpedMarket);

    // Shift Down
    bumpedMarket = new StaticReplicationDataBundle(market.getVolatilitySurface(), market.getDiscountCurve(),
  ...
  /**
   * This calculates the sensitivity of the present value (PV) to a unit move in the forward, under a default shift of 1% of the forward.
   *
   * @param derivative
   *          the EquityDerivative
   * @param market
  ...
  /**
   * Calculates the sensitivity of the present value (PV) to a change in the funding rate from valuation to settlement. Also know as PVBP
   * and DV01, though note this return per UNIT change in rate. calcPV01 returns per basis point change in rates.
   * <p>
   * Rates enter the pricing of a EquityDerivative in two places: in the discounting and forward projection.
   * <p>
   * The presentValue has been structured such that the form of the PV = Z(t,T) * FwdPrice(t,T) with Z a zero coupon bond, and t and T the
   * valuation and settlement times respectively. The form of our discounting rates is such that Z(t,T) = exp[- R(t,T) * (T-t)], hence dZ/dR
   * = -(T-t)*Z(t,T) and d(PV)/dR = PV * dZ/dR The forward's dependence on the discounting rate is similar to the zero coupon bonds, but of
   * opposite sign, dF/dR = (T-t)*F(t,T)
   *
   * @param derivative
   *          the EquityDerivative
   * @param market
  ...
  /**
   * Calculates the sensitivity of the present value (PV) to a basis point (bp) move in the funding rates across all maturities.
   * <p>
  ...
  public DoubleMatrix1D calcDeltaBucketed(final InstrumentDerivative derivative, final StaticReplicationDataBundle market) {
    ...
    // This spreads the sensitivity across yield nodes if the curve interpolator is not local.
    ...
    final List<DoublesPair> sensitivityList = Arrays.asList(DoublesPair.of(settlement, sens));
  ...
  /**
   * This calculates the sensitivity of the present value (PV) to the lognormal Black implied volatilities at the knot points of the
   * surface.
   * <p>
   * Note - the change of the surface due to the movement of a single node is interpolator-dependent, so an instrument may have non-local
   * sensitivity
   *
   * @param derivative
   *          the EquityDerivative
   ...
   *          the EquityOptionDataBundle
   * @param shift
   *          Size of shift made in centered-finite difference approximation. e.g. 1% would be 0.01, and 1bp 0.0001
   * @return A Double. Currency amount per unit amount change in the black volatility
   */
  public Double calcBlackVegaParallel(final InstrumentDerivative derivative, final StaticReplicationDataBundle market, final double shift) {
    ...
    final double pvUp = derivative.accept(_pricer,
        new StaticReplicationDataBundle(upSurface, market.getDiscountCurve(), market.getForwardCurve()));

    // Parallel shift DOWN
    final BlackVolatilitySurface<?> downSurface = market.getVolatilitySurface().withShift(-shift, true);
    final double pvDown = derivative.accept(_pricer,
        new StaticReplicationDataBundle(downSurface, market.getDiscountCurve(), market.getForwardCurve()));
   ...
   * surface.
   * <p>
   * The return format is a DoubleMatrix2D with rows equal to the total number of maturities and columns equal to the number of strikes.
   * <p>
   * Note - the change of the surface due to the movement of a single node is interpolator-dependent, so an instrument may have non-local
   * sensitivity
   *
   ...
   * @param market
   *          the EquityOptionDataBundle
   * @return A NodalDoublesSurface with same axes as market.getVolatilitySurface(). Contains currency amount per unit amount change in the
   *         black volatility of each node
   */
  public NodalDoublesSurface calcBlackVegaForEntireSurface(final InstrumentDerivative derivative,
   ...
   *          the EquityOptionDataBundle
   * @param shift
   *          Size of shift made in centred-finite difference approximation. e.g. 1% would be 0.01, and 1bp 0.0001
   * @return A NodalDoublesSurface with same axes as market.getVolatilitySurface(). Contains currency amount per unit amount change in the
   *         black volatility of each node
   */
    ...
    if (market.getVolatilitySurface().getSurface() instanceof InterpolatedDoublesSurface) {
      ...
      if (derivative instanceof EquityIndexOption) {
        ...
        throw new OpenGammaRuntimeException(
            "Calculator with BlackVolatilitySurfaceMoneynessFcnBackedByGrid was expecting an EquityIndexOption.");
      }
      // Unpack
      ...
      for (int t = Math.max(0, expiryIndex - 3); t < Math.min(nExpiries, expiryIndex + 4); t++) {
        ...
        for (int k = Math.max(0, strikeIndex - 6); k < Math.min(nStrikes, strikeIndex + 7); k++) {
          ...
          final Function1D<Double, Double> thisExpirysSmile = strikeInterpolator.getVolatilityFunction(forwards[t], strikes[t],
              volExpiries[t], bumpedVols);
          ...
          // BlackVolatilitySurfaceMoneynessFcnBackedByGrid shiftedSurface = surfaceInterpolator.getBumpedVolatilitySurface(volGrid, t, k,
          // -shift);
  ...
  /**
   * Compute the price sensitivity to a shift of the Black volatility at a given maturity and strike.
   * <p>
   * Note - the change of the surface due to the movement of a single node is interpolator-dependent, so an instrument may have non-local
   * sensitivity.
   * <p>
   * Important!!! If the <i>(x, y)</i> value(s) of the shift(s) are not in the nodal points of the original surface, they are added (with
   * shift) to the nodal points of the new surface.
   *
   * @param derivative
   *          the EquityDerivative
   * @param market
   *          the EquityOptionDataBundle
   * @param maturity
   *          a double in same unit as VolatilitySurface
   * @param strike
   *          a double in same unit as VolatilitySurface
   * @param shift
   *          Size of shift made in centered-finite difference approximation. e.g. 1% would be 0.01, and 1bp 0.0001
  ...
  public double calcBlackVegaForSinglePoint(final InstrumentDerivative derivative, final StaticReplicationDataBundle market,
    ...
    StaticReplicationDataBundle bumpedMarket = new StaticReplicationDataBundle(market.getVolatilitySurface().withSurface(bumpedVolUp),
    ...
    bumpedMarket = new StaticReplicationDataBundle(market.getVolatilitySurface().withSurface(bumpedVolDown), market.getDiscountCurve(),
        market.getForwardCurve());
    final double pvDown = derivative.accept(_pricer, bumpedMarket);
