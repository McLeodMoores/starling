...
public class CapFloorCMSSpreadG2ppNumericalIntegrationMethod {
  ...
  public CurrencyAmount presentValue(final CapFloorCMSSpread cmsSpread, final G2ppPiecewiseConstantDataBundle g2Data) {
    ...
    final int[] nbCfFixed = new int[] { cmsSpread.getUnderlyingSwap1().getFixedLeg().getNumberOfPayments(),
                  cmsSpread.getUnderlyingSwap2().getFixedLeg().getNumberOfPayments() };
    ...
    for (int loopswap = 0; loopswap < 2; loopswap++) {
      ...
      for (int loopcf = 0; loopcf < nbCfIbor[loopswap]; loopcf++) {
        ...
        discountedCashFlowIbor[loopswap][loopcf] = dfIbor[loopswap][loopcf] * cfeIbor[loopswap].getNthPayment(loopcf).getAmount()
            / notionalSwap[loopswap];
      }
    ...
    for (int loopcf = 0; loopcf < nbCfFixed[0]; loopcf++) {
      ...
      discountedCashFlowFixed[0][loopcf] = dfFixed[0][loopcf]
          * cmsSpread.getUnderlyingSwap1().getFixedLeg().getNthPayment(loopcf).getPaymentYearFraction();
    }
    for (int loopcf = 0; loopcf < nbCfFixed[1]; loopcf++) {
      ...
      discountedCashFlowFixed[1][loopcf] = dfFixed[1][loopcf]
          * cmsSpread.getUnderlyingSwap2().getFixedLeg().getNthPayment(loopcf).getPaymentYearFraction();
    }
    // Model parameters
    ...
    for (int loopswap = 0; loopswap < 2; loopswap++) {
      ...
      for (int loopcf = 0; loopcf < nbCfFixed[loopswap]; loopcf++) {
        ...
        tau2Fixed[loopswap][loopcf] = alphaFixed[loopswap][loopcf][0] * alphaFixed[loopswap][loopcf][0]
            + alphaFixed[loopswap][loopcf][1] * alphaFixed[loopswap][loopcf][1] + 2 * rhog2pp * gamma[0][1]
      ...
      for (int loopcf = 0; loopcf < nbCfIbor[loopswap]; loopcf++) {
        ...
        tau2Ibor[loopswap][loopcf] = alphaIbor[loopswap][loopcf][0] * alphaIbor[loopswap][loopcf][0]
            + alphaIbor[loopswap][loopcf][1] * alphaIbor[loopswap][loopcf][1] + 2 * rhog2pp * gamma[0][1]
    ...
    // Integration
    final SpreadIntegrant integrant = new SpreadIntegrant(discountedCashFlowFixed, alphaFixed, tau2Fixed, discountedCashFlowIbor, alphaIbor,
        tau2Ibor, alphaTp, tau2Tp, rhobar, strike,
        cmsSpread.isCap());
    ...
    try {
      pv = 1.0 / (2.0 * Math.PI * Math.sqrt(1 - rhobar * rhobar))
          * integrator2D.integrate(integrant, new Double[] { -INTEGRATION_LIMIT, -INTEGRATION_LIMIT },
              new Double[] { INTEGRATION_LIMIT, INTEGRATION_LIMIT });
  ...
  private static final class SpreadIntegrant extends Function2D<Double, Double> {
    ...
    SpreadIntegrant(final double[][] discountedCashFlowFixed, final double[][][] alphaFixed, final double[][] tau2Fixed,
        final double[][] discountedCashFlowIbor, final double[][][] alphaIbor,
        final double[][] tau2Ibor, final double[] alphaTp, final double tau2Tp, final double rhobar, final double strike,
    ...
    public Double apply(final Double x0, final Double x1) {
      ...
      rate[0] = MODEL_G2PP.swapRate(x, _discountedCashFlowFixed[0], _alphaFixed[0], _tau2Fixed[0], _discountedCashFlowIbor[0],
          _alphaIbor[0], _tau2Ibor[0]);
      rate[1] = MODEL_G2PP.swapRate(x, _discountedCashFlowFixed[1], _alphaFixed[1], _tau2Fixed[1], _discountedCashFlowIbor[1],
          _alphaIbor[1], _tau2Ibor[1]);
