...
/**
 * ForwardCurve for Equity assets that are modelled to pay known discrete dividends with an affine form: d(i) = alpha[i] +
 * beta[i]*share_price(tau[i]).
 */
public class ForwardCurveAffineDividends extends ForwardCurve {
  ...
  protected static Curve<Double, Double> getForwardCurve(final double spot, final YieldAndDiscountCurve riskFreeCurve,
    ...
    final Function<Double, Double> f = t -> {
