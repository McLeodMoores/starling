...
public class EquityVarianceSwapBackwardsPurePDE {
  ...
  public double[] expectedVariance(final double spot, final YieldAndDiscountCurve discountCurve, final AffineDividends dividends,
    ...
    final Function<Double, Double> logPayoff = getLogPayoff(divs, expiry);
    ...
    final PDE1DDataBundle<ConvectionDiffusionPDE1DCoefficients> db = new PDE1DDataBundle<>(pde, logPayoff, lower, upper, grid);
  ...
  private double getCorrection(final AffineDividends ad, final EquityDividendsCurvesBundle curves, final int index,
    ...
    final Function<Double, Double> initalCond = getCorrectionInitialCondition(ad, curves, index, correctForDividends);

    final BoundaryCondition lower = new NeumannBoundaryCondition(
        getCorrectionLowerBoundaryCondition(ad, curves, index, correctForDividends, index), yMin, true);
    ...
    final PDEGrid1D grid = new PDEGrid1D(timeMesh, spaceMesh);
    final PDE1DDataBundle<ConvectionDiffusionPDE1DCoefficients> db = new PDE1DDataBundle<>(pde, initalCond, lower, upper, grid);
    final PDEResults1D res = _solver.solve(db);
  ...
  private Function<Double, Double> getLogPayoff(final EquityDividendsCurvesBundle divs, final double expiry) {
  ...
  private Function<Double, Double> getLowerBoundaryCondition(final EquityDividendsCurvesBundle divs, final double expiry) {
    final double f = divs.getF(expiry);
    final double d = divs.getD(expiry);
    return y -> {
      if (d == 0) {
  ...
  private Function<Double, Double> getCorrectionInitialCondition(final AffineDividends ad, final EquityDividendsCurvesBundle curves,
      final int index,
  ...
  private Function<Double, Double> getCorrectionLowerBoundaryCondition(final AffineDividends ad, final EquityDividendsCurvesBundle curves,
      final int index,
    ...
    // the assumption is that for very small x, x is stuck at low values, so the stock value at the dividend is just this projected by the
    // forward
    final double s = (f - d) * x + d;
    final double temp = Math.log(s * (1 - beta) / (s + alpha));
