 ...
 * multiplied by the notional and the payment accrual factor and discounted to settlement. The discounting to settlement is done using the forward rate over the
 * fixing period. The value is further discounted from settlement to today using the discounting curve. $$ \begin{equation*}
 * P^D(0,t_1)\frac{\delta_P(F-K)}{1+\delta_P F} \quad \mbox{and}\quad F = \frac{1}{\delta_F}\left( \frac{P^j(0,t_1)}{P^j(0,t_2)}-1\right) \end{equation*} $$
...
public final class ForwardRateAgreementDiscountingProviderMethod {
  ...
  public MultipleCurrencyAmount presentValue(final ForwardRateAgreement fra, final MulticurveProviderInterface multicurve) {
    ...
    final double forward = multicurve.getSimplyCompoundForwardRate(fra.getIndex(), fra.getFixingPeriodStartTime(), fra.getFixingPeriodEndTime(),
  ...
  public MultipleCurrencyMulticurveSensitivity presentValueCurveSensitivity(final ForwardRateAgreement fra, final MulticurveProviderInterface multicurve) {
    ...
    listForward
        .add(new SimplyCompoundedForwardSensitivity(fra.getFixingPeriodStartTime(), fra.getFixingPeriodEndTime(), fra.getFixingYearFraction(), forwardBar));
